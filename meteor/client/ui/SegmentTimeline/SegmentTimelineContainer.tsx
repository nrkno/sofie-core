...
export const SegmentTimelineContainer = withTracker<
	IProps,
	IState,
	ITrackedProps
>(
	...
	(data: ITrackedProps, props: IProps, nextProps: IProps): boolean => {
		...
		if (
			...
			((props.rundown.currentPartId !== nextProps.rundown.currentPartId ||
				...
				(data.parts.find(
					(i) =>
						i._id === props.rundown.currentPartId ||
						i._id === nextProps.rundown.currentPartId
				) ||
					data.parts.find(
						(i) =>
							i._id === props.rundown.nextPartId ||
							i._id === nextProps.rundown.nextPartId
					))) ||
		...
		if (
			...
			!_.isEqual(
				...
				nextProps.showStyleBase.outputLayers
			)
		) {
			return true;
		}
	...
	class extends MeteorReactComponent<IProps & ITrackedProps, IState> {
		...
		componentDidMount() {
			...
			window.addEventListener(
				RundownViewEvents.rewindsegments,
		...
		componentWillUnmount() {
			...
			if (
				...
				if (typeof this.props.onSegmentScroll === 'function')
					this.props.onSegmentScroll();
			}
			this.stopLive();
			...
			if (this.props.isLiveSegment && this.props.currentLivePart) {
				const partOffset =
					(this.context.durations &&
						...
						this.context.durations.partDisplayStartsAt[
							this.props.currentLivePart._id
						] -
							this.context.durations.partDisplayStartsAt[
		...
		visibleChanged = (entries: IntersectionObserverEntry[]) => {
			if (
				...
				!isMaintainingFocus() &&
				Date.now() - this.mountedTime > 2000
			) {
		...
		startLive = () => {
			...
			this.intersectionObserver.observe(
				this.timelineDiv.parentElement!.parentElement!
			);
		};
			...
			return (
				(this.props.segmentui && (
					<SegmentTimeline
						...
						onZoomChange={(newScale: number, e) =>
							this.props.onTimeScaleChange &&
							this.props.onTimeScaleChange(newScale)
						}
