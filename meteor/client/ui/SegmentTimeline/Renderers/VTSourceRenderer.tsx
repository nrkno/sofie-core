...
> {
	...
	updateTime = () => {
		if (this.vPreview) {
			...
			const itemDuration =
				(piece.content ? (piece.content.sourceDuration as number) : undefined) ||
	...
	getInspectorWarnings = (time: number): JSX.Element | undefined => {
		...
		const metadata = item.contentMetaData as MediaObject;
		const timebase =
			metadata.mediainfo && metadata.mediainfo.timebase ? metadata.mediainfo.timebase : 20;
		if (this.state.blacks) {
			...
			if (tot > 0)
				msgBlacks = `${Math.round(tot / timebase)} black frame${tot > timebase ? 's' : ''} in clip`;
		}
						...
						<div
							...
							<div>
								...
								<span className="mini-inspector__value">
									{this.props.piece.content && this.props.piece.content.fileName}
								</span>
							</div>
						</div>
					)}
