...
function getEditAttribute<TObj, TObj2>(
	collection: TransformedCollection<TObj2, TObj>,
	object: TObj,
...
interface IConfigManifestSettingsProps<
	TCol extends TransformedCollection<TObj2, TObj>,
	TObj,
	TObj2
> {
...
> {
	...
	render() {
		...
		const vals: TableConfigItemValue =
			objectPath.get(this.props.object, baseAttribute) || [];
			...
			<div>
				...
				<button
					...
					onClick={() => {
						...
						_.each(
							item2.columns,
							(col) => (rowDefault[col.id] = col.defaultVal)
						);
...
export class ConfigManifestSettings<
	...
	TObj2
> extends React.Component<
	Translated<IConfigManifestSettingsProps<TCol, TObj, TObj2>>,
	IConfigManifestSettingsState
> {
	constructor(
		props: Translated<IConfigManifestSettingsProps<TCol, TObj, TObj2>>
	) {
		super(props);
	...
	handleConfirmAddItemAccept = (e) => {
		if (this.state.addItemId) {
			const item = this.props.manifest.find(
				(c) => c.id === this.state.addItemId
			);
			const m: any = {};
			m[this.props.configPath] = literal<IConfigItem>({
	...
	renderConfigValue(
		...
		rawValue: ConfigItemValue | undefined
	) {
		const { t } = this.props;
	...
	getAddOptions() {
		...
		addOptions = this.props.manifest.map((c) => ({
			value: c.id,
			name: c.name
		}));
		...
		return (
			<div>
				...
				<ModalDialog
					...
					<p>
						{t(
							'Are you sure you want to delete this config item "{{configId}}"?',
							{
								configId:
				...
				<div className="mod mhs">
					<button
						...
						<Tooltip
							overlay={t(
								'More settings specific to this studio can be found here'
							)}
							visible={getHelpMode()}
							placement="right">
...
export function collectConfigs(
	item: Studio | ShowStyleBase | ShowStyleVariant
): ConfigManifestEntry[] {
