...
import { PartId, PartInstanceId } from '../dataModel/Ids'
...
interface BreakProps {
	rundownsBeforeNextBreak: DBRundown[]
...
export class RundownTimingCalculator {
	private temporaryPartInstances: Map<PartId, DBPartInstance> = new Map<PartId, DBPartInstance>()
	...
	updateDurations(
		...
		rundowns: DBRundown[],
		currentRundown: DBRundown | undefined,
		parts: DBPart[],
		partInstancesMap: Map<PartId, DBPartInstance>,
		...
		let rundownsBeforeNextBreak: DBRundown[] | undefined
	...
	private recalculateBreaks(orderedRundowns: DBRundown[], currentRundown: DBRundown | undefined) {
		if (!currentRundown) {
...
export namespace PlaylistTiming {
	...
	export function sortTiminings(a: DBRundown, b: DBRundown): number {
