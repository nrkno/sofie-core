...
> {
	...
	checkSmallMode = () => {
		...
		if (
			selAreaElementWidth &&
			selAreaElementWidth < this.SMALL_WIDTH_BREAKPOINT
		) {
			this.setState({
				smallMode: true
			});
	...
	outsideZoomAreaClick = (e: Event & any) => {
		...
		if (offset) {
			...
			let seconds =
				((e.clientX - this.offsetX) / this.state.width) *
				this.props.segmentDuration;
			seconds -= this.props.liveLineHistorySize / this.props.timeScale;
			if (this.props.onScroll) {
				this.props.onScroll(
					Math.min(Math.max(0, seconds), this.props.segmentDuration),
					e
				);
			}
	...
	zoomAreaLeftMove = (e: Event & any) => {
		let begin = Math.max(
			0,
			Math.min(1, (e.clientX - this.offsetX) / this.state.width)
		);
		let end =
	...
	render() {
		return (
			<div
				className="segment-timeline__zoom-area__controls"
