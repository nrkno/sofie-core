...
const SegmentTimelineZoom = class extends React.Component<
...
> {
	...
	calculateSegmentDuration(): number {
		...
		if (this.context && this.context.durations) {
			const durations = this.context
				.durations as RundownTiming.RundownTimingContext;
			this.props.parts.forEach((item) => {
				...
				const duration = Math.max(
					item.duration || item.renderedDuration || 0,
					(durations.partDisplayDurations &&
						durations.partDisplayDurations[item._id]) ||
						0
				);
				total += duration;
			});
	...
	getSegmentDuration(): number {
		return this.props.isLiveSegment
			? this.calculateSegmentDuration()
			: this.state.totalSegmentDuration;
	}
	...
	renderMiniLiveLine() {
		if (this.props.isLiveSegment) {
			let lineStyle = {
				left:
					(
						(this.props.livePosition / this.getSegmentDuration()) *
						100
					).toString() + '%'
			};
			...
			return (
				<div
					className="segment-timeline__zoom-area__liveline"
					style={lineStyle}></div>
			);
		}
	}
	...
	render() {
		return (
			<div className="segment-timeline__zoom-area-container">
				<div
					className="segment-timeline__zoom-area"
...
export class SegmentTimelineClass extends React.Component<
	Translated<IProps>,
	IStateHeader
> {
	...
	setSegmentRef = (el: HTMLDivElement) => {
		...
		if (typeof this.props.segmentRef === 'function')
			this.props.segmentRef(this as any, this.props.segment._id);

		if (this.segmentBlock) {
			this.segmentBlock.addEventListener('wheel', this.onTimelineWheel, {
				passive: false,
				capture: true
			});
		}
	...
	onTimelineTouchMove = (e: React.TouchEvent<HTMLDivElement> & any) => {
		if (e.touches.length === 2) {
			...
			this.props.onScroll(
				Math.max(
					0,
					this.props.scrollLeft + scrollAmount / this.props.timeScale
				),
				e
			);
			this._lastTouch = {
		...
		return (
			<div
				...
				<div
					...
					tabIndex={0}
					onClick={(e) =>
						this.props.onCollapseSegmentToggle &&
						this.props.onCollapseSegmentToggle(e)
					}>
					{this.props.rundown &&
						this.props.parts &&
				...
				<div className="segment-timeline__mos-id">
