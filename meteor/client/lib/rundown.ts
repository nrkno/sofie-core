...
export namespace RundownUtils {
	function padZerundown(input: number, places?: number): string {
		places = places || 2;
		return input < Math.pow(10, places - 1)
			? '0'.repeat(places - 1) + input.toString(10)
			: input.toString(10);
	}
	...
	export function formatTimeToTimecode(
		...
		return (
			sign +
			(hideFrames ? timeCodeString.substr(0, timeCodeString.length - 3) : timeCodeString)
		);
	}
	...
	export function getSourceLayerClassName(partType: SourceLayerType): string {
		...
		return ((SourceLayerType[partType] || 'unknown-sourceLayer-' + partType) + '')
			.toLowerCase()
			.replace(/_/g, '-');
	}
}
