...
export const GenericDeviceSettingsComponent = translate()(
	class GenericDeviceSettingsComponent extends React.Component<
		...
		IMosDeviceSettingsComponentState
	> {
		...
		addNewItem = (itemConfig: TableConfigManifestEntry, path: string) => {
			...
			if (itemConfig.isSubDevices && Object.keys(itemConfig.config).length > 1) {
				// adding subdevice with type
		...
		renderDeviceSummary(
			...
			deviceId: string,
			obj: any,
			isEdited: (deviceId: string) => boolean
		) {
		...
		renderDevices(configManifest: TableConfigManifestEntry, obj?: any, prefix?: string) {
			...
			if (deviceTypes.length === 1) {
											...
											<td colSpan={6}>
												...
												<div className="mod alright">
													<button
														...
														onClick={(e) => this.finishEditDevice(deviceId)}>
				...
				const propNames = [
					<th key="ID">ID</th>,
					..._.map(this.getConfigSummaryFields(configManifest), (f) => (
						<th key={f.columnName}>{f.columnName}</th>
					))
				];
				propNames.push(<th key="action">&nbsp;</th>);
						...
						{_.map(devices, (device: any, deviceId: string) => {
							const configField =
								configManifest.config[
									configManifest.deviceTypesMapping
			...
			return (
				<React.Fragment key={configField.id}>
					...
					<table className="expando settings-studio-device-table">
						<tbody>{this.renderDevices(configField, obj, prefix)}</tbody>
					</table>
		...
		renderConfigTable(configField: TableConfigManifestEntry, obj?: any, prefix?: string) {
			...
			if (configTypes.length === 1) {
					...
					<React.Fragment key={configField.id}>
						...
						<div className="mod mhs">
							<button
								...
								onClick={(e) =>
									this.addNewItem(configField, prefix + ((tableContent || []).length || 0))
								}>
								<FontAwesomeIcon icon={faPlus} />
							</button>
						</div>
				...
				const propNames = [
					..._.map(this.getConfigSummaryFields(configField), (f) => (
						<th key={f.columnName}>{f.columnName}</th>
					))
				];
				propNames.push(<th key="actions">&nbsp;</th>);
															...
															<div className="mod mvs mhs">
																<label className="field">
																	...
																	<EditAttribute
																		modifiedClassName="bghl"
																		attribute={
																			prefix + i + '.' + (configField.defaultType || 'type')
																		}
																		obj={this.props.device}
																		type="dropdown"
																		options={configTypes}
			...
			for (const configField of configManifest) {
				if (configField.type === ConfigManifestEntryType.TABLE) {
					if ((configField as TableConfigManifestEntry).isSubDevices) {
						fields.push(
							this.renderSubdeviceConfig(configField as TableConfigManifestEntry, obj, prefix)
						);
					} else {
						fields.push(
							this.renderConfigTable(
								configField as TableConfigManifestEntry,
								obj,
								prefix + configField.id + '.'
							)
						);
					}
				} else {
		...
		render() {
			...
			return (
				<div>
					...
					{this.renderConfigFields(
						this.props.device.configManifest.deviceConfig,
						device,
						'settings.'
					)}
						...
						<p>
							{t('Are you sure you want to remove {{type}} "{{deviceId}}"?', {
								deviceId:
									...
									this.state.deleteConfirmItemPath.split('.').pop(),
								type:
									this.state.deleteConfirmItemPath &&
