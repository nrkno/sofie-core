			...
			filter: (durations: RundownTiming.RundownTimingContext) => {
				return [
					...
					props.firstPartInSegment
						? ((durations || {})['partDisplayStartsAt'] || {})[props.firstPartInSegment._id]
						: undefined,
					props.firstPartInSegment
						? ((durations || {})['partDisplayDurations'] || {})[props.firstPartInSegment._id]
						: undefined
				];
			}
		};
		...
		class SegmentTimelinePart0 extends React.Component<Translated<WithTiming<IProps>>, IState> {
			...
			static getDerivedStateFromProps(nextProps: IProps & RundownTiming.InjectedROTimingProps) {
				...
				const isDurationSettling =
					!!nextProps.rundown.active && !isLive && !!startedPlayback && !nextProps.part.duration;
						...
						? Math.max(
								...
								nextProps.timingDurations.partDurations
									? nextProps.part.displayDuration ||
											nextProps.timingDurations.partDurations[nextProps.part._id]
									: 0
						  )
						: 0;
			...
			static getCurrentLiveLinePosition(part: PartUi, currentTime: number) {
				if (part.startedPlayback && part.getLastStartedPlayback()) {
					if (part.duration) {
						...
						return (
							currentTime - (part.getLastStartedPlayback() || 0) + (part.getLastPlayOffset() || 0)
						);
					}
				} else {
					return 0;
			...
			getLayerStyle() {
				...
				if (this.props.relative) {
					return {
						width:
							((this.getPartDuration() / (this.props.totalSegmentDuration || 1)) * 100).toString() +
							'%',
				...
				return Math.max(
					...
					this.props.part.duration ||
						(this.props.timingDurations.partDisplayDurations &&
							this.props.timingDurations.partDisplayDurations[this.props.part._id]) ||
						...
						Math.min(
							Math.max(
								...
								(this.props.livePosition || 0) +
									...
									(this.props.part.expectedDuration || this.props.part.renderedDuration || 0)
							),
							SegmentTimelinePart0.getLiveLineTimePadding(this.props.timeScale)
						) *
							this.props.timeScale +
			...
			render() {
				...
				const isEndOfShow =
					...
					(!this.state.isLive || (this.state.isLive && !this.props.rundown.nextPartId));
									...
									<div
										...
										style={{
											left: this.props.relative
												? (this.props.rundown.nextTimeOffset / (this.getPartDuration() || 1)) *
														100 +
												  '%'
