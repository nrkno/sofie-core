...
import { selectNextPart, isTooCloseToAutonext, getSelectedPartInstancesFromCache, getAllOrderedPartsFromCache } from '../playout/lib'
...
export namespace UpdateNext {
	...
	export function afterInsertParts (cache: CacheForRundownPlaylist, playlist: RundownPlaylist, newPartExternalIds: string[], removePrevious: boolean) {
		if (playlist && playlist.active) {
			...
			if (!playlist.nextPartInstanceId && playlist.currentPartInstanceId) {
				...
				const newNextPart = selectNextPart(playlist, currentPartInstance || null, getAllOrderedPartsFromCache(cache, playlist))
				ServerPlayoutAPI.setNextPartInner(cache, playlist, newNextPart ? newNextPart.part : null)
