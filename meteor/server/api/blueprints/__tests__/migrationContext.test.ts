...
describe('Test blueprint migrationContext', () => {
	...
	describe('MigrationContextStudio', () => {
		...
		describe('mappings', () => {
			function getMappingFromDb(
				studio: Studio,
				mappingId: string
			): MappingExt | undefined {
		...
		describe('devices', () => {
			...
			testInFiber('insertDevice: ok', () => {
				const ctx = getContext();
				const studio = getStudio(ctx);
				const initialSettings = getPlayoutDevice(studio)
	...
	describe('MigrationContextShowStyle', () => {
		...
		describe('variants', () => {
			...
			testInFiber('getAllVariants: missing base', () => {
				const ctx = new MigrationContextShowStyle({
					_id: 'fakeStyle'
				} as any);
				const variants = ctx.getAllVariants();
		...
		describe('sourcelayer', () => {
			function getAllSourceLayersFromDb(
			...
			testInFiber('updateSourceLayer: missing', () => {
				...
				try {
					...
					expect(e.message).toBe(
						`[404] SourceLayer "fake99" cannot be updated as it does not exist`
					);
				}
			...
			testInFiber('removeSourceLayer: good', () => {
				...
				const expectedSourceLayers = _.filter(
					initialSourceLayers,
		...
		describe('outputlayer', () => {
			function getAllOutputLayersFromDb(
				showStyle: ShowStyleBase
			): IOutputLayer[] {
				const showStyle2 = ShowStyleBases.findOne(
					showStyle._id
			...
			testInFiber('updateOutputLayer: missing', () => {
				...
				try {
					...
					expect(e.message).toBe(
						`[404] OutputLayer "fake99" cannot be updated as it does not exist`
					);
				}
			...
			testInFiber('removeOutputLayer: good', () => {
				...
				const expectedOutputLayers = _.filter(
					initialOutputLayers,
					(layer) => layer._id !== 'lay1'
				);
		...
		describe('variant-config', () => {
			function getAllVariantConfigFromDb(
				ctx: MigrationContextShowStyle,
				variantId: string
			): IConfigItem[] {
				const variant = ShowStyleVariants.findOne(
					ctx.getVariantId(variantId)
				) as ShowStyleVariant;
				expect(variant).toBeTruthy();
			...
			testInFiber('setVariantConfig: no id', () => {
				...
				expect(getAllVariantConfigFromDb(ctx, 'configVariant')).toEqual(
			...
			testInFiber('setVariantConfig: insert', () => {
				...
				expect(ctx.getVariantConfig('configVariant', 'conf19')).toEqual(
					expectedItem.value
				);
		...
		describe('runtimeArguments', () => {
			function getAllRuntimeArgumentsFromDb(
				showStyle: ShowStyleBase
			): IBlueprintRuntimeArgumentsItem[] {
				const showStyle2 = ShowStyleBases.findOne(
					showStyle._id
			...
			testInFiber('getRuntimeArgument: good', () => {
				...
				const ra = ctx.getRuntimeArgument(
					'ra0'
				) as IBlueprintRuntimeArgumentsItem;
			...
			testInFiber('insertRuntimeArgument: no id', () => {
				...
				expect(getShowStyle(ctx).runtimeArguments).toEqual(
					initialRuntimeArguments
				);
				expect(getAllRuntimeArgumentsFromDb(showStyle)).toEqual(
					initialRuntimeArguments
				);
			});
			...
			testInFiber('updateRuntimeArgument: missing', () => {
				...
				try {
					...
					expect(e.message).toBe(
						`[404] RuntimeArgument "fake99" cannot be updated as it does not exist`
					);
				}
			...
			testInFiber('removeRuntimeArgument: good', () => {
				...
				const expectedRuntimeArguments = _.filter(
					initialRuntimeArguments,
					(ra) => ra._id !== 'lay1'
				);
				expect(getShowStyle(ctx).runtimeArguments).toEqual(
