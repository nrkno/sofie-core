...
export namespace ServerPlayoutAdLibAPI {
			...
			() => {
				...
				if (!rundown.active)
					throw new Meteor.Error(
						403,
						`Part AdLib-pieces can be only placed in an active rundown!`
					);
				...
				if (sourceL && sourceL.type !== SourceLayerType.GRAPHICS)
					throw new Meteor.Error(
						403,
						`Piece "${pieceId}" is not a GRAPHICS item!`
					);
				...
				if (piece.partId === part._id) {
					...
					if (
						...
						piece.startedPlayback <= getCurrentTime()
					) {
						if (
							resPiece &&
							...
							throw new Meteor.Error(
								409,
								`Piece "${piece._id}" is currently live and cannot be used as an ad-lib`
							);
						}
					}
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (
					...
					throw new Meteor.Error(
						403,
						`Part AdLib-pieces can not be used in combination with hold!`
					);
				}
				...
				if (!adLibPiece)
					throw new Meteor.Error(
						404,
						`Part Ad Lib Item "${adLibPieceId}" not found!`
					);
				if (adLibPiece.invalid)
					throw new Meteor.Error(
						404,
						`Cannot take invalid Part Ad Lib Item "${adLibPieceId}"!`
					);
				if (adLibPiece.floated)
					throw new Meteor.Error(
						404,
						`Cannot take floated Part Ad Lib Item "${adLibPieceId}"!`
					);

				if (!queue && rundown.currentPartId !== partId)
					throw new Meteor.Error(
						403,
						`Part AdLib-pieces can be only placed in a currently playing part!`
					);
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (!adLibPiece)
					throw new Meteor.Error(
						404,
						`Rundown Baseline Ad Lib Item "${baselineAdLibPieceId}" not found!`
					);
				if (!queue && rundown.currentPartId !== partId)
					throw new Meteor.Error(
						403,
	...
	function innerStartAdLibPiece(
		...
		if (queue) {
			...
			Pieces.find({ rundownId: rundown._id, partId: orgPartId }).forEach(
				(piece) => {
					// console.log(piece.name + ' has life span of ' + piece.infiniteMode)
					if (
						piece.infiniteMode &&
						piece.infiniteMode >= PieceLifespan.Infinite
					) {
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (!rundown.active)
					throw new Meteor.Error(
						403,
						`Part AdLib-copy-pieces can be only manipulated in an active rundown!`
					);
				if (rundown.currentPartId !== partId)
					throw new Meteor.Error(
						403,
						`Part AdLib-copy-pieces can be only manipulated in a current part!`
					);
				...
				if (!piece)
					throw new Meteor.Error(
						404,
						`Part AdLib-copy-piece "${pieceId}" not found!`
					);
				if (!piece.dynamicallyInserted)
					throw new Meteor.Error(
						501,
						`"${pieceId}" does not appear to be a dynamic Piece!`
					);
				if (!piece.adLibSourceId)
					throw new Meteor.Error(
						501,
						`"${pieceId}" does not appear to be a Part AdLib-copy-piece!`
					);
				...
				if (!tlObj)
					throw new Meteor.Error(
						404,
						`Part AdLib-copy-piece "${pieceId}" not found in the playout Timeline!`
					);
				...
				if (piece.startedPlayback) {
					...
					logger.warn(
						`"${pieceId}" timeline object is not positioned absolutely or is still set to play now, assuming it's about to be played.`
					);
				}
