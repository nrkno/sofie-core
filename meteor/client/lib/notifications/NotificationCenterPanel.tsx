...
import {
	...
	Translated,
	withTracker
} from '../ReactMeteorData/ReactMeteorData';
...
class NotificationPopUp extends React.Component<IPopUpProps> {
	triggerEvent = (action: NotificationAction, e) => {
		if (action.action) {
			...
			if (
				this.props.item.actions &&
				this.props.item.actions.find((i) => i.type === action.type)
			) {
	...
	render() {
		...
		const defaultActions: NotificationAction[] = _.filter(
			item.actions || [],
			(i) => i.type === 'default'
		);
			...
			<div
				...
				{this.props.showDismiss && (
					<ContextMenuTrigger
						...
						attributes={{
							className: 'notification-pop-up__dismiss'
						}}>
 ...
 */
export const NotificationCenterPopUps = translateWithTracker<
	IProps,
	IState,
	ITrackedProps
>((props: IProps, state: IState) => {
	...
	class NotificationCenterPopUps extends MeteorReactComponent<
	...
	> {
		...
		UNSAFE_componentWillUpdate() {
			Array.from(
				document.querySelectorAll('.notification-pop-up.is-highlighted')
		...
		componentDidUpdate(prevProps, prevState, snapshot) {
			...
			if (
				...
				const items: NodeListOf<HTMLElement> = document.querySelectorAll(
					'.notification-pop-up.is-highlighted'
				);
				if (items.length > 0) {
					// scroll to highlighted items
					const currentAnimationName = getComputedStyle(
						items[0]
					).getPropertyValue('animation-name');

					const container = document.querySelector(
						'.notification-center-panel .notification-pop-ups'
					);
					if (container) {
		...
		render() {
			...
			const displayList = this.props.notifications
				...
				.map((item) => (
					<NotificationPopUp
						key={
							item.created +
							(item.message || 'undefined').toString() +
							(item.id || '')
						}
						item={item}
				...
				(this.props.showEmptyListLabel || displayList.length > 0) && (
					<div className="notification-pop-ups">
						...
						<ContextMenu id="context-menu-dissmiss-all">
							<MenuItem onClick={() => this.dismissAll()}>
 ...
 */
export const NotificationCenterPanelToggle = withTracker<
	IToggleProps,
	{},
	ITrackedCountProps
>(() => {
	...
	class NotificationCenterPanelToggle extends MeteorReactComponent<
		IToggleProps & ITrackedCountProps
	> {
		render() {
			return (
				<button
