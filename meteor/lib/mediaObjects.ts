...
export function getAcceptedFormats(
	...
	const formatsString: string =
		(formatsConfigField && formatsConfigField !== ''
			? formatsConfigField
			: '1920x1080i5000') + '';
	return _.compact(
		formatsString.split(',').map((res) => {
			const match = /((\d+)x(\d+))?((i|p|\?)(\d+))?((tff)|(bff))?/.exec(
				res.trim()
			);
			if (match) {
...
export function checkPieceContentStatus(
	...
	t =
		t ||
		((s: string, options?: _.Dictionary<any>) =>
			_.template(s, { interpolate: /\{\{(.+?)\}\}/g })(options));
	let newStatus: RundownAPI.PieceStatusCode =
								...
								for (const stream of mediaObject.mediainfo
									...
									if (stream.width && stream.height) {
										...
										if (!acceptFormat(format, formats)) {
											messages.push(
												t(
													'Source format ({{format}}) is not in accepted formats',
													{ format }
												)
											);
										}
