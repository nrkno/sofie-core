...
import {
	RecordedFiles,
	RecordedFile
} from '../../lib/collections/RecordedFiles';
import {
	Studios,
	Studio,
	ITestToolsConfig,
...
export function getStudioConfig(studio: Studio): ITestToolsConfig {
	const config: ITestToolsConfig = studio.testToolsConfig || {
		recordings: defaultConfig
	};
...
export function generateRecordingTimelineObjs(
	...
	if (!config.recordings.decklinkDevice)
		throw new Meteor.Error(
			500,
			`Recording decklink for Studio "${studio._id}" not defined!`
		);

	if (!studio.mappings[layerInput] || !studio.mappings[layerRecord]) {
		throw new Meteor.Error(
			500,
			`Recording layer mappings in Studio "${studio._id}" not defined!`
		);
	}
...
export namespace ServerTestToolsAPI {
	...
	export function recordStart(studioId: string, name: string) {
		...
		if (active)
			throw new Meteor.Error(
				404,
				`An active recording for "${studioId}" was found!`
			);

		if (name === '')
			name = moment(getCurrentTime()).format('YYYY-MM-DD HH:mm:ss');

		const config = getStudioConfig(studio);
		if (!config.recordings.channelIndex)
			throw new Meteor.Error(
				500,
				`Recording channel for Studio "${studio._id}" not defined!`
			);
		if (!config.recordings.deviceId)
			throw new Meteor.Error(
				500,
				`Recording device for Studio "${studio._id}" not defined!`
			);
		if (!config.recordings.decklinkDevice)
			throw new Meteor.Error(
				500,
		...
		const setter: any = {};
		setter['mappings.' + layerInput] = literal<
			TSR.MappingCasparCG & MappingExt
		>({
		...
		setter['mappings.' + layerRecord] = literal<
			TSR.MappingCasparCG & MappingExt
		>({
			device: TSR.DeviceType.CASPARCG,
			deviceId: config.recordings.deviceId,
	...
	export function recordDelete(id: string) {
		...
		if (!config.recordings.urlPrefix)
			throw new Meteor.Error(
				500,
				`URL prefix for Studio "${studio._id}" not defined!`
			);

		const p = deleteRequest({
			uri: config.recordings.urlPrefix + file.path
		}).then((res) => {
...
methods[TestToolsAPI.methods.recordStart] = (
	studioId: string,
	name: string
) => {
...
_.each(methods, (fcn: Function, key) => {
	methods[key] = (...args: any[]) => {
		...
		try {
			...
			logger.error(
				e.message || e.reason || (e.toString ? e.toString() : null) || e
			);
			throw e;
		}
	};
