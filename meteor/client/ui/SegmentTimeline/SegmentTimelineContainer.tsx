...
import { RundownTiming, TimingEvent, computeSegmentDuration } from '../RundownView/RundownTiming';
	...
	(data: ITrackedProps, props: IProps, nextProps: IProps): boolean => {
		...
		if (
			...
			((props.rundown.currentPartId !== nextProps.rundown.currentPartId ||
				...
				data.parts &&
				(data.parts.find(
					(i) => i._id === props.rundown.currentPartId || i._id === nextProps.rundown.currentPartId
				) ||
					data.parts.find(
						(i) => i._id === props.rundown.nextPartId || i._id === nextProps.rundown.nextPartId
					))) ||
	...
	class extends MeteorReactComponent<IProps & ITrackedProps, IState> {
				...
				this.setState(
					_.extend(
						...
						this.state.followLiveLine
							? {
									scrollLeft: Math.max(
										newLivePosition - this.props.liveLineHistorySize / this.props.timeScale,
										0
									)
							  }
							: null
		...
		onFollowLiveLine = (state: boolean, event: any) => {
			this.setState({
				...
				scrollLeft: Math.max(
					this.state.livePosition - this.props.liveLineHistorySize / this.props.timeScale,
					0
				)
			});
