...
function postHandler(
	...
	if (
		!criticality ||
		(typeof criticality === 'string' && criticality.trim().length < 1)
	) {
		res.statusCode = 400;
	...
	if (
		isNaN(criticality) ||
		validCriticalities.indexOf(Number(criticality)) < 0
	) {
		res.statusCode = 400;
		res.end(
			`Invalid value for criticality: ${criticality}, wanted one of ${validCriticalities.join(
				','
			)}`
		);
		return;
