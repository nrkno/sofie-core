		...
		if (this.context && this.context.durations) {
			...
			this.props.parts.forEach((item) => {
				...
				const duration = Math.max(
					...
					(durations.partDisplayDurations && durations.partDisplayDurations[item._id]) || 0
				);
				total += duration;
			});
		} else {
...
export class SegmentTimelineClass extends React.Component<Translated<IProps>, IStateHeader> {
	...
	onTimelineTouchMove = (e: React.TouchEvent<HTMLDivElement> & any) => {
		if (e.touches.length === 2) {
			...
			this.props.onScroll(
				Math.max(0, this.props.scrollLeft + scrollAmount / this.props.timeScale),
				e
			);
			this._lastTouch = {
						...
						return (
							<div
								...
								className={ClassNames('segment-timeline__output-layer-control', {
									collapsable:
										outputLayer.sourceLayers !== undefined && outputLayer.sourceLayers.length > 1,
										...
										.map((sourceLayer, index, array) => {
											return (
												<div
													...
													{array.length === 1 || sourceLayer.name === outputLayer.name
														? ' '
														: sourceLayer.name}
												</div>
											);
		...
		return (
			<div
						...
						<div className="segment-timeline__title__notes">
							{criticalNotes > 0 && (
								<div
									...
									onClick={(e) =>
										this.props.onHeaderNoteClick && this.props.onHeaderNoteClick(NoteType.ERROR)
									}>
							...
							{warningNotes > 0 && (
								<div
									className="segment-timeline__title__notes__note"
									onClick={(e) =>
										this.props.onHeaderNoteClick && this.props.onHeaderNoteClick(NoteType.WARNING)
									}>
									<img className="icon" src="/icons/warning_icon.svg" />
									<div>
										{t('Warnings')}:&nbsp;
				...
				<div
					...
					tabIndex={0}
					onClick={(e) =>
						this.props.onCollapseSegmentToggle && this.props.onCollapseSegmentToggle(e)
					}>
					{this.props.rundown &&
						this.props.parts &&
