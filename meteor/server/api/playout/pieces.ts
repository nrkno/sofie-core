...
export function getResolvedPieces(part: Part): Piece[] {
	...
	if (tlResolved.statistics.unresolvedCount > 0) {
		logger.warn(
			`Got ${tlResolved.statistics.unresolvedCount} unresolved pieces for piece #${
				part._id
			} (${unresolvedIds.join(', ')})`
		);
	}
			...
			for (let i = index + 1; i < source.length; i++) {
				...
				if (piece.sourceLayerId === sourcePiece.sourceLayerId) {
					...
					piece.playoutDuration =
						(sourcePiece.enable.start as number) - (piece.enable.start as number);
					return;
				}
			}
		}
...
export function getResolvedPiecesFromFullTimeline(
	...
	const objs = clone(
		allObjs.filter(
			(o) => o.isGroup && ((o as any).isPartGroup || (o.metadata && o.metadata.pieceId))
		)
	);
	...
	if (rundownData.rundown.currentPartId && rundownData.rundown.nextPartId) {
		...
		if (part && part.autoNext) {
			pieces.push(
				...rundownData.pieces.filter((p) => p.partId === rundownData.rundown.nextPartId)
			);
		}
	}
	...
	if (tlResolved.statistics.unresolvedCount > 0) {
		logger.warn(
			`Got ${tlResolved.statistics.unresolvedCount} unresolved pieces (${unresolvedIds.join(
				', '
			)})`
		);
	}
