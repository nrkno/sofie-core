...
export class ShelfBase extends React.Component<Translated<ShelfProps>, IState> {
	...
	constructor(props: Translated<ShelfProps>) {
		...
		this.state = {
			moving: false,
			shelfHeight:
				localStorage.getItem('rundownView.shelf.shelfHeight') || '50vh',
			overrideHeight: undefined,
			selectedTab: UIStateStorage.getItem(
				`rundownView.${props.rundown._id}`,
				'shelfTab',
				undefined
			) as string | undefined
		};
	...
	componentDidUpdate(prevProps: ShelfProps, prevState: IState) {
		if (
			...
			prevState.shelfHeight !== this.state.shelfHeight
		) {
			if (
				this.props.onChangeBottomMargin &&
				typeof this.props.onChangeBottomMargin === 'function'
			) {
	...
	restoreDefaultTab() {
		if (
			...
			const defaultTab = this.props.rundownLayout.filters.find(
				(i) => i.default
			);
			if (defaultTab) {
				this.setState({
	...
	getHeight(): string {
		const top = parseFloat(
			this.state.shelfHeight.substr(0, this.state.shelfHeight.length - 2)
		);
	...
	dropHandle = (e: MouseEvent) => {
		...
		if (Date.now() - this._mouseDown > 350) {
			if (
				...
				window.innerHeight - this.state.overrideHeight > CLOSE_MARGIN
			) {
				stateChange = _.extend(stateChange, {
					shelfHeight:
						Math.max(0.1, 0, this.state.overrideHeight / window.innerHeight) *
							100 +
						'vh'
				});
	...
	grabHandle = (e: React.MouseEvent<HTMLDivElement>) => {
		...
		if (handlePosition) {
			this._mouseOffset.x =
				handlePosition.left - window.scrollX - this._mouseStart.x;
			this._mouseOffset.y =
				handlePosition.top - window.scrollY - this._mouseStart.y;
		}
		...
		return (
			<React.Fragment>
				<div className="rundown-view__shelf__tabs">
					<OverflowingContainer className="rundown-view__shelf__tabs__tab-group">
						<div
							className={ClassNames('rundown-view__shelf__tabs__tab', {
								selected:
									(this.state.selectedTab || DEFAULT_TAB) === ShelfTabs.ADLIB
							})}
							onClick={(e) => this.switchTab(ShelfTabs.ADLIB)}
							tabIndex={0}>
							{t('AdLib')}
						</div>
						{rundownLayout &&
							rundownLayout.filters
								...
								.map((panel) => (
									<div
										className={ClassNames('rundown-view__shelf__tabs__tab', {
											selected:
												(this.state.selectedTab || DEFAULT_TAB) ===
												`${ShelfTabs.ADLIB_LAYOUT_FILTER}_${panel._id}`
										})}
										key={panel._id}
										onClick={(e) =>
											this.switchTab(
					...
					<div
						className={ClassNames('rundown-view__shelf__tabs__tab', {
							selected:
								(this.state.selectedTab || DEFAULT_TAB) ===
								ShelfTabs.GLOBAL_ADLIB
						})}
						onClick={(e) => this.switchTab(ShelfTabs.GLOBAL_ADLIB)}
						tabIndex={0}>
						{t('Global AdLib')}
					</div>
					<div
						className={ClassNames('rundown-view__shelf__tabs__tab', {
							selected:
								(this.state.selectedTab || DEFAULT_TAB) ===
								ShelfTabs.SYSTEM_HOTKEYS
						})}
						onClick={(e) => this.switchTab(ShelfTabs.SYSTEM_HOTKEYS)}
						tabIndex={0}>
						{t('Shortcuts')}
					</div>
				</div>
				<div className="rundown-view__shelf__panel super-dark">
					...
					<GlobalAdLibPanel
						visible={
							(this.state.selectedTab || DEFAULT_TAB) === ShelfTabs.GLOBAL_ADLIB
						}
			...
			<div
				...
				<ErrorBoundary>
					...
					RundownLayoutsAPI.isRundownLayout(this.props.rundownLayout)
						...
						: this.props.rundownLayout &&
						  RundownLayoutsAPI.isDashboardLayout(this.props.rundownLayout)
