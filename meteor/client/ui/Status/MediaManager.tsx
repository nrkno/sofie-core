...
const MediaManagerWorkFlowItem: React.SFC<IItemProps &
	i18next.InjectedTranslateProps> = (
				...
				<div className="workflow__header__summary">
					{i.comment && i.name !== i.comment ? (
						<div className="workflow__header__name">
							<span className="workflow__header__name__name">
								{i.name || 'Unnamed Workflow'}
							</span>
							<span className="workflow__header__name__comment">
								{i.comment}
							</span>
						</div>
					) : (
					...
					<div
						className="workflow__header__expand"
						onClick={() => props.toggleExpanded(i._id)}>
				...
				<div className="workflow__header__actions">
					<Tooltip overlay={t('Restart')} placement="top">
						<button
							...
							onClick={(e) => props.actionRestart(e, i)}>
							...
							.map((j) => (
								<div
									...
									<div className="workflow__step__action pas">
										{actionLabel(t, j.action)}
									</div>
									<div className="workflow__step__status pas">
										{workStepStatusLabel(t, j)}
									</div>
	...
	> {
		...
		actionRestartAll = (event: React.MouseEvent<HTMLElement>) => {
			doUserAction(
				this.props.t,
				event,
				UserActionAPI.methods.mediaRestartAllWorkflows,
				[]
			);
		};
		actionAbortAll = (event: React.MouseEvent<HTMLElement>) => {
			doUserAction(
				this.props.t,
				event,
				UserActionAPI.methods.mediaAbortAllWorkflows,
				[]
			);
		};
				...
				<div className="mhl gutter media-manager-status">
					...
					<div className="mod mvl alright">
						<button
							...
							onClick={this.actionAbortAll}>
							{t('Abort All')}
						</button>
						<button
							className="btn btn-secondary mls"
							onClick={this.actionRestartAll}>
							{t('Restart All')}
						</button>
					</div>
