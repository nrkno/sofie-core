...
export class Rundown implements DBRundown {
	...
	getShowStyleBase(): ShowStyleBase {
		...
		if (!showStyleBase)
			throw new Meteor.Error(404, `ShowStyleBase "${this.showStyleBaseId}" not found!`);
	...
	remove() {
		if (!Meteor.isServer)
			throw new Meteor.Error('The "remove" method is available server-side only (sorry)');
...
export const Rundowns: TransformedCollection<Rundown, DBRundown> = createMongoCollection<Rundown>(
	'rundowns',
