			...
			<div
				...
				{this.props.showDismiss && (
					<ContextMenuTrigger
						...
						attributes={{ className: 'notification-pop-up__dismiss' }}>
...
export const NotificationCenterPopUps = translateWithTracker<IProps, IState, ITrackedProps>(
	...
	> {
		...
		UNSAFE_componentWillUpdate() {
			Array.from(document.querySelectorAll('.notification-pop-up.is-highlighted')).forEach(
				(element: HTMLElement) => {
		...
		componentDidUpdate(prevProps, prevState, snapshot) {
			...
			if (
				...
				if (items.length > 0) {
					...
					const currentAnimationName = getComputedStyle(items[0]).getPropertyValue(
						'animation-name'
					);
