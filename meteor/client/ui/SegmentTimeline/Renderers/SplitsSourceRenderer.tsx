...
export class SplitsSourceRenderer extends CustomLayerItemRenderer<IProps, IState> {
	...
	constructor(props) {
		...
		this.state = {
			subItems: _.map(
				(props.piece.content as SplitsContent).boxSourceConfiguration,
				(item, index) => {
							...
							style={{
								...
								clipPath:
									item.content && item.content.crop
										? `inset(${item.content.crop.top * 100}% ${item.content.crop.right *
												100}% ${item.content.crop.bottom * 100}% ${item.content.crop.left * 100}%)`
										: undefined
							}}>
