...
export default class SplitInputIcon extends React.Component<{
	...
	getCameraLabel(piece: Piece | undefined) {
		if (piece && piece.content) {
			...
			const camera = c.boxSourceConfiguration.find(
				(i) => i.type === SourceLayerType.CAMERA
			);
	...
	getRightSourceType(piece: Piece | undefined): string {
		if (piece && piece.content) {
			...
			return (
				sourceType +
				(this.getLeftSourceType(piece) === sourceType ? ' second' : '')
			);
		}
		return 'remote';
	}
		...
		return (
			<svg
				...
				<rect
					width="126.5"
					height="44.5"
					className={this.getLeftSourceType(this.props.piece)}
				/>
				<rect
					width="126.5"
					height="44.5"
					y="44.5"
					className={this.getRightSourceType(this.props.piece)}
				/>
				<text
					x="9.6414976"
					textLength="106.5"
