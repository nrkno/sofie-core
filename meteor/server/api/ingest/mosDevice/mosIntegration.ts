...
export namespace MosIntegration {
	...
	export function mosRoStoryStatus(
		...
		if (part) {
			...
			throw new Meteor.Error(
				404,
				`Part ${status.ID} in rundown ${status.RunningOrderId} not found`
			);
	}
	export function mosRoStoryInsert(
		...
		logger.info(
			`mosRoStoryInsert after "${Action.StoryID}" Stories: ${Stories}`
		);
		// @ts-ignore
	...
	export function mosRoStoryDelete(
		...
		logger.info(
			`mosRoStoryDelete "${Action.RunningOrderID}" Stories: ${Stories}`
		);
