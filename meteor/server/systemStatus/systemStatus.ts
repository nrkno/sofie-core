	...
	_.each(devices, (device: PeripheralDevice) => {
		...
		if (deviceStatus === StatusCode.GOOD) {
			if (device.expectedVersions) {
				...
				_.each(
					device.expectedVersions,
					(expectedVersionStr, libraryName: string) => {
						...
						if (semver.satisfies(version, '0.0.0')) {
							...
							messages.push(
								`${libraryName}: Expected version ${expectedVersionStr}, got undefined`
							);
							...
							if (
								version0 &&
								expectedVersion0 &&
								version0.major !== expectedVersion0.major
							) {
								statusCode = StatusCode.BAD;
		...
		let so: StatusResponse = {
			...
			documentation: '',
			statusMessage: deviceStatusMessages.length
				? deviceStatusMessages.join(', ')
				: undefined,
			_internal: {
