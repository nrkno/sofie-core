...
import {
	...
	ICustomLayerItemProps
} from './CustomLayerItemRenderer';
...
export const MicSourceRenderer = translate()(
	class extends CustomLayerItemRenderer<
		IProps & InjectedTranslateProps,
		IState
	> {
		itemPosition: number;
		itemWidth: number;
		...
		render() {
			...
			if (startOfScript.length > SCRIPT_PART_LENGTH) {
				startOfScript = startOfScript.substring(
					0,
					startOfScript.substr(0, SCRIPT_PART_LENGTH).lastIndexOf(' ')
				);
			...
			const breakScript = !!(
				content &&
				content.fullScript &&
				content.fullScript.length > BREAK_SCRIPT_BREAKPOINT
			);
						...
						<div
							...
							<div>
								{content && content.fullScript ? (
									breakScript ? (
										<React.Fragment>
											<span className="mini-inspector__full-text text-broken">
												{startOfScript + '\u2026'}
											</span>
											<span className="mini-inspector__full-text text-broken text-end">
												{'\u2026' + endOfScript}
											</span>
										</React.Fragment>
									) : (
										<span className="mini-inspector__full-text">
											{content.fullScript}
										</span>
									)
								) : (
									<span className="mini-inspector__system">
										{t('Script is empty')}
									</span>
								)}
							</div>
