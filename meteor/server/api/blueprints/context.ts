	...
	private _pushNote(type: NoteType, message: string) {
		if (this.handleNotesExternally) {
			...
			if (type === NoteType.WARNING) {
				logger.warn(
					`Warning from "${this._getLoggerName()}": "${message}"\n(${this.getLoggerIdentifier()})`
				);
			} else {
				logger.error(
					`Error from "${this._getLoggerName()}": "${message}"\n(${this.getLoggerIdentifier()})`
				);
			}
		}
	}
...
export class StudioContext extends StudioConfigContext
	implements IStudioContext {
...
export class ShowStyleContext extends StudioContext
	implements IShowStyleContext {
	...
	getShowStyleBase(): ShowStyleBase {
		...
		if (!showStyleBase)
			throw new Meteor.Error(
				404,
				'ShowStyleBase "' + this.showStyleBaseId + '" not found'
			);
	...
	getShowStyleConfig(): { [key: string]: ConfigItemValue } {
		...
		if (!showStyleCompound)
			throw new Meteor.Error(
				404,
				`no showStyleCompound for "${this.showStyleVariantId}"`
			);
...
export class RundownContext extends ShowStyleContext
	implements IRundownContext {
...
export type BlueprintRuntimeArgumentsSet = {
	[key: string]: BlueprintRuntimeArguments | undefined;
};
export class SegmentContext extends RundownContext implements ISegmentContext {
	...
	getRuntimeArguments(
		externalId: string
	): BlueprintRuntimeArguments | undefined {
...
export class PartEventContext extends RundownContext
	implements IPartEventContext {
...
export class AsRunEventContext extends RundownContext
	implements IAsRunEventContext {
	...
	protected getLoggerIdentifier(): string {
		...
		if (this.asRunEvent.timelineObjectId)
			ids.push('timelineObjectId: ' + this.asRunEvent.timelineObjectId);
		return ids.join(',');
	}
}
