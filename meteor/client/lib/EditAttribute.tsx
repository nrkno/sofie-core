...
export class EditAttributeBase extends React.Component<
	IEditAttributeBaseProps,
...
> {
	...
	updateValue(newValue) {
		...
		if (
			...
			typeof this.props.updateFunction === 'function'
		) {
			...
			if (this.props.collection && this.props.attribute) {
				if (newValue === undefined) {
					...
					this.props.collection.update(this.props.obj._id, {
						$unset: m
					});
				} else {
					let m = {};
...
const EditAttributeDropdown = wrapEditAttribute(
	class extends EditAttributeBase {
		...
		handleChange(event) {
			...
			this.handleUpdate(
				this.props.optionsAreNumbers ? parseInt(value, 10) : value
			);
		}
