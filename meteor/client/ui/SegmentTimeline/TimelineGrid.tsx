...
const GRID_FONT_URL =
	'url("/fonts/roboto-gh-pages/fonts/Light/Roboto-Light.woff")';
...
export class TimelineGrid extends React.Component<ITimelineGridProps> {
	...
	contextResize = _.throttle(() => {
		...
		if (this.props.onResize) {
			this.props.onResize([
				getElementWidth(this.parentElement) || 1,
				getElementHeight(this.parentElement) || 1
			]);
		}
	...
	repaint = () => {
		if (this.ctx) {
			...
			this.ctx.font =
				(15 * this.pixelRatio).toString() +
				'px GridTimecodeFont, Roboto, Arial, sans-serif';
			...
			for (i = 0; i < maxTicks; i++) {
				...
				this.ctx.moveTo(
					xPosition,
					isLabel ? 25 * this.pixelRatio : 30 * this.pixelRatio
				);
				this.ctx.lineTo(
					xPosition,
					isLabel ? this.height : 36 * this.pixelRatio
				);
				this.ctx.stroke();
			}
		}
