...
export namespace ServerPeripheralDeviceAPI {
	export function initialize(
		...
		try {
			peripheralDevice = PeripheralDeviceSecurity.getPeripheralDevice(
				id,
				token,
				this
			);
	...
	export function killProcess(id: string, token: string, really: boolean) {
		...
		if (really) {
			this.logger.info(
				'KillProcess command received from ' +
					peripheralDevice._id +
					', shutting down in 1000ms!'
			);
			setTimeout(() => {
				process.exit(0);
			}, 1000);
...
postRoute.route(
	'/devices/:deviceId/uploadCredentials',
	(params, req: IncomingMessage, res: ServerResponse, next) => {
		...
		let fileName: string =
			(_.isArray(fileNames) ? fileNames[0] : fileNames) || '';
...
let methods: Methods = {};
methods[PeripheralDeviceAPI.methods.initialize] = (
	deviceId: string,
	deviceToken: string,
	options: PeripheralDeviceAPI.InitOptions
) => {
...
methods[PeripheralDeviceAPI.methods.unInitialize] = (
	deviceId: string,
	deviceToken: string
) => {
	return ServerPeripheralDeviceAPI.unInitialize(deviceId, deviceToken);
};
methods[PeripheralDeviceAPI.methods.setStatus] = (
	deviceId: string,
	deviceToken: string,
	status: PeripheralDeviceAPI.StatusObject
) => {
...
methods[PeripheralDeviceAPI.methods.ping] = (
	deviceId: string,
	deviceToken: string
) => {
	return ServerPeripheralDeviceAPI.ping(deviceId, deviceToken);
};
methods[PeripheralDeviceAPI.methods.getPeripheralDevice] = (
	deviceId: string,
	deviceToken: string
) => {
	return ServerPeripheralDeviceAPI.getPeripheralDevice(deviceId, deviceToken);
};
methods[PeripheralDeviceAPI.methods.partPlaybackStarted] = (
	deviceId: string,
	deviceToken: string,
	r: PeripheralDeviceAPI.PartPlaybackStartedResult
) => {
	return ServerPeripheralDeviceAPI.partPlaybackStarted(
		deviceId,
		deviceToken,
		r
	);
};
methods[PeripheralDeviceAPI.methods.partPlaybackStopped] = (
	deviceId: string,
	deviceToken: string,
	r: PeripheralDeviceAPI.PartPlaybackStartedResult
) => {
	return ServerPeripheralDeviceAPI.partPlaybackStopped(
		deviceId,
		deviceToken,
		r
	);
};
methods[PeripheralDeviceAPI.methods.piecePlaybackStopped] = (
	deviceId: string,
	deviceToken: string,
	r: PeripheralDeviceAPI.PiecePlaybackStartedResult
) => {
	return ServerPeripheralDeviceAPI.piecePlaybackStopped(
		deviceId,
		deviceToken,
		r
	);
};
methods[PeripheralDeviceAPI.methods.piecePlaybackStarted] = (
	deviceId: string,
	deviceToken: string,
	r: PeripheralDeviceAPI.PiecePlaybackStartedResult
) => {
	return ServerPeripheralDeviceAPI.piecePlaybackStarted(
		deviceId,
		deviceToken,
		r
	);
};
methods[PeripheralDeviceAPI.methods.pingWithCommand] = (
	deviceId: string,
	deviceToken: string,
	message: string,
	cb?: Function
) => {
	return ServerPeripheralDeviceAPI.pingWithCommand(
		deviceId,
		deviceToken,
		message,
		cb
...
methods[PeripheralDeviceAPI.methods.killProcess] = (
	deviceId: string,
	deviceToken: string,
	really: boolean
) => {
...
methods[PeripheralDeviceAPI.methods.testMethod] = (
	deviceId: string,
	deviceToken: string,
	returnValue: string,
	throwError?: boolean
) => {
	return ServerPeripheralDeviceAPI.testMethod(
		deviceId,
		deviceToken,
		returnValue,
		throwError
...
methods[PeripheralDeviceAPI.methods.timelineTriggerTime] = (
	deviceId: string,
	deviceToken: string,
	r: PeripheralDeviceAPI.TimelineTriggerTimeResult
) => {
	return ServerPeripheralDeviceAPI.timelineTriggerTime(
		deviceId,
		deviceToken,
		r
	);
...
methods[PeripheralDeviceAPI.methods.requestUserAuthToken] = (
	deviceId: string,
	deviceToken: string,
	authUrl: string
) => {
	return ServerPeripheralDeviceAPI.requestUserAuthToken(
		deviceId,
		deviceToken,
		authUrl
	);
};
methods[PeripheralDeviceAPI.methods.storeAccessToken] = (
	deviceId: string,
	deviceToken: string,
	authToken: any
) => {
	return ServerPeripheralDeviceAPI.storeAccessToken(
		deviceId,
		deviceToken,
		authToken
	);
...
// --------------------
methods[PeripheralDeviceAPI.methods.functionReply] = (
	deviceId: string,
	deviceToken: string,
	commandId: string,
	err: any,
