...
describe('Test recieved mos ingest payloads', () => {
	...
	testInFiber('mosRoCreate', () => {
		...
		Meteor.call(
			PeripheralDeviceAPI.methods.mosRoCreate,
			device._id,
			device.token,
			roData
		);
		...
		expect(
			fixSnapshot(Rundowns.findOne(rundown._id), true)
		).toMatchSnapshot();
		expect(
			fixSnapshot(Segments.find({ rundownId: rundown._id }).fetch(), true)
		).toMatchSnapshot();
		expect(
			fixSnapshot(Parts.find({ rundownId: rundown._id }).fetch(), true)
		).toMatchSnapshot();
	});
	...
	testInFiber('mosRoCreate: replace existing', () => {
		...
		expect(
			Rundowns.findOne({ externalId: roData.ID.toString() })
		).toBeTruthy();
	...
	testInFiber('mosRoDelete', () => {
		...
		Meteor.call(
			PeripheralDeviceAPI.methods.mosRoDelete,
			device._id,
			device.token,
			roData.ID
		);
	...
	testInFiber('mosRoDelete: Does not exist', () => {
		...
		try {
			...
			expect(e.message).toBe(
				`[404] Rundown ${roData.ID.toString()} not found`
			);
		}
	});
	...
	testInFiber('mosRoStatus: Update ro', () => {
		...
		Meteor.call(
			PeripheralDeviceAPI.methods.mosRoStatus,
			device._id,
			device.token,
			payload
		);
	...
	testInFiber('mosRoReadyToAir: Update ro', () => {
		...
		Meteor.call(
			PeripheralDeviceAPI.methods.mosRoReadyToAir,
			device._id,
			device.token,
			payload
		);
	...
	testInFiber('mosRoStoryStatus: Update part', () => {
		...
		Meteor.call(
			PeripheralDeviceAPI.methods.mosRoStoryStatus,
			device._id,
			device.token,
			payload
		);
		...
		expect(
			fixSnapshot(Pieces.find({ rundownId: rundown._id }).fetch(), true)
		).toMatchSnapshot();
	});
	...
	testInFiber('mosRoStatus: Wrong ro for part', () => {
		...
		try {
			...
			expect(e.message).toBe(
				`[404] Rundown ${rundownExternalId} not found`
			);
		}
	});
	...
	testInFiber('mosRoStatus: Missing part', () => {
		...
		try {
			...
			expect(e.message).toBe(
				`[404] Part ${partExternalId} in rundown ${rundown.externalId} not found`
			);
		}
	});
	...
	testInFiber('mosRoStoryInsert: Into segment', () => {
		...
		Meteor.call(
			PeripheralDeviceAPI.methods.mosRoStoryInsert,
			device._id,
			device.token,
			action,
			[newPartData]
	...
	testInFiber('mosRoStoryInsert: New segment', () => {
		...
		const newPartData = mockRO.newItem(
			'ro1;s1b;newPart1',
			'SEGMENT1B;new1'
		);
	...
	testInFiber('mosRoStoryInsert: Invalid previous id', () => {
		...
		try {
			...
			expect(e.message).toBe(
				`[404] Part ${action.StoryID.toString()} in rundown ${
					rundown.externalId
				} not found`
			);
		}
		...
		expect(
			Parts.findOne({ externalId: newPartData.ID.toString() })
		).toBeFalsy();
	});
	...
	testInFiber('mosRoStoryInsert: Existing externalId', () => {
		...
		try {
			...
			expect(e.message).toBe(
				`[500] Parts ${newPartData.ID.toString()} already exist in rundown ${
					rundown.externalId
				}`
			);
		}
	...
	testInFiber('mosRoStoryReplace: Same segment', () => {
		...
		Meteor.call(
			PeripheralDeviceAPI.methods.mosRoStoryReplace,
			device._id,
			device.token,
			action,
			[newPartData]
	...
	testInFiber('mosRoStoryDelete: Remove segment', () => {
		...
		Meteor.call(
			PeripheralDeviceAPI.methods.mosRoStoryDelete,
			device._id,
			device.token,
			action,
			partExternalIds
		...
		expect(
			Parts.find({ externalId: { $in: partExternalIds } }).count()
		).toEqual(0);
	...
	testInFiber('mosRoStoryDelete: Remove invalid id', () => {
		...
		try {
			...
			expect(e.message).toBe(
				`[404] Parts fakeId in rundown ${rundown.externalId} were not found`
			);
		}
	...
	testInFiber('mosRoFullStory: Valid data', () => {
		...
		Meteor.call(
			PeripheralDeviceAPI.methods.mosRoFullStory,
			device._id,
			device.token,
			story
		);
	...
	testInFiber('mosRoFullStory: Unknown Part', () => {
		...
		try {
			...
			expect(e.message).toBe(
				`[404] Part "${story.ID.toString()}" in rundown "${
					rundown.externalId
				}" is missing cached ingest data`
			);
		}
	});
	...
	testInFiber('mosRoFullStory: Unknown Rundown', () => {
		...
		try {
			...
			expect(e.message).toBe(
				`[404] Rundown ${story.RunningOrderId.toString()} not found`
			);
		}
	});
	...
	testInFiber('mosRoStorySwap: Within same segment', () => {
		...
		Meteor.call(
			PeripheralDeviceAPI.methods.mosRoStorySwap,
			device._id,
			device.token,
			action,
			story0,
	...
	testInFiber('mosRoStorySwap: Story not found', () => {
		...
		try {
			...
			expect(e.message).toBe(
				`[404] Story ${story1} not found in rundown ${action.RunningOrderID.toString()}`
			);
		}
	...
	testInFiber('mosRoStoryMove: Within segment', () => {
		...
		Meteor.call(
			PeripheralDeviceAPI.methods.mosRoStoryMove,
			device._id,
			device.token,
			action,
	...
	testInFiber('mosRoStoryDelete: Remove first story in segment', () => {
		...
		const partToBeRemoved = rundown.getParts({
		...
		const partsInSegmentBefore = rundown.getParts({
			segmentId: partToBeRemoved.segmentId
		});
		expect(partsInSegmentBefore).toHaveLength(3);

		expect(
			partsInSegmentBefore[1]['aCheckToSeeThatThePartHasNotBeenRemoved']
		).toEqual(true);
		expect(
			partsInSegmentBefore[2]['aCheckToSeeThatThePartHasNotBeenRemoved']
		).toEqual(true);
		...
		const partsInSegmentAfter = rundown.getParts({
