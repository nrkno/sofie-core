...
>(() => {
	...
	> {
		...
		studioHasError(studio: Studio) {
			...
			if (
				!peripherals.filter((device) => device.type === PeripheralDeviceAPI.DeviceType.PLAYOUT)
					.length
			)
				return true;
			return false;
							...
							<NavLink
								...
								<div className="selectable clickable">
									...
									{showStyleBase.sourceLayers && showStyleBase.outputLayers && (
										<p>
											{t('Source Layers')}: {showStyleBase.sourceLayers.length.toString()}{' '}
											{t('Output Channels')}: {showStyleBase.outputLayers.length.toString()}
										</p>
									)}
								</div>
							</NavLink>,
						...
						.map((item) => {
							return [
								<NavLink
									...
									<h3>{item.name}</h3>
									<p>
										{item.connected ? t('Connected') : t('Disconnected')}, {t('Status')}:{' '}
