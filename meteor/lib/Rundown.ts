	...
	if (parts.length > 0) {
		...
		const outputLayers = normalizeArray<IOutputLayerExtended>(
			showStyleBase.outputLayers.map((layer) => {
				return extendMandadory<IOutputLayer, IOutputLayerExtended>(_.clone(layer), {
					sourceLayers: [],
					used: false
				});
			}),
		...
		const sourceLayers = normalizeArray<ISourceLayerExtended>(
			showStyleBase.sourceLayers.map((layer) => {
				return extendMandadory<ISourceLayer, ISourceLayerExtended>(_.clone(layer), {
					followingItems: [],
					pieces: []
				});
			}),
		...
		partsE = _.map(parts, (part, itIndex) => {
			...
			_.each<PieceExtended>(partE.pieces, (piece) => {
				...
				let outputLayer = outputLayers[piece.outputLayerId] as
				...
				if (!piece.virtual && outputLayer) {
					...
					if (
						sourceLayers[piece.sourceLayerId] &&
						!sourceLayers[piece.sourceLayerId].isHidden
					) {
			...
			_.each(tlResolved.objects, (obj) => {
				if (obj.resolved.resolved) {
					...
					if (instance) {
						piece.renderedDuration = instance.end
							? instance.end - instance.start
							: null;

						// if there is no renderedInPoint, use 0 as the starting time for the item
						piece.renderedInPoint = instance.start
							? instance.start - TIMELINE_TEMP_OFFSET
							: 0;
			...
			if (
				...
				(displayDurationGroups[partE.displayDurationGroup] !== undefined ||
					...
					(parts[itIndex + 1] &&
						parts[itIndex + 1].displayDurationGroup === partE.displayDurationGroup))
			) {
				displayDurationGroups[partE.displayDurationGroup] =
					(displayDurationGroups[partE.displayDurationGroup] || 0) +
				...
				displayDurationGroups[partE.displayDurationGroup] = Math.max(
					0,
					displayDurationGroups[partE.displayDurationGroup] -
						(partE.duration || partE.renderedDuration)
				);
			}
		...
		const resolveDuration = (item: PieceExtended): number => {
			const expectedDurationNumber =
				typeof item.enable.duration === 'number' ? item.enable.duration || 0 : 0;
				...
				_.each(itemsByLayer, (layerItems, outputSourceCombination) => {
					...
					for (let i = 1; i < sortedItems.length; i++) {
						...
						if (
							...
							currentItem.renderedDuration !== null &&
		...
		if (followingPart && followingPart.pieces) {
			_.each<PieceExtended>(followingPart.pieces, (piece) => {
				...
				let sourceLayer =
					...
					outputLayer.sourceLayers.find((el) => {
						return el._id === piece.sourceLayerId;
					});
		...
		if (isNextSegment && !isLiveSegment && !autoNextPart && rundown.currentPartId) {
			...
			if (
				currentOtherPart &&
				currentOtherPart.expectedDuration &&
				currentOtherPart.autoNext
			) {
				autoNextPart = true;
			}
		}
...
export function calculatePieceTimelineEnable(
	piece: Piece,
	offset?: number
): SuperTimeline.TimelineEnable {
