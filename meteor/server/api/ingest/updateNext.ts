...
function getRundownValidParts(rundown: Rundown) {
	return rundown.getParts({
		$or: [{ invalid: false }, { invalid: { $exists: false } }]
	});
}
...
export namespace UpdateNext {
	export function ensureNextPartIsValid(rundown: Rundown) {
		...
		if (rundown && rundown.active && rundown.nextPartId) {
			...
			const currentPart = allValidParts.find(
				(part) => part._id === rundown.currentPartId
			);
			const currentNextPart = allValidParts.find(
				(part) => part._id === rundown.nextPartId
			);
			...
			if (!currentPart && rundown.currentPartId !== null) {
				...
				const expectedAutoNextPart = fetchAfter(
					allValidParts,
					{},
					currentPart ? currentPart._rank : null
				);
				const expectedAutoNextPartId = expectedAutoNextPart
					? expectedAutoNextPart._id
					: null;

				// If not manually set, make sure that next is done by rank
				if (
					!rundown.nextPartManual &&
					expectedAutoNextPartId !== rundown.nextPartId
				) {
					ServerPlayoutAPI.setNextPartInner(
						rundown,
						expectedAutoNextPart || null
					);
