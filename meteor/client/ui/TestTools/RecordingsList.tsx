...
const RecordingsList = translateWithTracker<
	IRecordingListProps,
	IRecordingListState,
	IRecordingListTrackedProps
>((props: IRecordingListProps) => {
	...
	> {
		constructor(
			props: Translated<IRecordingListProps & IRecordingListTrackedProps>
		) {
			super(props);
		...
		stopRecording(e) {
			if (this.props.match && this.props.match.params) {
				const { t } = this.props;
				doUserAction(t, e, UserActionAPI.methods.recordStop, [
					this.props.match.params.studioId
				]);
			}
		}
		startRecording(e) {
			if (this.props.match && this.props.match.params) {
				...
				doUserAction(
					t,
					e,
					UserActionAPI.methods.recordStart,
					[this.props.match.params.studioId, this.state.filename],
					() => {
						this.setState({
							filename: ''
						});
		...
		handleConfirmDeleteAccept = (e) => {
			if (this.state.deleteConfirmItem) {
				const { t } = this.props;
				doUserAction(t, e, UserActionAPI.methods.recordDelete, [
					this.state.deleteConfirmItem._id
				]);
			}
			this.setState({
				showDeleteConfirm: false
		...
		renderControlPanel() {
			...
			return (
				<React.Fragment>
					...
					<p>
						{active ? (
							<button onClick={(e) => this.stopRecording(e)}>
								{t('Stop')}
							</button>
						) : (
...
export class RecordedFilesListItem extends React.Component<
	IRecordedFilesListItemProps
> {
	render() {
		return (
			<tr className="recorded-file-list-item">
				...
				<td className="recorded-file-list-item__started">
					<Moment format="YYYY/MM/DD HH:mm:ss">
						{this.props.file.startedAt}
					</Moment>
				</td>
				<td className="recorded-file-list-item__stopped">
					{this.props.file.stoppedAt && (
						<Moment format="YYYY/MM/DD HH:mm:ss">
							{this.props.file.stoppedAt}
						</Moment>
					)}
