...
export const MigrationView = translateWithTracker<
	IProps,
	IState,
	ITrackedProps
>((props: IProps) => {
	...
	class MigrationView extends MeteorReactComponent<
	...
	> {
		...
		setErrorMessage(err) {
			this.setState({
				errorMessage: _.isString(err)
					? err
					: err.reason || err.toString() || err + ''
			});
		}
		updateVersions() {
			this.setState({
			...
			Meteor.call(
				...
				(err, r: GetMigrationStatusResult) => {
					...
					if (err) {
						...
						_.each(
							r.migration.manualInputs,
							(manualInput: MigrationStepInput) => {
								if (
									manualInput.stepId &&
		...
		forceMigration() {
			...
			if (this.state.migration) {
				Meteor.call(
					MigrationMethods.forceMigration,
					this.state.migration.chunks,
					(err) => {
						if (this.cancelRequests) return;
						if (err) {
		...
		setDatabaseVersion(version: string) {
			...
			doModalDialog({
				...
				message:
					t('Are you sure you want to set the database version to') +
					` ${version}?`,
				onAccept: () => {
					Meteor.call(
						MigrationMethods.forceMigration,
		...
		renderManualSteps() {
			if (this.state.migration) {
				let rank = 0;
				return _.map(
					this.state.migration.manualInputs,
					(manualInput: MigrationStepInput) => {
						if (manualInput.stepId) {
							let stepId = manualInput.stepId;
							let value;
							if (manualInput.attribute) {
								value = (this.state.inputValues[stepId] || {})[
									manualInput.attribute
								];
							}
							return (
							...
							<div>
								...
								{this.state.migration
									...
									<div>
										<p className="mhn mvs">
											{t(
												'The migration procedure needs some help from you in order to complete, see below:'
											)}
										</p>
