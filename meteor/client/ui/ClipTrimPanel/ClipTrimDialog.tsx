...
export const ClipTrimDialog = translate()(
	class ClipTrimDialog extends React.Component<
	...
	> {
		constructor(props: IProps & InjectedTranslateProps) {
			...
			this.state = {
				inPoint: ((this.props.selectedPiece.content as VTContent)
					.editable as VTEditableParameters).editorialStart,
				duration: ((this.props.selectedPiece.content as VTContent)
					.editable as VTEditableParameters).editorialDuration
			};
		}
		...
		render() {
			...
			return (
				<ModalDialog
					title={t('Trim "{{name}}"', {
						name: this.props.selectedPiece.name
					})}
					show={true}
					acceptText={t('OK')}
