...
export function setNext(
	...
	if (nextPartId) {
		...
		if (nextPart.invalid)
			return ClientAPI.responseError('Part is marked as invalid, cannot set as next.');
		if (nextPart.floated)
			return ClientAPI.responseError('Part is marked as floated, cannot set as next.');
	}
...
export function moveNext(
	...
	if (!rundown) throw new Meteor.Error(404, `Rundown "${rundownId}" not found!`);
	if (!rundown.active)
		return ClientAPI.responseError('Rundown is not active, please activate it first');
	...
	return ClientAPI.responseSuccess(
		ServerPlayoutAPI.moveNextPart(rundownId, horisontalDelta, verticalDelta, setManually)
	);
}
export function prepareForBroadcast(rundownId: string): ClientAPI.ClientResponse {
	...
	return ClientAPI.responseSuccess(ServerPlayoutAPI.prepareRundownForBroadcast(rundownId));
}
...
export function activate(rundownId: string, rehearsal: boolean): ClientAPI.ClientResponse {
	...
	return ClientAPI.responseSuccess(ServerPlayoutAPI.activateRundown(rundownId, rehearsal));
}
export function deactivate(rundownId: string): ClientAPI.ClientResponse {
	let rundown = Rundowns.findOne(rundownId);
	if (!rundown) throw new Meteor.Error(404, `Rundown "${rundownId}" not found!`);
	return ClientAPI.responseSuccess(ServerPlayoutAPI.deactivateRundown(rundownId));
}
...
export function disableNextPiece(rundownId: string, undo?: boolean) {
	return ClientAPI.responseSuccess(ServerPlayoutAPI.disableNextPiece(rundownId, undo));
}
...
export function pieceTakeNow(rundownId: string, partId: string, pieceId: string) {
	...
	if (sourceL && sourceL.type !== SourceLayerType.GRAPHICS)
		return ClientAPI.responseError(`Part "${pieceId}" is not a GRAPHICS piece!`);

	return ClientAPI.responseSuccess(ServerPlayoutAPI.pieceTakeNow(rundownId, partId, pieceId));
}
...
export function segmentAdLibPieceStop(rundownId: string, partId: string, pieceId: string) {
	...
	return ClientAPI.responseSuccess(ServerPlayoutAPI.stopAdLibPiece(rundownId, partId, pieceId));
}
...
export function activateHold(rundownId: string, undo?: boolean) {
	...
	if (undo) {
		return ClientAPI.responseSuccess(ServerPlayoutAPI.deactivateHold(rundownId));
	} else {
