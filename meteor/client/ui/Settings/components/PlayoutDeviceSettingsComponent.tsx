	...
	> {
		...
		handleConfirmRemoveAccept = (e) => {
			this.state.deleteConfirmDeviceId &&
				this.removeDevice(this.state.deleteConfirmDeviceId);
			this.setState({
				showDeleteConfirm: false,
				deleteConfirmDeviceId: undefined
			});
		...
		addNewDevice = () => {
			let settings =
				(this.props.device.settings as PlayoutDeviceSettings) || {};
			// find free key name
		...
		updateDeviceId = (edit: EditAttributeBase, newValue: string) => {
			...
			if (settings[newDeviceId]) {
				throw new Meteor.Error(
					400,
					'Device "' + newDeviceId + '" already exists'
				);
			}
			let mSet = {};
			let mUnset = {};
			...
			if (edit.props.collection) {
				...
				throw new Meteor.Error(
					500,
					'EditAttribute.props.collection is not set (it should be)!'
				);
			}
		...
		renderCasparCGDeviceSettings(
			_subDevice: TSR.DeviceOptionsAny,
			deviceId: string
		) {
			const { t } = this.props;
			return (
		...
		renderHTTPSendDeviceSettings(
			subDevice: TSR.DeviceOptionsAny,
			deviceId: string
		) {
			return (
				<HttpSendDeviceSettingsComponent
					parentDevice={this.props.device}
					device={subDevice}
					deviceId={deviceId}
				/>
			);
		}
		renderPanasonicPTZDeviceSettings(
			_subDevice: TSR.DeviceOptionsAny,
			deviceId: string
		) {
			const { t } = this.props;
		...
		renderHTTPWatcherDeviceSettings(
			_subDevice: TSR.DeviceOptionsAny,
			deviceId: string
		) {
			const { t } = this.props;
