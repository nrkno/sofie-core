...
export default translateWithTracker<IProps, IState, ITrackedProps>((props: IProps) => {
	...
	> {
		...
		onToggleDefault = (item: RundownLayout, index: number, value: boolean) => {
			const obj = _.object(
				item.filters.map((item, i) => [`filters.${i}.default`, i === index ? value : false])
			);
			RundownLayouts.update(item._id, {
				$set: obj
			});
		...
		onDeleteLayout = (e: any, item: RundownLayoutBase) => {
			...
			doModalDialog({
				...
				yes: t('Delete'),
				no: t('Cancel'),
				message: t('Are you sure you want to delete the shelf layout "{{name}}"?', {
					name: item.name
				}),
				onAccept: () => {
					callMethod(e, RundownLayoutsAPI.methods.removeRundownLayout, item._id);
				}
			});
		};
