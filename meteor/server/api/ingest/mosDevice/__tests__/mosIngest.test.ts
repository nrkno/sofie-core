...
describe('Test recieved mos ingest payloads', () => {
	...
	testInFiber('mosRoStoryInsert: Invalid previous id', () => {
		...
		try {
			...
			expect(e.message).toBe(
				`[404] Part ${action.StoryID.toString()} in rundown ${rundown.externalId} not found`
			);
		}
	...
	testInFiber('mosRoStoryMove: Within segment', () => {
		...
		Meteor.call(PeripheralDeviceAPI.methods.mosRoStoryMove, device._id, device.token, action, [
