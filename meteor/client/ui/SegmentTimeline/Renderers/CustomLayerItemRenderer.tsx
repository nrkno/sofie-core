...
> extends React.Component<
	ICustomLayerItemProps & IProps,
	ISourceLayerItemState & IState
> {
	getItemLabelOffsetLeft(): { [key: string]: string } {
		if (
			this.props.getItemLabelOffsetLeft &&
			typeof this.props.getItemLabelOffsetLeft === 'function'
		) {
	...
	getItemLabelOffsetRight(): { [key: string]: string } {
		if (
			this.props.getItemLabelOffsetRight &&
			typeof this.props.getItemLabelOffsetRight === 'function'
		) {
	...
	setAnchoredElsWidths(
		...
		if (
			this.props.setAnchoredElsWidths &&
			typeof this.props.setAnchoredElsWidths === 'function'
		) {
			return this.props.setAnchoredElsWidths(
				leftAnchoredWidth,
				rightAnchoredWidth
			);
		}
	...
	renderOverflowTimeLabel() {
		...
		if (
			...
			(this.props.piece.renderedInPoint || 0) + vtContent.sourceDuration >
				(this.props.partDuration || 0)
		) {
			let time =
				(this.props.piece.renderedInPoint || 0) +
				vtContent.sourceDuration -
				((this.props.partDuration || 0) as number);
	...
	renderInfiniteIcon() {
		return this.props.piece.infiniteMode &&
			this.props.piece.infiniteMode === PieceLifespan.Infinite &&
			!this.props.piece.playoutDuration &&
	...
	renderContentTrimmed() {
		...
		return vtContent &&
			vtContent.editable &&
			duration !== vtContent.editable.editorialDuration &&
			vtContent.editable.editorialDuration !== 0 ? (
