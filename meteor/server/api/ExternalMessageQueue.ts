...
function doMessageQueue() {
	...
	try {
		...
		messagesToSend = _.filter(
			messagesToSend,
			(msg: ExternalMessageQueueObj): boolean => {
				return (
					msg.retryUntil === undefined ||
					msg.manualRetry ||
					now < msg.retryUntil
				);
			}
		...
		_.each(messagesToSend, (msg) => {
			try {
				logger.debug(
					`Trying to send externalMessage, id: ${msg._id}, type: "${msg.type}"`
				);
				...
				if (msg.type === IBlueprintExternalMessageQueueType.SOAP) {
					...
					throw new Meteor.Error(
						500,
						`Unknown / Unsupported externalMessage type: "${msg.type}"`
					);
				}
				ps.push(
					Promise.resolve(p)
						.then((result) => {
							...
							logger.debug(
								`ExternalMessage sucessfully sent, id: ${msg._id}, type: "${msg.type}"`
							);
						})
						.catch((e) => {
...
methods[ExternalMessageQueueAPI.methods.toggleHold] = (id: string) => {
	...
	if (!m)
		throw new Meteor.Error(
			404,
			`ExternalMessageQueue "${id}" not found on toggleHold`
		);
	ExternalMessageQueue.update(id, {
		$set: {
			hold: !m.hold
		}
	});
...
methods[ExternalMessageQueueAPI.methods.retry] = (id: string) => {
	...
	if (!m)
		throw new Meteor.Error(
			404,
			`ExternalMessageQueue "${id}" not found on retry`
		);
...
methods[ExternalMessageQueueAPI.methods.setRunMessageQueue] = (
	value: boolean
) => {
	check(value, Boolean);
