...
export namespace MOSDeviceActions {
	export const reloadRundown: (
		...
		rundown: Rundown
	) => UserActionAPI.ReloadRundownResponse = Meteor.wrapAsync(function reloadRundown(
		peripheralDevice: PeripheralDevice,
		rundown: Rundown,
		cb: WrapAsyncCallback<UserActionAPI.ReloadRundownResponse>
	): void {
	...
	export function setPieceInOutPoint(
		...
		return new Promise((resolve, reject) => {
			if (!partCache.payload)
				throw new Meteor.Error(
					500,
					`Part Cache for "${partCache.externalId}" missing payload!`
				);
			const mosPayload = partCache.payload as MOS.IMOSROFullStory;
			if (!mosPayload.Body)
				throw new Meteor.Error(
					500,
					`Part Cache for "${partCache.externalId}" missing FullStory content!`
				);
