...
export class DbCacheCollection<Class extends DBInterface, DBInterface extends { _id: ProtectedString<any> }> {
	...
	findFetch (selector?: MongoSelector<DBInterface> | DBInterface['_id'] | SelectorFunction<DBInterface>, options?: FindOptions<DBInterface>): Class[] {
		this._initialize()
		...
		_.each(this.documents, (doc, _id) => {
			...
			if (
				...
				if (unprotectString(doc.document['_id']) !== _id) {
					throw new Meteor.Error(500, `Error: document._id "${doc.document['_id']}" is not equal to the key "${_id}"`)
				}
	...
	findOne (selector?: MongoSelector<DBInterface> | DBInterface['_id'] | SelectorFunction<DBInterface>, options?: FindOptions<DBInterface>): Class | undefined {
		this._initialize()
	...
	update (selector: MongoSelector<DBInterface> | DBInterface['_id'] | SelectorFunction<DBInterface>, modifier: ((doc: DBInterface) => DBInterface) | MongoModifier<DBInterface>): number {
		...
		_.each(this.findFetch(selector), (doc) => {
			...
			if (unprotectString(newDoc._id) !== _id) {
				throw new Meteor.Error(500, `Error: The (immutable) field '_id' was found to have been altered to _id: "${newDoc._id}"`)
			}
	...
	upsert (selector: MongoSelector<DBInterface> | DBInterface['_id'], doc: DBInterface): {
		...
		if (updatedCount > 0) {
			...
			if (doc._id !== selector['_id']) {
				throw new Meteor.Error(500, `Can't upsert, selector._id "${selector['_id']}" not matching doc._id "${doc._id}"`)
			}
	...
	private _innerfillWithDataFromArray (documents: Class[]) {
		_.each(documents, doc => {
			...
			if (this.documents[id]) {
				throw new Meteor.Error(500, `Unable to fill cache with data "${this._collection['name']}", _id "${doc._id}" already exists`)
			}
...
export function saveIntoCache<DocClass extends DBInterface, DBInterface extends DBObj> (
	...
	const preparedChanges = prepareSaveIntoCache(
		collection,
		filter,
		newData,
		options
	...
	return savePreparedChangesIntoCache(
		preparedChanges,
		collection,
		options
	)
...
export function savePreparedChangesIntoCache<DocClass extends DBInterface, DBInterface extends DBObj> (
	...
	const checkInsertId = (id) => {
		if (newObjIds[id]) {
			throw new Meteor.Error(500, `savePreparedChangesIntoCache into collection "${(collection as any)._name}": Duplicate identifier "${id}"`)
		}
		newObjIds[id] = true
	}
