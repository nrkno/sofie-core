...
describe('User Actions', () => {
	...
	testInFiber('Basic rundown control', () => {
		...
		expect(Meteor.call(UserActionAPI.methods.prepareForBroadcast, rundownId0)).toMatchObject({
			success: 200
		});
		expect(getRundown0()).toMatchObject({
			active: true,
		...
		expect(Meteor.call(UserActionAPI.methods.activate, rundownId1, false)).toMatchObject({
			error: 409,
			message: expect.stringMatching(/only one rundown/i)
		});

		// Take the first Part:
		expect(Meteor.call(UserActionAPI.methods.take, rundownId0)).toMatchObject({ success: 200 });
		expect(getRundown0()).toMatchObject({
		...
		// Reset rundown:
		expect(Meteor.call(UserActionAPI.methods.resetRundown, rundownId0)).toMatchObject({
			success: 200
		});
		expect(getRundown0()).toMatchObject({
			currentPartId: null,
		...
		expect(Meteor.call(UserActionAPI.methods.moveNext, rundownId0, -1, 0)).toMatchObject({
			success: 200
		});
		expect(getRundown0()).toMatchObject({
			currentPartId: parts[parts.length - 1]._id,
		...
		// Deactivate rundown:
		expect(Meteor.call(UserActionAPI.methods.deactivate, rundownId0)).toMatchObject({
			success: 200
		});
		expect(getRundown0()).toMatchObject({
			active: false,
	...
	testInFiber('Restart Core', () => {
		...
		expect(
			Meteor.call(
				...
				getHash(OriginalUserActionAPI.RESTART_SALT + res.result)
			)
		).toMatchObject({ success: 200 });
