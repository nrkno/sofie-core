...
export default translateWithTracker<IProps, IState, ITrackedProps>(
	...
	class SnapshotsView extends MeteorReactComponent<
		...
		restoreStoredSnapshot = (snapshotId) => {
			...
			if (snapshot) {
				doModalDialog({
					...
					onAccept: () => {
						Meteor.call(
							SnapshotFunctionsAPI.RESTORE_SNAPSHOT,
							snapshotId,
							(err) => {
								if (err) {
									// todo: notify user
		...
		removeStoredSnapshot = (snapshotId: string) => {
			...
			if (snapshot) {
				doModalDialog({
					...
					onAccept: () => {
						Meteor.call(
							SnapshotFunctionsAPI.REMOVE_SNAPSHOT,
							snapshotId,
							(err) => {
								if (err) {
									// todo: notify user
								...
								{this.props.studios.length > 1 ? (
									<div>
										...
										<p className="mhn">
											<i>
												{t(
													'A Studio Snapshot contains all system settings related to that studio'
												)}
											</i>
										</p>
									...
									{_.map(this.props.snapshots, (snapshot) => {
										return (
											<tr key={snapshot._id}>
												...
												<td>
													<a
														href={`/snapshot/retrieve/${snapshot._id}`}
														target="_blank">
														{snapshot.name}
													</a>
												</td>
