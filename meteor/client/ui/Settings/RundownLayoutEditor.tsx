...
export default translateWithTracker<IProps, IState, ITrackedProps>(
	...
	class RundownLayoutEditor extends MeteorReactComponent<
	...
	> {
		...
		onDeleteLayout = (e: any, item: RundownLayoutBase) => {
			...
			doModalDialog({
				...
				no: t('Cancel'),
				message: t(
					'Are you sure you want to delete the shelf layout "{{name}}"?',
					{ name: item.name }
				),
				onAccept: () => {
					callMethod(
			...
			return (
				<React.Fragment>
					...
					{item.filters.map((tab, index) => (
						<div
							className="rundown-layout-editor-filter mod pan mas"
							key={tab._id}>
							<button
								className="action-btn right mod man pas"
								onClick={(e) => this.onRemoveFilter(item, tab)}>
								<FontAwesomeIcon icon={faTrash} />
							</button>
							{isRundownLayout && (
								<button
									className={ClassNames(
										'action-btn right mod man pas',
										{
											star: (tab as RundownLayoutFilter)
												.default
										}
									)}
									onClick={(e) =>
										this.onToggleDefault(
											item as RundownLayout,
											index,
											!(tab as RundownLayoutFilter)
												.default
										)
									}>
							...
							<div className="mod mvs mhs">
								<label className="field">
									{t(
										'Only Display AdLibs from Current Segment'
									)}
									<EditAttribute
										modifiedClassName="bghl"
							...
							{isDashboardLayout && (
								<React.Fragment>
									<div className="mod mvs mhs">
										<label className="field">
											{t(
												'Include Clear Source Layer in Ad-Libs'
											)}
											<EditAttribute
												modifiedClassName="bghl"
							...
							<div className="mod mvs mhs">
								...
								<EditAttribute
									...
									attribute={`filters.${index}.sourceLayerIds`}
									obj={item}
									options={this.props.showStyleBase.sourceLayers.map(
										(l) => {
											return {
												name: l.name,
												value: l._id
							...
							<div className="mod mvs mhs">
								...
								<EditAttribute
									...
									attribute={`filters.${index}.outputLayerIds`}
									obj={item}
									options={this.props.showStyleBase.outputLayers.map(
										(l) => {
											return {
												name: l.name,
												value: l._id
							...
							{isDashboardLayout && (
								<React.Fragment>
									<div className="mod mvs mhs">
										<label className="field">
											{t(
												'Register Shortcuts for this Panel'
											)}
											<EditAttribute
												modifiedClassName="bghl"
					...
					<tr
						...
						<td className="settings-studio-rundown-layouts-table__actions table-item-actions c3">
							<button
								...
								onClick={(e) => this.downloadItem(item)}>
							...
							<button
								className="action-btn"
								onClick={(e) => this.onDeleteLayout(e, item)}>
								<FontAwesomeIcon icon={faTrash} />
							</button>
						</td>
					</tr>
							...
							<td colSpan={4}>
								...
								<div>
									...
									RundownLayoutType.RUNDOWN_LAYOUT
										...
										: item.type ===
										  RundownLayoutType.DASHBOARD_LAYOUT
										? this.renderFilters(item)
										: null}
								</div>
								<div className="mod mls">
									...
									<button
										...
										onClick={(e) => this.onAddFilter(item)}>
				...
				doModalDialog({
					...
					onAccept: () => {
						if (uploadFileContents) {
							fetchFrom(
								`/shelfLayouts/upload/${this.props.showStyleBase._id}`,
								{
									method: 'POST',
								...
								.then((res) => {
									...
									NotificationCenter.push(
										new Notification(
											...
											t(
												'Shelf layout uploaded successfully.'
											),
											'RundownLayouts'
										)
									);
								...
								.catch((err) => {
									...
									NotificationCenter.push(
										new Notification(
											...
											NoticeLevel.WARNING,
											t(
												'Failed to upload shelf layout: {{errorMessage}}',
												{ errorMessage: err + '' }
											),
											'RundownLayouts'
										)
