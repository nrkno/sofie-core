...
import {
	MediaObject,
	MediaObjects
} from '../../../lib/collections/MediaObjects';
import {
	PeripheralDevice,
	PeripheralDevices
...
export namespace reactiveData {
	export function getRRundownId(
		rundownId: string
	): ReactiveVar<string | undefined> {
	...
	export function getRRundownStudioId(
		rundownId: string
	): ReactiveVar<string | undefined> {
		const rVar = new ReactiveVar<string | undefined>(undefined);
		Tracker.autorun(() => {
	...
	export function getRRundownShowStyleBaseId(
		rundownId: string
	): ReactiveVar<string | undefined> {
		const rVar = new ReactiveVar<string | undefined>(undefined);
		Tracker.autorun(() => {
	...
	export function getRStudio(
		studioId: string
	): ReactiveVar<Studio | undefined> {
		const rVar = new ReactiveVar<Studio | undefined>(
			undefined,
			ReactiveDataHelper.simpleObjCompare
		);
		Tracker.autorun(() => {
	...
	export function getRSourceLayer(
		showStyleBase: ShowStyleBase,
		sourceLayerId: string
	): ReactiveVar<ISourceLayer | undefined> {
		const rVar = new ReactiveVar<ISourceLayer | undefined>(
			undefined,
			ReactiveDataHelper.simpleObjCompare
		);
		Tracker.autorun(() => {
			const sourceLayer = showStyleBase.sourceLayers.find(
				(layer) => layer._id === sourceLayerId
			);
	...
	export function getRMediaObject(
		mediaId: string
	): ReactiveVar<MediaObject | undefined> {
		const rVar = new ReactiveVar<MediaObject | undefined>(
			undefined,
			ReactiveDataHelper.simpleObjCompare
		);
		Tracker.autorun(() => {
