...
> {
	...
	constructor(props) {
		...
		this.configOptions = {
			...
			followTake:
				queryParams['followtake'] === undefined ? true : queryParams['followtake'] === '1',
			...
			showMarker:
				queryParams['showmarker'] === undefined ? true : queryParams['showmarker'] === '1',
	...
	scrollToLive() {
		...
		const current =
			document.querySelector('.prompter .live') || document.querySelector('.prompter .next');
		...
		UNSAFE_componentWillUpdate() {
			...
			Array.from(document.querySelectorAll('.prompter .scroll-anchor')).forEach((anchor) => {
				...
				if (top + readPosition <= 10)
					foundPositions.push([top, '.' + anchor.className.replace(/\s/g, '.')]);
			});
				...
				return (
					<div
						...
						<div
							...
							<div className="prompter-break begin">{this.props.rundown.name}</div>
