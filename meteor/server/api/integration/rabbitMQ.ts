...
class ConnectionManager extends Manager<AMQP.Connection> {
	...
	async initConnection(): Promise<AMQP.Connection> {
		try {
			...
			logger.error(
				'Error when connecting AMQP (to ' + this.hostURL + ') ' + err
			);
			throw err;
		}
	}
...
class ChannelManager extends Manager<AMQP.ConfirmChannel> {
	...
	sendMessage(
		...
		return new Promise((resolve, reject) => {
			this.channel.assertExchange(exchangeTopic, 'topic', {
				durable: true
			});
...
export async function sendRabbitMQMessage(
	...
	if (!exchangeTopic)
		throw new Meteor.Error(400, `RabbitMQ: Message topic not set`);
	if (!routingKey)
		throw new Meteor.Error(400, `RabbitMQ: Message routing key not set`);
	if (!message)
		throw new Meteor.Error(400, `RabbitMQ: Message message not set`);
