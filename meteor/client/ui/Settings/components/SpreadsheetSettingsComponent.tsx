...
export const SpreadsheetSettingsComponent = translate()(
	class SpreadsheetSettingsComponent extends React.Component<
		Translated<ISpreadsheetSettingsComponentProps>,
		ISpreadsheetSettingsComponentState
	> {
					...
					.catch((err) => {
						NotificationCenter.push(
							new Notification(
								undefined,
								NoticeLevel.WARNING,
								t('Failed to upload spreadsheet credentials: {{errorMessage}}', {
									errorMessage: err + ''
								}),
								'SpreadsheetSettingsComponent'
							)
						...
						if (e) {
							...
							NotificationCenter.push(
								new Notification(
									undefined,
									NoticeLevel.WARNING,
									'Error when authorizing access token: ' + e,
									''
								)
							);
						} else {
							NotificationCenter.push(
								new Notification(undefined, NoticeLevel.NOTIFICATION, 'Access token saved!', '')
							);
						}
					},
					'receiveAuthToken',
						...
						{settings.secretCredentials && !settings.secretAccessToken ? (
							<label className="field">
								...
								<div className="mdi">
									<div>
										{t(
											'Click on the link below and accept the permissions request. Paste the received URL below.'
										)}
									</div>
									<div>
						...
						<label className="field">
							...
							<div className="mdi">
								<Tooltip
									...
									visible={
										getHelpMode() &&
										(!this.props.device.settings || !this.props.device.settings['folderPath'])
									}
									placement="top">
									<EditAttribute
										modifiedClassName="bghl"
