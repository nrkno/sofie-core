...
export function loadCachedRundownData(
	...
	const baseEntry = cacheEntries.find(
		(e) => e.type === IngestCacheType.RUNDOWN
	);
	if (!baseEntry)
		throw new Meteor.Error(
			404,
			`Rundown "${rundownId}", (${rundownExternalId}) has no cached ingest data`
		);
...
export function loadCachedIngestSegment(
	...
	if (segmentEntry.type !== IngestCacheType.SEGMENT)
...
export function loadIngestDataCachePart(
	...
	if (partEntry.type !== IngestCacheType.PART)
		throw new Meteor.Error(500, 'Wrong type on cached part');
	return partEntry;
}
...
export function saveRundownCache(
	...
	// cache the Data:
	const cacheEntries: IngestDataCacheObj[] = generateCacheForRundown(
		rundownId,
		ingestRundown
	);
...
export function saveSegmentCache(
	...
	// cache the Data:
	const cacheEntries: IngestDataCacheObj[] = generateCacheForSegment(
		rundownId,
		ingestSegment
	);
	saveIntoDb<IngestDataCacheObj, IngestDataCacheObj>(
...
function generateCacheForSegment(
	...
	const cacheEntries: Array<
		IngestDataCacheObjSegment | IngestDataCacheObjPart
	> = [];
