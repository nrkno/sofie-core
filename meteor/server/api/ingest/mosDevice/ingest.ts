...
function groupIngestParts(
	parts: AnnotatedIngestPart[]
): { name: string; parts: IngestPart[] }[] {
	...
	return rundownSyncFunction(
		...
		() => {
			const parts = _.compact(
				storiesToIngestParts(
					rundownId,
					mosRunningOrder.Stories || [],
					!createFresh
				)
			);
	...
	return rundownSyncFunction(
		...
		() => {
			const rundown = getRundown(
				rundownId,
				parseMosString(mosRunningOrderBase.ID)
			);
			...
			ingestRundown.payload = _.extend(
				ingestRundown.payload,
				mosRunningOrderBase
			);
	...
	return rundownSyncFunction(
		...
		() => {
			...
			if (!cachedPartData.segmentId) {
				throw new Meteor.Error(
					500,
					`SegmentId missing for part "${partId}" (MOSFullStory)`
				);
			}
	...
	return rundownSyncFunction(
		...
		() => {
			...
			const missingIds = _.filter(
				storyIds,
				(id) => ingestPartIds.indexOf(id) === -1
			);
			if (missingIds.length > 0) {
				throw new Meteor.Error(
					404,
					`Parts ${missingIds.join(', ')} in rundown ${
						rundown.externalId
					} were not found`
				);
			}

			const filteredParts = ingestParts.filter(
...
function getAnnotatedIngestParts(
	...
	return rundownSyncFunction(
		...
		() => {
			...
			const insertIndex = !insertBeforePartExternalId // insert last
				? ingestParts.length
				: ingestParts.findIndex(
						(p) => p.externalId === insertBeforePartExternalId
				  );
			if (insertIndex === -1) {
				throw new Meteor.Error(
					404,
			...
			const newParts = _.compact(
				storiesToIngestParts(rundown._id, newStories || [], true)
			);
			...
			const collidingPartIds = _.map(
				_.filter(
					ingestParts,
					(part) => newPartIds.indexOf(part.externalId) !== -1
				),
...
export function handleSwapStories(
	...
	if (story0Str === story1Str) {
		throw new Meteor.Error(
			400,
			`Cannot swap part ${story0Str} with itself in rundown ${parseMosString(
				runningOrderMosId
			)}`
		);
	}
	...
	return rundownSyncFunction(
		...
		() => {
			...
			const story0Index = ingestParts.findIndex(
				(p) => p.externalId === story0Str
			);
			if (story0Index === -1) {
				throw new Meteor.Error(
					404,
					`Story ${story0} not found in rundown ${parseMosString(
						runningOrderMosId
					)}`
				);
			}
			const story1Index = ingestParts.findIndex(
				(p) => p.externalId === story1Str
			);
			if (story1Index === -1) {
				throw new Meteor.Error(
					404,
					`Story ${story1} not found in rundown ${parseMosString(
						runningOrderMosId
					)}`
				);
			}
...
export function handleMoveStories(
	...
	return rundownSyncFunction(
		...
		() => {
			...
			const missingIds = _.filter(
				storyIds,
				(id) => movingIds.indexOf(id) === -1
			);
			if (missingIds.length > 0) {
				throw new Meteor.Error(
					404,
					`Parts ${missingIds.join(', ')} were not found in rundown ${
						rundown.externalId
					}`
				);
			}
...
function diffAndApplyChanges(
	...
	if (rundown.active && rundown.currentPartId) {
		...
		if (!currentPart) {
			...
			logger.warn(
				`Currently playing part "${rundown.currentPartId}" was removed during ingestData. Unsyncing the rundown!`
			);
	...
	_.each(
		segmentDiff.onlyExternalIdChanged,
		(newSegmentExternalId, oldSegmentExternalId) => {
	...
	// Remove old segments
	const removedSegmentIds = _.map(
		segmentDiff.removed,
		(_segmentEntry, segmentExternalId) =>
...
export function compileSegmentEntries(
...
export function diffSegmentEntries(
	...
	_.each(newSegmentEntries, (newSegmentEntry, segmentExternalId) => {
		const oldSegmentEntry = oldSegmentEntries[segmentExternalId] as
			| IngestSegment
			| undefined;
		if (oldSegmentEntry) {
			// Deep compare
	...
	_.each(oldSegmentEntries, (oldSegmentEntry, segmentExternalId) => {
		const newSegmentEntry = newSegmentEntries[segmentExternalId] as
			| IngestSegment
			| undefined;
		if (!newSegmentEntry) {
	...
	_.each(diff.removed, (segmentEntry, segmentExternalId) => {
		...
		let newSegmentEntry = _.find(
			diff.added,
			(se) => se.name === segmentEntry.name
		);
		if (!newSegmentEntry) {
		...
		if (newSegmentEntry) {
			diff.onlyExternalIdChanged[segmentExternalId] =
				newSegmentEntry.externalId;
		}
	});
