...
const RecordingView = translateWithTracker<
	IRecordingViewProps,
	IRecordingViewState,
	IRecordingViewTrackedProps
>((props: IRecordingViewProps) => {
	...
	> {
		...
		renderRecordingView(file: RecordedFile, studio: Studio) {
			...
			let urlPrefix = '';
			if (studio)
				urlPrefix = objectPath.get(
					studio,
					'testToolsConfig.recordings.urlPrefix',
					''
				);
			if (urlPrefix === '') {
				...
				<React.Fragment>
					...
					<div className="mod mvl">
						<UserActionsList
							...
							onItemClick={(item) =>
								this.seekToTime(item.timestamp - file.startedAt)
							}
						/>
					</div>
				</React.Fragment>
		...
		render() {
			...
			return (
				<div className="mtl gutter">
					{file && studio ? (
						this.renderRecordingView(file, studio)
					) : (
						<p>File not found</p>
					)}
				</div>
