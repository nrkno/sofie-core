...
describe('Test mos update next part helpers', () => {
	...
	testInFiber('ensureNextPartIsValid: Missing next part', () => {
		...
		expect(ServerPlayoutAPI.setNextPartInner).toHaveBeenCalledWith(
			expect.objectContaining({ _id: rundownId }),
	...
	// })
	testInFiber(
		'ensureNextPartIsValid: Missing current part with valid next',
		() => {
	...
	testInFiber(
		'ensureNextPartIsValid: Ensure correct part doesnt change',
		() => {
			resetPartIds('mock_part3', 'mock_part4');
	...
	testInFiber(
		'ensureNextPartIsValid: Ensure manual but missing part does change',
		() => {
	...
	testInFiber('afterInsertParts: Did not remove previous', () => {
		...
		const ensureMock = jest
			.spyOn(UpdateNext, 'ensureNextPartIsValid')
	...
	testInFiber(
		'afterInsertParts: Next part no longer exists, missing new parts',
		() => {
			resetPartIds('mock_part2', 'fake_part', true);
