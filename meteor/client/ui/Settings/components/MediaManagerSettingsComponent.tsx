...
export const MediaManagerSettingsComponent = translate()(
	class MediaManagerSettingsComponent extends React.Component<
		Translated<IMediaManagerSettingsComponentProps>,
		IMediaManagerSettingsComponentState
	> {
		...
		addNewStorage = () => {
			...
			let iter = 0;
			while (
				(settings.storages || []).findIndex((i) => i.id === newDeviceId + iter.toString()) >= 0
			) {
				iter++;
			}
			PeripheralDevices.update(this.props.device._id, {
				...
				return (
					<React.Fragment key={storage.id}>
						<tr
							...
							<td className="settings-studio-device__id c4">{StorageType[storage.type]}</td>
												...
												<div className="mod mvs mhs">
													<label className="field">
														...
														<EditAttribute
															modifiedClassName="bghl"
															attribute={
																'settings.storages.' + index + '.options.mappedNetworkedDriveTarget'
															}
															obj={this.props.device}
															type="text"
															collection={PeripheralDevices}
												...
												<div className="mod mvs mhs">
													<label className="field">
														...
														<EditAttribute
															modifiedClassName="bghl"
															attribute={
																'settings.storages.' + index + '.options.onlySelectedFiles'
															}
															obj={this.props.device}
															type="checkbox"
															collection={PeripheralDevices}
												...
												<React.Fragment>
													...
													<div className="mod mvs mhs">
														<label className="field">
															...
															<EditAttribute
																modifiedClassName="bghl"
																attribute={'settings.storages.' + index + '.options.usePolling'}
																obj={this.props.device}
																type="checkbox"
																collection={PeripheralDevices}
																className="input input-l"></EditAttribute>
		...
		addNewFlow = () => {
			...
			let iter = 0;
			while (
				(settings.mediaFlows || []).findIndex((i) => i.id === newFlowId + iter.toString()) >= 0
			) {
				iter++;
			}
			PeripheralDevices.update(this.props.device._id, {
				...
				return (
					<React.Fragment key={flow.id}>
						<tr
							...
							<td className="settings-studio-device__id c4">{MediaFlowType[flow.mediaFlowType]}</td>
							<td className="settings-studio-device__actions table-item-actions c3">
		...
		removeMonitor = (monitorId: string) => {
			...
			doModalDialog({
				...
				message: (
					<React.Fragment>
						<p>
							{t('Are you sure you want to delete the monitor "{{monitorId}}"?', {
								monitorId: monitorId
							})}
						</p>
					</React.Fragment>
				...
				return (
					<React.Fragment key={monitorId}>
						<tr
							...
							<td className="settings-studio-device__id c4">{MonitorSettingsType[monitor.type]}</td>
							<td className="settings-studio-device__actions table-item-actions c3">
									...
									<div>
										<div className="mod mvs mhs">
											<label className="field">
												...
												<EditAttribute
													...
													updateFunction={this.onEditMonitorId}></EditAttribute>
											</label>
										</div>
										<div className="mod mvs mhs">
											<label className="field">
												...
												<EditAttribute
													modifiedClassName="bghl"
													attribute={'settings.monitors.' + monitorId + '.type'}
													obj={this.props.device}
													type="dropdown"
										...
										<div className="mod mvs mhs">
											<label className="field">
												...
												<EditAttribute
													modifiedClassName="bghl"
													attribute={'settings.monitors.' + monitorId + '.storageId'}
													obj={this.props.device}
													type="text"
													collection={PeripheralDevices}
													className="input text-input input-l"></EditAttribute>
											...
											<React.Fragment>
												<div className="mod mvs mhs">
													<label className="field">
														...
														<EditAttribute
															modifiedClassName="bghl"
															attribute={'settings.monitors.' + monitorId + '.host'}
															obj={this.props.device}
															type="text"
															collection={PeripheralDevices}
															className="input text-input input-l"></EditAttribute>
												...
												<div className="mod mvs mhs">
													<label className="field">
														...
														<EditAttribute
															modifiedClassName="bghl"
															attribute={'settings.monitors.' + monitorId + '.port'}
															obj={this.props.device}
															type="int"
															collection={PeripheralDevices}
															className="input text-input input-l"></EditAttribute>
											...
											<React.Fragment>
												<div className="mod mvs mhs">
													<label className="field">
														...
														<EditAttribute
															modifiedClassName="bghl"
															attribute={'settings.monitors.' + monitorId + '.gatewayUrl'}
															obj={this.props.device}
															type="text"
															collection={PeripheralDevices}
															className="input text-input input-l"></EditAttribute>
												...
												<div className="mod mvs mhs">
													<label className="field">
														...
														<EditAttribute
															modifiedClassName="bghl"
															attribute={'settings.monitors.' + monitorId + '.ISAUrl'}
															obj={this.props.device}
															type="text"
															collection={PeripheralDevices}
															className="input text-input input-l"></EditAttribute>
												...
												<div className="mod mvs mhs">
													<label className="field">
														...
														<EditAttribute
															modifiedClassName="bghl"
															attribute={'settings.monitors.' + monitorId + '.zoneId'}
															obj={this.props.device}
															type="text"
															collection={PeripheralDevices}
															className="input text-input input-l"></EditAttribute>
												...
												<div className="mod mvs mhs">
													<label className="field">
														...
														<EditAttribute
															modifiedClassName="bghl"
															attribute={'settings.monitors.' + monitorId + '.serverId'}
															obj={this.props.device}
															type="int"
															collection={PeripheralDevices}
															className="input text-input input-l"></EditAttribute>
				...
				<div>
					...
					{settings && settings.storages && (
						<table className="expando settings-studio-device-table">
							<tbody>{this.renderStorages()}</tbody>
						</table>
					)}
					...
					{settings && settings.monitors && (
						<table className="expando settings-studio-device-table">
							<tbody>{this.renderMonitors()}</tbody>
						</table>
					)}
					<div className="mod mhs">
					...
					{subDevices && (
						<React.Fragment>
							...
							{subDevices.map((item) => (
								<DeviceItem key={item._id} device={item} showRemoveButtons={true} />
							))}
						</React.Fragment>
					)}
				</div>
