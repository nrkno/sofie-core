...
export const ClipTrimPanel = translateWithTracker<
	IProps,
	IState,
	ITrackedProps
>((props: IProps) => {
	...
	class ClipTrimPanel extends MeteorReactComponent<
	...
	> {
		...
		onInChange = (val: number) => {
			if (val < this.state.outPoint) {
				const ns = {
					inPoint: val,
					duration: Math.min(
						this.state.maxDuration - val,
						Math.max(0, this.state.outPoint - val)
					)
				};
				this.setState(ns);
				this.props.onChange(
					(ns.inPoint / this.fps) * 1000,
					(ns.duration / this.fps) * 1000
				);
			} else {
				...
				const ns = {
					inPoint: inp,
					duration: Math.min(
						this.state.maxDuration - inp,
						this.state.outPoint - inp
					)
				};
				this.setState(ns);
				this.props.onChange(
		...
		onDurationChange = (val: number) => {
			if (val > 0) {
				...
				this.setState(ns);
				this.props.onChange(
					((ns.outPoint - ns.duration) / this.fps) * 1000,
					(ns.duration / this.fps) * 1000
				);
			}
		};
		...
		onOutChange = (val: number) => {
			if (val > this.state.inPoint) {
				const ns = {
					...
					duration: Math.min(
						this.state.maxDuration - this.state.inPoint,
						Math.max(0, val - this.state.inPoint)
					)
				};
				this.setState(ns);
				this.props.onChange(
		...
		render() {
			...
			if (this.props.mediaObject && this.props.studio) {
				...
				previewUrl =
					mediaPreviewUrl +
					'media/preview/' +
					encodeURIComponent(this.props.mediaObject.mediaId);
			}
						...
						<div className="clip-trim-panel__monitors__monitor">
							<VideoEditMonitor
								...
								onCurrentTimeChange={(time) =>
									this.onOutChange(time * this.fps)
								}
							/>
						</div>
					</div>
