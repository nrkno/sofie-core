...
describe('lib/mediaObjects', () => {
	...
	testInFiber('checkPieceContentStatus', () => {
		...
		const status1 = checkPieceContentStatus(
			piece1,
			sourcelayer1,
			mockStudioSettings
		);
		...
		const status2 = checkPieceContentStatus(
			piece2,
			sourcelayer1,
			mockStudioSettings
		);
		expect(status2.status).toEqual(
			RundownAPI.PieceStatusCode.SOURCE_BROKEN
		);
		expect(status2.message).toContain('is not in accepted formats');

		const status3 = checkPieceContentStatus(
			piece3,
			sourcelayer1,
			mockStudioSettings
		);
		expect(status3.status).toEqual(
