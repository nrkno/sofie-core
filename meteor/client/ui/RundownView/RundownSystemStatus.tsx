...
export const MOSLastUpdateStatus = translate()(
	class extends React.Component<IMOSStatusProps & InjectedTranslateProps> {
		_interval: number;
		...
		render() {
			...
			return (
				<span>
					...
					{timeDiff >= 5 * 60 * 1000 &&
						timeDiff < 10 * 60 * 1000 &&
						t('Around 10 minutes ago')}
					{timeDiff >= 10 * 60 * 1000 &&
						timeDiff < 30 * 60 * 1000 &&
						t('More than 10 minutes ago')}
					{timeDiff >= 30 * 60 * 1000 &&
						timeDiff < 2 * 60 * 60 * 1000 &&
						t('More than 30 minutes ago')}
					{timeDiff >= 2 * 60 * 60 * 1000 &&
						timeDiff < 5 * 60 * 60 * 1000 &&
						t('More than 2 hours ago')}
					{timeDiff >= 5 * 60 * 60 * 1000 &&
...
function diffOnLineOffLineList(
	prevList: OnLineOffLineList,
	list: OnLineOffLineList
): OnLineOffLineList {
...
export const RundownSystemStatus = translateWithTracker(
	(props: IProps) => {
		...
		let playoutDevices = attachedDevices.filter(
			(i) => i.type === PeripheralDeviceAPI.DeviceType.PLAYOUT
		);

		const [ingest, playout] = _.map(
			[ingestDevices, playoutDevices],
			(devices) => {
				...
				const lastUpdate = _.reduce(
					devices,
					(memo, device: MosParentDevice) =>
						Math.max(device.lastDataReceived || 0, memo),
					0
				);
				return {
					status: status,
	...
	(data, props: IProps, nextProps: IProps) => {
		if (
			...
			props.studio._id === nextProps.studio._id
		)
			return false;
		return true;
	}
)(
	class RundownSystemStatus extends MeteorReactComponent<
	...
	> {
		...
		componentWillUnmount() {
			...
			if (this.notificationTimeout)
				Meteor.clearTimeout(this.notificationTimeout);
		}
		...
		componentDidUpdate(prevProps: IProps & ITrackedProps) {
			if (prevProps !== this.props) {
				...
				const mosDiff = diffOnLineOffLineList(
					prevProps.mosDevices,
					this.props.mosDevices
				);
				const playoutDiff = diffOnLineOffLineList(
					prevProps.playoutDevices,
					this.props.playoutDevices
				);
		...
		render() {
			const { t } = this.props;
			const playoutDevicesIssues = this.props.playoutDevices.offLine.filter(
				(dev) => dev.connected
			);
			const mosDevicesIssues = this.props.mosDevices.offLine.filter(
				(dev) => dev.connected
			);
			const mosDisconnected = this.props.mosDevices.offLine.filter(
				(dev) => !dev.connected
			);
			const playoutDisconnected = this.props.playoutDevices.offLine.filter(
				(dev) => !dev.connected
			);
