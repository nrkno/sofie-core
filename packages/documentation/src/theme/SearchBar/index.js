...
const Search = (props) => {
	...
	useEffect(() => {
		function handleCtrlK(event) {
			if (event.key !== 'k' || !event.ctrlKey || event.altKey || event.shiftKey || event.metaKey) return
		...
		document.body.addEventListener('keydown', handleCtrlK)
		return () => {
			document.body.removeEventListener('keydown', handleCtrlK)
		}
	}, [])
