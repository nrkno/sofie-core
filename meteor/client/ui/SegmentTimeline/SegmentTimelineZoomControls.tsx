...
export const SegmentTimelineZoomControls = class extends React.Component<
	IPropsHeader,
	IStateHeader
> {
	...
	zoomAreaMove = (e: Event | (TouchEvent & any)) => {
		...
		if (this._isTouch) {
			...
			percent = Math.max(
				0,
				Math.min(1, (e.clientX - this.offsetX - this.clickOffsetX) / this.state.width)
			);
		}
		// console.log(percent)
		if (this.props.onScroll) {
		...
		return (
			<div className="segment-timeline__zoom-area__controls" ref={this.setParentRef}>
				<div
					...
					style={{
						width:
							Math.min(
								100,
								Math.max(0, (this.props.scrollLeft / this.props.segmentDuration) * 100)
							).toString() + '%'
					}}
						...
						width:
							Math.min(
								...
								Math.max(
									0,
									(1 -
										(this.props.scrollLeft + this.props.scrollWidth) / this.props.segmentDuration) *
										100
								)
							).toString() + '%'
					}}
				...
				<div
					...
					style={{
						left:
							Math.max((this.props.scrollLeft / this.props.segmentDuration) * 100, 0).toString() +
							'%',
						width:
							Math.min(
