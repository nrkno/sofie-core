		...
		saveForm = (e: React.MouseEvent<HTMLElement>) => {
			...
			if (answers.q0 !== 'nothing') {
				doUserAction(
					...
					[this.props.rundown._id, 'Evaluation form'],
					(err, response) => {
						if (!err && response) {
...
export function getQuestionOptions(t) {
	return [
		...
		{
			value: 'minor',
			name: t("Something went wrong, but it didn't affect the output")
		},
		{
			value: 'major',
			name: t('Something went wrong, and it affected the output')
		}
	];
}
