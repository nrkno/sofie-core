...
export function getPreviousPartForSegment(
	rundownId: string,
	dbSegment: DBSegment
): Part | undefined {
...
export function setNextPart(
	...
	if (nextPart) {
		if (nextPart.rundownId !== rundown._id)
			throw new Meteor.Error(
				409,
				`Part "${nextPart._id}" not part of rundown "${rundown._id}"`
			);
		if (nextPart._id === rundown.currentPartId) {
			throw new Meteor.Error(
				402,
				'Not allowed to Next the currently playing Part'
			);
		}
		if (nextPart.invalid) {
			throw new Meteor.Error(
		...
		ps.push(
			asyncCollectionUpdate(Parts, nextPart._id, {
				$push: {
