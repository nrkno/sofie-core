...
postJsRoute.route(
	'/shelfLayouts/upload/:showStyleBaseId',
		...
		try {
			...
			if (!body)
				throw new Meteor.Error(
					400,
					'Restore Shelf Layout: Missing request body'
				);

			if (typeof body !== 'string' || body.length < 10)
				throw new Meteor.Error(
					400,
					'Restore Shelf Layout: Invalid request body'
				);
...
let methods: Methods = {};
methods[RundownLayoutsAPI.methods.createRundownLayout] = function(
	name: string,
	type: RundownLayoutType,
	showStyleBaseId: string
) {
...
methods[RundownLayoutsAPI.methods.removeRundownLayout] = function(id: string) {
	check(id, String);
