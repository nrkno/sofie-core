...
export function selectShowStyleVariant(
	...
	if (!studioBlueprint)
		throw new Meteor.Error(500, `Studio "${studio._id}" does not have a blueprint`);
	...
	if (!showStyleVariants.length)
		throw new Meteor.Error(500, `ShowStyleBase "${showStyleBase._id}" has no variants`);
...
export namespace ServerRundownAPI {
	export function removeRundown(rundownId: string) {
		...
		if (rundown.active)
			throw new Meteor.Error(400, `Not allowed to remove an active Rundown "${rundownId}".`);
