...
import { Studio, Studios, MappingExt } from '../../../lib/collections/Studios';
...
const StudioDevices = translate()(
	class StudioDevices extends React.Component<
		Translated<IStudioDevicesProps>,
		IStudioDevicesSettingsState
	> {
					...
					<tr key={device._id}>
						...
						<td className="settings-studio-device__actions table-item-actions c3">
							<button
								...
								onClick={(e) => this.confirmRemove(device)}>
				...
				<div>
					...
					<div className="mod mhs">
						<button
							...
							onClick={(e) => this.showAvailableDevices()}>
...
const StudioMappings = translate()(
	class StudioMappings extends React.Component<
		Translated<IStudioMappingsProps>,
		IStudioMappingsState
	> {
			...
			doModalDialog({
				...
				message: (
					<React.Fragment>
						<p>
							{t(
								'Are you sure you want to remove mapping for layer "{{mappingId}}"?',
								{ mappingId: mappingId }
							)}
						</p>
		...
		updateLayerId = (edit: EditAttributeBase, newValue: string) => {
			...
			if (this.props.studio.mappings[newLayerId]) {
				throw new Meteor.Error(
					400,
					'Layer "' + newLayerId + '" already exists'
				);
			}
		...
		renderMappings() {
			...
			return _.map(
				...
				(mapping: MappingExt, layerId: string) => {
					// If an internal mapping, then hide it
					if (mapping.internal)
						return <React.Fragment key={layerId}></React.Fragment>;
						...
						<React.Fragment key={layerId}>
							<tr
								...
								<td className="settings-studio-device__actions table-item-actions c3">
									<button
										className="action-btn"
										onClick={(e) => this.editItem(layerId)}>
										...
										<div>
											<div className="mod mvs mhs">
												<label className="field">
													...
													<i>
														{t(
															'ID of the timeline-layer to map to some output'
														)}
													</i>
												</label>
											</div>
...
> {
	...
	updateStatus(props?: Translated<IStudioBaselineStatusProps>) {
		...
		Meteor.call(
			PlayoutAPI.methods.shouldUpdateStudioBaseline,
			studio._id,
			(err, res) => {
				if (err) {
	...
	reloadBaseline() {
		Meteor.call(
			PlayoutAPI.methods.updateStudioBaseline,
			...
			<div>
				<p className="mhn">
					...
					{needsUpdate ? (
						<Tooltip
							overlay={t(
								'Baseline needs reload, this studio may not work until reloaded'
							)}
							visible={getHelpMode()}
							placement="right">
							<span>{t('Yes')}</span>
		...
		renderShowStyleEditButtons() {
			...
			if (this.props.studio) {
				this.props.studio.supportedShowStyleBase.map((style) => {
					let base = this.props.availableShowStyleBases.find(
						(base) => base.showStyleBase._id === style
					);
					if (base) {
						buttons.push(
							<SettingsNavigation
...
export function findHighestRank(
	array: Array<{ _rank: number }>
): { _rank: number } | null {
