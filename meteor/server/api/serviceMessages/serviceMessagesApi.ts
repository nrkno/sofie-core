...
function readAllMessages(): Array<ServiceMessage> {
	...
	if (!coreSystem || !coreSystem.serviceMessages) {
		logger.error(
			'coreSystem.serviceMessages doesnt exist. ServiceMessages API wont work.'
		);
		throw new Error(
			'coreSystem.serviceMessages is not available. Database not migrated?'
		);
	}

	const { serviceMessages } = coreSystem;
	const messages = Object.keys(serviceMessages).map(
		(key) => serviceMessages[key]
	);
...
function deleteMessage(id: string): ServiceMessage {
	...
	try {
		if (message) {
			...
			throw new Error(
				`Message with id ${id} can not be found, and therefore not deleted`
			);
		}
	} catch (error) {
		logger.error(error.message);
