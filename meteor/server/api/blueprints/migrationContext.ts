...
export class MigrationContextStudio implements IMigrationContextStudio {
	...
	insertMapping(
		...
		if (this.studio.mappings[mappingId]) {
			throw new Meteor.Error(
				404,
				`Mapping "${mappingId}" cannot be inserted as it already exists`
			);
		}
		if (!mappingId) {
	...
	updateMapping(mappingId: string, mapping: Partial<BlueprintMapping>): void {
		...
		if (!this.studio.mappings[mappingId]) {
			throw new Meteor.Error(
				404,
				`Mapping "${mappingId}" cannot be updated as it does not exist`
			);
		}

		if (mappingId) {
			let m: any = {};
			m['mappings.' + mappingId] = _.extend(
				this.studio.mappings[mappingId],
				mapping
			);
			Studios.update(this.studio._id, { $set: m });
			this.studio.mappings[mappingId] = m['mappings.' + mappingId]; // Update local
	...
	insertDevice(deviceId: string, device: TSR.DeviceOptionsAny): string {
		...
		if (!parentDevice) {
			throw new Meteor.Error(
				404,
				`No parent device for new device id "${deviceId}"`
			);
		}
		...
		if (settings && settings.devices[deviceId]) {
			throw new Meteor.Error(
				404,
				`Device "${deviceId}" cannot be inserted as it already exists`
			);
		}
	...
	updateDevice(
		...
		if (!parentDevice || !parentDevice.settings) {
			throw new Meteor.Error(
				404,
				`Device "${deviceId}" cannot be updated as it does not exist`
			);
		}
...
export class MigrationContextShowStyle implements IMigrationContextShowStyle {
	...
	insertVariant(
		variantId: string,
		variant: OmitId<ShowStyleVariantPart>
	): string {
		check(variantId, String);
		if (!variantId) {
			throw new Meteor.Error(500, `Variant id "${variantId}" is invalid`);
	...
	updateVariant(
		variantId: string,
		variant: Partial<ShowStyleVariantPart>
	): void {
		check(variantId, String);
		if (!variantId) {
			throw new Meteor.Error(500, `Variant id "${variantId}" is invalid`);
	...
	getSourceLayer(sourceLayerId: string): ISourceLayer | undefined {
		...
		if (!sourceLayerId) {
			throw new Meteor.Error(
				500,
				`SourceLayer id "${sourceLayerId}" is invalid`
			);
		}

		return _.find(
			this.showStyleBase.sourceLayers,
			(part) => part._id === sourceLayerId
		);
	}
	insertSourceLayer(
		sourceLayerId: string,
		layer: OmitId<ISourceLayer>
	): string {
		check(sourceLayerId, String);
		if (!sourceLayerId) {
			throw new Meteor.Error(
		...
		if (oldLayer) {
			throw new Meteor.Error(
				500,
				`SourceLayer "${sourceLayerId}" already exists`
			);
		}
	...
	updateSourceLayer(
		...
		const localLayerIndex = _.findIndex(
			this.showStyleBase.sourceLayers,
			(part) => part._id === sourceLayerId
		);
		if (localLayerIndex === -1) {
			throw new Meteor.Error(
				404,
				`SourceLayer "${sourceLayerId}" cannot be updated as it does not exist`
			);
		}
	...
	removeSourceLayer(sourceLayerId: string): void {
		...
		// Update local:
		this.showStyleBase.sourceLayers = _.reject(
			this.showStyleBase.sourceLayers,
			(c) => c._id === sourceLayerId
		);
	}
	getOutputLayer(outputLayerId: string): IOutputLayer | undefined {
		...
		if (!outputLayerId) {
			throw new Meteor.Error(
				500,
				`OutputLayer id "${outputLayerId}" is invalid`
			);
		}

		return _.find(
			this.showStyleBase.outputLayers,
			(part) => part._id === outputLayerId
		);
	}
	insertOutputLayer(
		outputLayerId: string,
		layer: OmitId<IOutputLayer>
	): string {
		check(outputLayerId, String);
		if (!outputLayerId) {
			throw new Meteor.Error(
		...
		if (oldLayer) {
			throw new Meteor.Error(
				500,
				`OutputLayer "${outputLayerId}" already exists`
			);
		}
	...
	updateOutputLayer(
		...
		if (localLayerIndex === -1) {
			throw new Meteor.Error(
				404,
				`OutputLayer "${outputLayerId}" cannot be updated as it does not exist`
			);
		}
	...
	removeOutputLayer(outputLayerId: string): void {
		...
		// Update local:
		this.showStyleBase.outputLayers = _.reject(
			this.showStyleBase.outputLayers,
			(c) => c._id === outputLayerId
		);
	}
	getBaseConfig(configId: string): ConfigItemValue | undefined {
		check(configId, String);
		let configItem = _.find(
			this.showStyleBase.config,
			(c) => c._id === configId
		);
		if (configItem) return trimIfString(configItem.value);
	}
	setBaseConfig(configId: string, value: ConfigItemValue): void {
		...
		if (_.isUndefined(value))
			throw new Meteor.Error(
				400,
				`setBaseConfig "${configId}": value is undefined`
			);
	...
	removeBaseConfig(configId: string): void {
		...
		if (configId) {
			...
			// Update local:
			this.showStyleBase.config = _.reject(
				this.showStyleBase.config,
				(c) => c._id === configId
			);
		}
	...
	setVariantConfig(
		...
		if (_.isUndefined(value))
			throw new Meteor.Error(
				400,
				`setVariantConfig "${variantId}", "${configId}": value is undefined`
			);
	...
	getRuntimeArgument(
		argumentId: string
	): IBlueprintRuntimeArgumentsItem | undefined {
		check(argumentId, String);
		if (!argumentId) {
			throw new Meteor.Error(
				500,
				`RuntimeArgument id "${argumentId}" is invalid`
			);
		}

		return _.find(
			this.showStyleBase.runtimeArguments || [],
			(ra) => ra._id === argumentId
		);
	}
	insertRuntimeArgument(
		argumentId: string,
		argument: OmitId<IBlueprintRuntimeArgumentsItem>
	) {
		check(argumentId, String);
		if (!argumentId) {
			throw new Meteor.Error(
		...
		if (oldRa) {
			throw new Meteor.Error(
				500,
				`RuntimeArgument "${argumentId}" already exists`
			);
		}
		...
		if (!this.showStyleBase.runtimeArguments)
			this.showStyleBase.runtimeArguments = [];
		this.showStyleBase.runtimeArguments.push(fullRa); // Update local
	}
	updateRuntimeArgument(
		argumentId: string,
		argument: Partial<OmitId<IBlueprintRuntimeArgumentsItem>>
	) {
		check(argumentId, String);
		if (!argumentId) {
			throw new Meteor.Error(
		...
		const localRaIndex = _.findIndex(
			this.showStyleBase.runtimeArguments || [],
			(ra) => ra._id === argumentId
		);
		if (localRaIndex === -1) {
			throw new Meteor.Error(
				404,
				`RuntimeArgument "${argumentId}" cannot be updated as it does not exist`
			);
		}
	...
	removeRuntimeArgument(argumentId: string) {
		...
		// Update local:
		this.showStyleBase.runtimeArguments = _.reject(
			this.showStyleBase.runtimeArguments,
			(c) => c._id === argumentId
		);
	}
