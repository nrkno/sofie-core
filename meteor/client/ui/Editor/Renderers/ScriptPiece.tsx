...
export const ScriptPiece = withTracker<IProps, IState, ITrackedProps>(
	(props: IProps) => {
	...
	class ScriptPiece extends MeteorReactComponent<Translated<IProps> & ITrackedProps, IState> {
		isLiveSegment: boolean
		isVisible: boolean
		...
		render() {
			console.log('PieceSCRV:', this.props)
			return (
				<React.Fragment>
					{this.props.segmentui && <h3>{this.props.segmentui.name}</h3>}
					{this.props.parts.map((part) => (
						<p key={part.partId + ''}>{part.partId}</p>
					))}
				</React.Fragment>
			)
		}
