...
export function getRundownIdFromMosRO(
	studio: Studio,
	runningOrderMosId: MOS.MosString128
) {
	if (!runningOrderMosId)
		throw new Meteor.Error(401, 'parameter runningOrderMosId missing!');
	return getRundownId(studio, parseMosString(runningOrderMosId));
}

export function getRundownFromMosRO(
	studio: Studio,
	runningOrderMosId: MOS.MosString128
) {
...
export function getPartIdFromMosStory(
	rundownId: string,
	partMosId: MOS.MosString128
) {
	if (!partMosId) throw new Meteor.Error(401, 'parameter partMosId missing!');
	return getPartId(rundownId, parseMosString(partMosId));
}

export function getSegmentExternalId(
	rundownId: string,
	ingestPart: IngestPart
) {
	return `${rundownId}_${ingestPart.name.split(';')[0]}_${
		ingestPart.externalId
	}`;
}
...
export function parseMosString(str: MOS.MosString128): string {
	if (!str)
		throw new Meteor.Error(401, 'parseMosString: str parameter missing!');
