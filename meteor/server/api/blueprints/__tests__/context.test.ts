...
describe('Test blueprint api context', () => {
	...
	describe('ShowStyleContext', () => {
		...
		test('handleNotesExternally', () => {
			...
			expect(notesContext.handleNotesExternally).toEqual(
		...
		test('getShowStyleConfigRef', () => {
			...
			const getShowStyleConfigRef = jest.spyOn(
				ConfigRef,
			...
			try {
				...
				expect(getShowStyleConfigRef).toHaveBeenCalledWith(
					(context as any).showStyleVariantId,
					'conf1'
				);
			} finally {
		...
		class FakeNotesContext implements INotesContext {
			...
			getHashId: (
				originString: string,
				originIsNotUnique?: boolean | undefined
			) => string = jest.fn(() => 'hashed');
	...
	describe('AsRunEventContext', () => {
		...
		test('getSegment - good', () => {
			...
			const segment = context.getSegment(
				`${rundown._id}_segment1`
			) as IBlueprintSegmentDB;
		...
		test('getPiece - good', () => {
			...
			const piece = context.getPiece(
				`${rundown._id}_part0_1_piece3`
