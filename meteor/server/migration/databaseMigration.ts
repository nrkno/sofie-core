 ...
 * @param steps
 */
export function addMigrationSteps(
	version: string,
	steps: Array<MigrationStepBase>
) {
	_.each(steps, (step) => {
		addMigrationStep(
			_.extend(step, {
...
export function prepareMigration(returnAllChunks?: boolean): PreparedMigration {
	...
	allMigrationSteps.sort((a, b) => {
		// First, sort by type:
		if (
			a.chunk.sourceType === MigrationStepType.CORE &&
			b.chunk.sourceType !== MigrationStepType.CORE
		)
			return -1;
		if (
			a.chunk.sourceType !== MigrationStepType.CORE &&
		...
		if (
			a.chunk.sourceType === MigrationStepType.STUDIO &&
			b.chunk.sourceType !== MigrationStepType.STUDIO
		)
			return -1;
		if (
			a.chunk.sourceType !== MigrationStepType.STUDIO &&
		...
		if (
			a.chunk.sourceType === MigrationStepType.SHOWSTYLE &&
			b.chunk.sourceType !== MigrationStepType.SHOWSTYLE
		)
			return -1;
		if (
			a.chunk.sourceType !== MigrationStepType.SHOWSTYLE &&
	...
	_.each(allMigrationSteps, (step: MigrationStepInternal) => {
		if (
			!step.canBeRunAutomatically &&
			(!step.input || (_.isArray(step.input) && !step.input.length))
		)
			throw new Meteor.Error(
				500,
				`MigrationStep "${step.id}" is manual, but no input is provided`
			);
		...
		if (
			...
			if (migrationSteps[step.id] || ignoredSteps[step.id])
				throw new Meteor.Error(
					500,
					`Error: MigrationStep.id must be unique: "${step.id}"`
				);
			...
			try {
				if (step.chunk.sourceType === MigrationStepType.CORE) {
					...
					throw new Meteor.Error(
						500,
						`Unknown step.chunk.sourceType "${step.chunk.sourceType}"`
					);
			} catch (error) {
				throw new Meteor.Error(
					500,
					`Error in migration step "${step.id}": ${error.reason ||
						error.toString()}`
				);
			}
...
function prefixIdsOnStep(
	prefix: string,
	step: MigrationStepInternal
): MigrationStepInternal {
...
export function runMigration(
	...
	let manualInputsWithUserPrompt = _.filter(
		migration.manualInputs,
		(manualInput) => {
	...
	if (migration.hash !== hash)
		throw new Meteor.Error(
			500,
			`Migration input hash differ from expected: "${hash}", "${migration.hash}"`
		);
	...
	if (unmatchedChunk)
		throw new Meteor.Error(
			500,
			`Migration input chunks differ from expected, chunk "${JSON.stringify(
				unmatchedChunk
			)}" not found in input`
		);
	...
	if (migration.chunks.length !== chunks.length)
		throw new Meteor.Error(500, `Migration input chunk lengths differ`);
...
function completeMigration(chunks: Array<MigrationChunk>) {
	_.each(chunks, (chunk) => {
		if (chunk.sourceType === MigrationStepType.CORE) {
			...
			if (!chunk.blueprintId)
				throw new Meteor.Error(500, `chunk.blueprintId missing!`);
			if (!chunk.sourceId)
				throw new Meteor.Error(500, `chunk.sourceId missing!`);

			let blueprint = Blueprints.findOne(chunk.blueprintId);
			if (!blueprint)
				throw new Meteor.Error(
					404,
					`Blueprint "${chunk.blueprintId}" not found!`
				);
			...
			if (chunk.sourceType === MigrationStepType.STUDIO) {
				...
				throw new Meteor.Error(
					500,
					`Bad chunk.sourcetype: "${chunk.sourceType}"`
				);
			...
			throw new Meteor.Error(
				500,
				`Unknown chunk.sourcetype: "${chunk.sourceType}"`
			);
	});
}
...
function getMigrationStudioContext(
	chunk: MigrationChunk
): IMigrationContextStudio {
	if (chunk.sourceType !== MigrationStepType.STUDIO)
		throw new Meteor.Error(
			500,
			`wrong chunk.sourceType "${chunk.sourceType}", expected STUDIO`
		);
	if (!chunk.sourceId) throw new Meteor.Error(500, `chunk.sourceId missing`);

	let studio = Studios.findOne(chunk.sourceId);
	if (!studio)
		throw new Meteor.Error(404, `Studio "${chunk.sourceId}" not found`);

	return new MigrationContextStudio(studio);
}
function getMigrationShowStyleContext(
	chunk: MigrationChunk
): IMigrationContextShowStyle {
	if (chunk.sourceType !== MigrationStepType.SHOWSTYLE)
		throw new Meteor.Error(
			500,
			`wrong chunk.sourceType "${chunk.sourceType}", expected SHOWSTYLE`
		);
	if (!chunk.sourceId) throw new Meteor.Error(500, `chunk.sourceId missing`);
