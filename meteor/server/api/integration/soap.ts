...
export async function sendSOAPMessage(msg: ExternalMessageQueueObjSOAP0 & ExternalMessageQueueObj) {
	...
	if (!msg.receiver.url)
		throwFatalError(msg, new Meteor.Error(401, 'attribute .receiver.url missing!'));
	if (!msg.message) throwFatalError(msg, new Meteor.Error(401, 'attribute .message missing!'));
	if (!msg.message.fcn)
		throwFatalError(msg, new Meteor.Error(401, 'attribute .message.fcn missing!'));
	if (!msg.message.clip_key)
		throwFatalError(msg, new Meteor.Error(401, 'attribute .message.clip_key missing!'));
	if (!msg.message.clip)
		throwFatalError(msg, new Meteor.Error(401, 'attribute .message.clip missing!'));
	...
	await new Promise((resolve, reject) => {
		...
		if (fcn) {
			...
			reject(
				new Meteor.Error(401, 'SOAP method "' + msg.message.fcn + '" missing on endpoint!')
			);
		}
	});
}
