...
import { Rundowns, RundownHoldState } from '../../lib/collections/Rundowns';
...
export function take(rundownId: string): ClientAPI.ClientResponse {
	...
	if (!rundown.active) {
		return ClientAPI.responseError(
			`Rundown is not active, please activate the rundown before doing a TAKE.`
		);
	}
	if (!rundown.nextPartId) {
		return ClientAPI.responseError(
			'No Next point found, please set a part as Next before doing a TAKE.'
		);
	}
	if (rundown.currentPartId) {
		const currentPart = Parts.findOne(rundown.currentPartId);
		if (currentPart && currentPart.timings) {
			const lastStartedPlayback = currentPart.timings.startedPlayback
				? currentPart.timings.startedPlayback[
						currentPart.timings.startedPlayback.length - 1
				  ]
				: 0;
			const lastTake = currentPart.timings.take
				? currentPart.timings.take[currentPart.timings.take.length - 1]
				: 0;
			...
			logger.error(
				`Part "${rundown.currentPartId}", set as currentPart in "${rundownId}", not found!`
			);
		}
	}
...
export function setNext(
	...
	if (!rundown.active)
		return ClientAPI.responseError(
			'Rundown is not active, please activate it before setting a part as Next'
		);
	...
	if (nextPartId) {
		...
		if (nextPart.invalid)
			return ClientAPI.responseError(
				'Part is marked as invalid, cannot set as next.'
			);
		if (nextPart.floated)
			return ClientAPI.responseError(
				'Part is marked as floated, cannot set as next.'
			);
	}

	if (rundown.holdState && rundown.holdState !== RundownHoldState.COMPLETE) {
		return ClientAPI.responseError(
			'The Next cannot be changed next during a Hold!'
		);
	}

	return ServerPlayoutAPI.setNextPart(
		rundownId,
		nextPartId,
		setManually,
		timeOffset
...
export function moveNext(
	...
	if (!rundown.active)
		return ClientAPI.responseError(
			'Rundown is not active, please activate it first'
		);
...
export function prepareForBroadcast(
	...
	if (rundown.active)
		return ClientAPI.responseError(
			'Rundown is active, please deactivate before preparing it for broadcast'
		);
	...
	if (anyOtherActiveRundowns.length) {
		return ClientAPI.responseError(
			...
			'Only one rundown can be active at the same time. Currently active rundowns: ' +
				_.map(anyOtherActiveRundowns, rundown.name).join(', '),
...
export function resetRundown(rundownId: string): ClientAPI.ClientResponse {
	...
	if (rundown.active && !rundown.rehearsal) {
		return ClientAPI.responseError(
			'Rundown is active but not in rehearsal, please deactivate it or set in in rehearsal to be able to reset it.'
		);
	}

	return ClientAPI.responseSuccess(ServerPlayoutAPI.resetRundown(rundownId));
}
...
export function reloadData(rundownId: string) {
	return ClientAPI.responseSuccess(ServerPlayoutAPI.reloadData(rundownId));
}
...
export function togglePartArgument(
	...
	if (
		...
		return ClientAPI.responseError(
			`Part-arguments can't be toggled while Rundown is in Hold mode!`
		);
	}
	return ClientAPI.responseSuccess(
		ServerPlayoutAPI.rundownTogglePartArgument(
			rundownId,
			partId,
			property,
			value
...
export function pieceTakeNow(
	...
	if (!rundown.active)
		return ClientAPI.responseError(
			`The Rundown isn't active, please activate it before starting an AdLib!`
		);
	...
	if (rundown.currentPartId !== part._id)
		return ClientAPI.responseError(
			`Part AdLib-pieces can be only placed in a current part!`
		);

	let showStyleBase = rundown.getShowStyleBase();
	const sourceL = showStyleBase.sourceLayers.find(
		(i) => i._id === piece.sourceLayerId
	);
	if (sourceL && sourceL.type !== SourceLayerType.GRAPHICS)
		return ClientAPI.responseError(
			`Part "${pieceId}" is not a GRAPHICS piece!`
		);
...
export function pieceSetInOutPoints(
	...
	if (rundown && rundown.active && part.status === 'PLAY') {
		return ClientAPI.responseError(
			`Part cannot be active while setting in/out!`
		); // @todo: un-hardcode
	}
	...
	if (!partCache)
		throw new Meteor.Error(404, `Part Cache for "${partId}" not found!`);
...
export function segmentAdLibPieceStart(
	...
	if (
		...
		return ClientAPI.responseError(
			`Can't start AdLibPiece when the Rundown is in Hold mode!`
		);
	}
...
export function sourceLayerOnPartStop(
	...
	if (!rundown.active)
		return ClientAPI.responseError(
			`The Rundown isn't active, can't stop an AdLib on a deactivated Rundown!`
		);
...
export function rundownBaselineAdLibPieceStart(
	...
	if (
		...
		return ClientAPI.responseError(
			`Can't start AdLib piece when the Rundown is in Hold mode!`
		);
	}
	return ClientAPI.responseSuccess(
		ServerPlayoutAPI.rundownBaselineAdLibPieceStart(
			rundownId,
			partId,
			pieceId,
			queue
...
export function segmentAdLibPieceStop(
	rundownId: string,
	partId: string,
	pieceId: string
) {
	...
	if (!rundown.active)
		return ClientAPI.responseError(
			`The Rundown isn't active, can't stop an AdLib in a deactivated Rundown!`
		);
...
export function sourceLayerStickyPieceStart(
	...
	if (!rundown.active)
		return ClientAPI.responseError(
			`The Rundown isn't active, please activate it before starting a sticky-item!`
		);
	if (!rundown.currentPartId)
		return ClientAPI.responseError(
			`No part is playing, please Take a part before starting a sticky-item.`
		);
...
export function activateHold(rundownId: string, undo?: boolean) {
	...
	if (!rundown.currentPartId)
		return ClientAPI.responseError(
			`No part is currently playing, please Take a part before activating Hold mode!`
		);
	if (!rundown.nextPartId)
		return ClientAPI.responseError(
			`No part is set as Next, please set a Next before activating Hold mode!`
		);
...
export function userSaveEvaluation(
	...
	return ClientAPI.responseSuccess(saveEvaluation.call(this, evaluation));
}
export function userStoreRundownSnapshot(rundownId: string, reason: string) {
	return ClientAPI.responseSuccess(storeRundownSnapshot(rundownId, reason));
}
export function removeRundown(rundownId: string) {
	...
	if (rundown.active)
		return ClientAPI.responseError(
			`The Rundown is currently active, you can't remove an active Rundown!`
		);
...
export function recordStop(studioId: string) {
	...
	if (!record)
		return ClientAPI.responseError(
			`No active recording for "${studioId}" was found!`
		);
...
export function recordStart(studioId: string, fileName: string) {
	...
	if (!studio)
		throw new Meteor.Error(404, `Studio "${studioId}" was not found!`);
	...
	if (active)
		return ClientAPI.responseError(
			`There is already an active recording in studio "${studioId}"!`
		);

	const config = getStudioConfig(studio);
	if (!config.recordings.channelIndex)
		return ClientAPI.responseError(
			`Cannot start recording due to a missing setting: "channel".`
		);
	if (!config.recordings.deviceId)
		return ClientAPI.responseError(
			`Cannot start recording due to a missing setting: "device".`
		);
	if (!config.recordings.decklinkDevice)
		return ClientAPI.responseError(
			`Cannot start recording due to a missing setting: "decklink".`
		);
	if (!config.recordings.channelIndex)
		return ClientAPI.responseError(
			`Cannot start recording due to a missing setting: "channel".`
...
export function regenerateRundown(rundownId: string) {
	...
	if (rundown.active) {
		return ClientAPI.responseError(
			`Rundown is active, please deactivate it before regenerating it.`
		);
	}
...
interface UserMethods {
	[method: string]: (
		...args: any[]
	) => ClientAPI.ClientResponse | Promise<ClientAPI.ClientResponse>;
}
let methods: UserMethods = {};

methods[UserActionAPI.methods.take] = function(
	rundownId: string
): ClientAPI.ClientResponse {
	return take.call(this, rundownId);
};
methods[UserActionAPI.methods.setNext] = function(
	rundownId: string,
	partId: string,
	timeOffset?: number
): ClientAPI.ClientResponse {
...
methods[UserActionAPI.methods.moveNext] = function(
	rundownId: string,
	horisontalDelta: number,
	verticalDelta: number
): ClientAPI.ClientResponse {
...
methods[UserActionAPI.methods.prepareForBroadcast] = function(
	rundownId: string
): ClientAPI.ClientResponse {
	return prepareForBroadcast.call(this, rundownId);
};
methods[UserActionAPI.methods.resetRundown] = function(
	rundownId: string
): ClientAPI.ClientResponse {
	return resetRundown.call(this, rundownId);
};
methods[UserActionAPI.methods.resetAndActivate] = function(
	rundownId: string,
	rehearsal?: boolean
): ClientAPI.ClientResponse {
	return resetAndActivate.call(this, rundownId, rehearsal);
};
methods[UserActionAPI.methods.activate] = function(
	rundownId: string,
	rehearsal: boolean
): ClientAPI.ClientResponse {
	return activate.call(this, rundownId, rehearsal);
};
methods[UserActionAPI.methods.deactivate] = function(
	rundownId: string
): ClientAPI.ClientResponse {
	return deactivate.call(this, rundownId);
};
methods[UserActionAPI.methods.forceResetAndActivate] = function(
	rundownId: string,
	rehearsal: boolean
): ClientAPI.ClientResponse {
	return forceResetAndActivate.call(this, rundownId, rehearsal);
};
methods[UserActionAPI.methods.reloadData] = function(
	rundownId: string
): ClientAPI.ClientResponse {
	return reloadData.call(this, rundownId);
};
methods[UserActionAPI.methods.unsyncRundown] = function(
	rundownId: string
): ClientAPI.ClientResponse {
	return unsyncRundown.call(this, rundownId);
};
methods[UserActionAPI.methods.disableNextPiece] = function(
	rundownId: string,
	undo?: boolean
): ClientAPI.ClientResponse {
	return disableNextPiece.call(this, rundownId, undo);
};
methods[UserActionAPI.methods.togglePartArgument] = function(
	rundownId: string,
	partId: string,
	property: string,
	value: string
...
methods[UserActionAPI.methods.pieceTakeNow] = function(
	rundownId: string,
	partId: string,
	pieceId: string
): ClientAPI.ClientResponse {
...
methods[UserActionAPI.methods.setInOutPoints] = function(
	rundownId: string,
	partId: string,
	pieceId: string,
	inPoint: number,
	...
	return pieceSetInOutPoints.call(
		this,
		rundownId,
		partId,
		pieceId,
...
methods[UserActionAPI.methods.segmentAdLibPieceStart] = function(
	rundownId: string,
	partId: string,
	salliId: string,
	queue: boolean
...
methods[UserActionAPI.methods.sourceLayerOnPartStop] = function(
	rundownId: string,
	partId: string,
	sourceLayerId: string
) {
...
methods[UserActionAPI.methods.baselineAdLibPieceStart] = function(
	rundownId: string,
	partId: string,
	pieceId: string,
	queue: boolean
) {
	return rundownBaselineAdLibPieceStart.call(
		this,
		rundownId,
		partId,
		pieceId,
...
methods[UserActionAPI.methods.segmentAdLibPieceStop] = function(
	rundownId: string,
	partId: string,
	pieceId: string
) {
...
methods[UserActionAPI.methods.sourceLayerStickyPieceStart] = function(
	rundownId: string,
	sourceLayerId: string
) {
	return sourceLayerStickyPieceStart.call(this, rundownId, sourceLayerId);
};
methods[UserActionAPI.methods.activateHold] = function(
	rundownId: string,
	undo?: boolean
): ClientAPI.ClientResponse {
	return activateHold.call(this, rundownId, undo);
};
methods[UserActionAPI.methods.saveEvaluation] = function(
	evaluation: EvaluationBase
): ClientAPI.ClientResponse {
	return userSaveEvaluation.call(this, evaluation);
};
methods[UserActionAPI.methods.storeRundownSnapshot] = function(
	rundownId: string,
	reason: string
) {
...
methods[UserActionAPI.methods.recordStart] = function(
	studioId: string,
	name: string
) {
...
methods[UserActionAPI.methods.mediaRestartWorkflow] = function(
	workflowId: string
) {
	return mediaRestartWorkflow.call(this, workflowId);
};
methods[UserActionAPI.methods.mediaAbortWorkflow] = function(
	workflowId: string
) {
	return mediaAbortWorkflow.call(this, workflowId);
};
methods[UserActionAPI.methods.mediaPrioritizeWorkflow] = function(
	workflowId: string
) {
