...
describe('Test blueprint post-process', () => {
	...
	describe('postProcessPieces', () => {
		testInFiber('no pieces', () => {
			...
			// Ensure that an empty array works ok
			const res = postProcessPieces(context, [], protectString('blueprint9'), context._rundown._id, protectString('part8'))
			expect(res).toHaveLength(0)
		})
		testInFiber('null piece', () => {
			const context = getContext()
			...
			// Ensure that a null object gets dropped
			const res = postProcessPieces(context, [null as any], protectString('blueprint9'), context._rundown._id, protectString('part8'))
			expect(res).toHaveLength(0)
		})
		testInFiber('various pieces', () => {
			const context = getContext()
			...
			const res = postProcessPieces(context, pieces, protectString('blueprint9'), context._rundown._id, protectString('part8'))
			expect(res).toMatchObject(pieces.map(p => _.omit(p, '_id')))
		...
		testInFiber('piece with content', () => {
			...
			const res = postProcessPieces(context, [piece], protectString('blueprint9'), context._rundown._id, protectString('part6'))
			expect(res).toHaveLength(1)
