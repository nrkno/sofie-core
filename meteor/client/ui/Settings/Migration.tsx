...
export const MigrationView = translateWithTracker<IProps, IState, ITrackedProps>(
	(props: IProps) => {
		return {};
	}
)(
	class MigrationView extends MeteorReactComponent<Translated<IProps & ITrackedProps>, IState> {
		...
		forceMigration() {
			this.setErrorMessage('');
			if (this.state.migration) {
				Meteor.call(MigrationMethods.forceMigration, this.state.migration.chunks, (err) => {
					if (this.cancelRequests) return;
					if (err) {
						logger.error(err);
						// todo: notify user
							...
							<div>
								{this.state.migration
									? _.map(this.state.migration.chunks, (chunk, i) => {
											...
											return (
												<div key={i}>
													{chunk._dbVersion === chunk._targetVersion ? <b>{str}</b> : str}
												</div>
											);
									  })
									: null}
										...
										<tr
											...
											<td className="table-item-actions c3">
												<button
													...
													onClick={(e) =>
														this.setState({ showAllSteps: !this.state.showAllSteps })
													}>
