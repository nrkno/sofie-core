...
import { PieceUi, SegmentTimelineContainer } from './SegmentTimeline/SegmentTimelineContainer';
import {
	RONotificationEvent,
	RundownNotifier,
	reloadRundownClick,
...
import { TSR, VTContent, VTEditableParameters } from 'tv-automation-sofie-blueprints-integration';
import { Translated, translateWithTracker } from '../lib/ReactMeteorData/react-meteor-data';
import { getAllowDeveloper, getAllowStudio, getHelpMode } from '../lib/localStorage';
import {
	maintainFocusOnPart,
	scrollToPart,
	scrollToPosition,
							...
							? (this.props.rundown.expectedStart && (
									<span className="timing-clock countdown playback-started left">
										...
										{RundownUtils.formatDiffToTimecode(
											this.props.rundown.startedPlayback - this.props.rundown.expectedStart,
											true,
											false,
											true,
											true,
								...
								{this.props.rundown.expectedStart && this.props.rundown.expectedDuration && (
									<span className="timing-clock plan-end right visual-last-child">
										...
										<Moment
											interval={0}
											format="HH:mm:ss"
											date={this.props.rundown.expectedStart + this.props.rundown.expectedDuration}
										/>
									</span>
								)}
								{this.props.rundown.expectedStart && this.props.rundown.expectedDuration && (
								...
								{this.props.timingDurations ? (
									<span className="timing-clock plan-end right visual-last-child">
										...
										<Moment
											...
											format="HH:mm:ss"
											date={
												getCurrentTime() + (this.props.timingDurations.totalRundownDuration || 0)
											}
										/>
									</span>
								) : null}
...
const RundownHeader = translate()(
	class extends React.Component<Translated<IRundownHeaderProps>, IRundownHeaderState> {
		bindKeys: Array<{
			key: string;
		...
		rundownShouldHaveEnded() {
			return (
				getCurrentTime() >
				(this.props.rundown.expectedStart || 0) + (this.props.rundown.expectedDuration || 0)
			);
		}
		...
		activate = (e: any) => {
			...
			if (
				this.props.studioMode &&
				(!this.props.rundown.active || (this.props.rundown.active && this.props.rundown.rehearsal))
			) {
				const onSuccess = () => {
					...
					<div
						...
						<ContextMenuTrigger
							...
							<div className="row first-row super-dark">
								...
								{this.props.studio && (
									<RundownSystemStatus studio={this.props.studio} rundown={this.props.rundown} />
								)}
							</div>
	...
	class RundownView extends MeteorReactComponent<Translated<IProps & ITrackedProps>, IState> {
		...
		onSelectPiece = (piece: PieceUi, e: React.MouseEvent<HTMLDivElement>) => {
			if (
				...
				(((piece.content as VTContent).editable as VTEditableParameters).editorialDuration !==
					undefined ||
					((piece.content as VTContent).editable as VTEditableParameters).editorialStart !==
						undefined)
			) {
				this.setState({
		...
		renderSegments() {
			if (this.props.segments) {
				return this.props.segments.map((segment, index, array) => {
					if (this.props.studio && this.props.rundown && this.props.showStyleBase) {
						return (
					...
					return (
						<RundownTimingProvider
							...
							<div
								...
								<ErrorBoundary>{this.state.studioMode && <KeyboardFocusMarker />}</ErrorBoundary>
								<ErrorBoundary>
