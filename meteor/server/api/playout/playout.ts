...
export namespace ServerPlayoutAPI {
	...
	export function prepareRundownForBroadcast(rundownId: string) {
		return rundownSyncFunction(
			rundownId,
			RundownSyncFunctionPriority.Playout,
			() => {
				...
				if (rundown.active)
					throw new Meteor.Error(
						404,
						`rundownPrepareForBroadcast cannot be run on an active rundown!`
					);

				const anyOtherActiveRundowns = areThereActiveRundownsInStudio(
					rundown.studioId,
					rundown._id
				);
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (rundown.active && !rundown.rehearsal)
					throw new Meteor.Error(
						401,
						`rundownResetBroadcast can only be run in rehearsal!`
					);
	 ...
	 */
	export function resetAndActivateRundown(
		rundownId: string,
		rehearsal?: boolean
	) {
		return rundownSyncFunction(
			...
			() => {
				...
				if (rundown.active && !rundown.rehearsal)
					throw new Meteor.Error(
						402,
						`rundownResetAndActivate cannot be run when active!`
					);
	 ...
	 */
	export function forceResetAndActivateRundown(
		rundownId: string,
		rehearsal: boolean
	) {
		...
		return rundownSyncFunction(
			...
			() => {
				...
				let anyOtherActiveRundowns = areThereActiveRundownsInStudio(
					rundown.studioId,
					rundown._id
				);
				let error: any;
				...
				if (error) {
					...
					anyOtherActiveRundowns = areThereActiveRundownsInStudio(
						rundown.studioId,
						rundown._id
					);
					if (anyOtherActiveRundowns.length) {
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (!rundown.active)
					throw new Meteor.Error(
						501,
						`Rundown "${rundownId}" is not active!`
					);
				if (!rundown.nextPartId)
					throw new Meteor.Error(500, 'nextPartId is not set!');
				...
				let pBlueprint = makePromise(() =>
					getBlueprintOfRundown(rundown)
				);
				...
				if (currentPart && currentPart.transitionDuration) {
					const prevPart = rundown.previousPartId
						? rundownData.partsMap[rundown.previousPartId]
						: undefined;
					const allowTransition =
						prevPart && !prevPart.disableOutTransition;
					...
					if (allowTransition) {
						...
						if (
							start &&
							now < start + currentPart.transitionDuration
						) {
							return ClientAPI.responseError(
								'Cannot take during a transition'
							);
						}
					}
				}
				...
				if (rundown.holdState === RundownHoldState.COMPLETE) {
					...
					ps.push(
						asyncCollectionUpdate(Rundowns, rundown._id, {
							$set: {
					...
					if (rundown.currentPartId) {
						...
						if (!currentPart)
							throw new Meteor.Error(
								404,
								'currentPart not found!'
							);

						// Remove the current extension line
						ps.push(
							asyncCollectionRemove(Pieces, {
				...
				if (!takePart)
					throw new Meteor.Error(404, 'takePart not found!');
				// let takeSegment = rundownData.segmentsMap[takePart.segmentId]
				let partAfter = fetchAfter(
					rundownData.parts,
					{
				...
				if (blueprint.getEndStateForPart && previousPart) {
					...
					previousPartEndState = blueprint.getEndStateForPart(
						context,
						rundown.previousPersistentState,
						previousPart.previousPartEndState,
						resolvedPieces,
						time
					);
					logger.info(
						`Calculated end state in ${getCurrentTime() - time}ms`
					);
				}
				...
				let m: Partial<Rundown> = {
					...
					holdState:
						!rundown.holdState ||
						rundown.holdState === RundownHoldState.COMPLETE
				...
				if (m.previousPartId) {
					ps.push(
						asyncCollectionUpdate(Parts, m.previousPartId, {
							$push: {
								'timings.takeOut': now
							}
						})
				...
				if (
					m.previousPartId &&
					m.holdState === RundownHoldState.ACTIVE
				) {
					let previousPart = rundownData.partsMap[m.previousPartId];
					if (!previousPart)
						throw new Meteor.Error(404, 'previousPart not found!');

					// Make a copy of any item which is flagged as an 'infinite' extension
					const itemsToCopy = previousPart
						.getAllPieces()
						.filter((i) => i.extendOnHold);
	...
	export function setNextPartInner(
		...
		if (!rundown.active)
			throw new Meteor.Error(
				501,
				`Rundown "${rundown._id}" is not active!`
			);
		...
		if (
			...
			throw new Meteor.Error(
				501,
				`Rundown "${rundown._id}" cannot change next during hold!`
			);
		...
		if (nextPartId) {
			...
			if (!nextPart)
				throw new Meteor.Error(404, `Part "${nextPartId}" not found!`);
		}
	...
	export function moveNextPart(
		...
		if (!horisontalDelta && !verticalDelta)
			throw new Meteor.Error(
				402,
				`rundownMoveNext: invalid delta: (${horisontalDelta}, ${verticalDelta})`
			);
	...
	function moveNextPartInner(
		...
		if (
			...
			throw new Meteor.Error(
				501,
				`Rundown "${rundownId}" cannot change next during hold!`
			);
		...
		if (nextPartId0) {
			...
			if (!rundown.nextPartId) {
				if (!rundown.currentPartId)
					throw new Meteor.Error(
						501,
						`Rundown "${rundownId}" has no next and no current part!`
					);
		...
		let currentNextSegment = Segments.findOne(
			currentNextPart.segmentId
		) as Segment;
		if (!currentNextSegment)
			throw new Meteor.Error(
				404,
				`Segment "${currentNextPart.segmentId}" not found!`
			);
		...
		if (partIndex === -1)
			throw new Meteor.Error(404, `Part not found in list of parts!`);
		if (segmentIndex === -1)
			throw new Meteor.Error(
				404,
				`Segment "${currentNextSegment._id}" not found in segmentsWithParts!`
			);
		if (verticalDelta !== 0) {
			...
			if (!part)
				throw new Meteor.Error(
					404,
					`No Parts in segment "${segment._id}"!`
				);
			...
			if (partIndex === -1)
				throw new Meteor.Error(
					404,
					`Part (from segment) not found in list of parts!`
				);
		}
		...
		if (!part)
			throw new Meteor.Error(
				501,
				`Part index ${partIndex} not found in list of parts!`
			);

		if (
			(part._id === rundown.currentPartId && !nextPartId0) ||
			part.invalid ||
			part.floated
		) {
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (!rundown.currentPartId)
					throw new Meteor.Error(
						400,
						`Rundown "${rundownId}" no current part!`
					);
				if (!rundown.nextPartId)
					throw new Meteor.Error(
						400,
						`Rundown "${rundownId}" no next part!`
					);

				const currentPart = Parts.findOne({
				...
				if (!nextPart)
					throw new Meteor.Error(
						404,
						`Part "${rundown.nextPartId}" not found!`
					);

				if (
					currentPart.holdMode !== PartHoldMode.FROM ||
					nextPart.holdMode !== PartHoldMode.TO
				) {
					throw new Meteor.Error(
						400,
						`Rundown "${rundownId}" incompatible pair of HoldMode!`
					);
				}
				...
				if (rundown.holdState) {
					throw new Meteor.Error(
						400,
						`Rundown "${rundownId}" already doing a hold!`
					);
				}

				Rundowns.update(rundownId, {
					$set: { holdState: RundownHoldState.PENDING }
				});
	...
	export function deactivateHold(rundownId: string) {
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (rundown.holdState !== RundownHoldState.PENDING)
					throw new Meteor.Error(
						400,
						`Rundown "${rundownId}" is not pending a hold!`
					);

				Rundowns.update(rundownId, {
					$set: { holdState: RundownHoldState.NONE }
				});
	...
	export function disableNextPiece(rundownId: string, undo?: boolean) {
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (!rundown.currentPartId)
					throw new Meteor.Error(401, `No current part!`);
				...
				const nextPart = rundown.nextPartId
					? Parts.findOne(rundown.nextPartId)
					: undefined;
				...
				if (!currentSement)
					throw new Meteor.Error(
						404,
						`Segment "${currentPart.segmentId}" not found!`
					);

				let o = getResolvedSegment(
					showStyleBase,
					rundown,
					currentSement
				);
				...
				let allowedSourceLayers: {
					[layerId: string]: ISourceLayerExtended;
				} = {};
				_.each(
					o.segmentExtended.sourceLayers,
					(sourceLayer: ISourceLayerExtended) => {
						if (sourceLayer.allowDisable)
							allowedSourceLayers[sourceLayer._id] = sourceLayer;
					}
				);
				...
				let getNextPiece = (part: Part, undo?: boolean) => {
					...
					if (
						...
						let lastStartedPlayback = _.last(
							part.timings.startedPlayback
						);
					...
					let filteredPieces = _.sortBy(
						_.filter(pieces, (piece: PieceResolved) => {
							let sourceLayer =
								allowedSourceLayers[piece.sourceLayerId];
							if (
								sourceLayer &&
					...
					let nextPiece: PieceResolved | undefined = _.find(
						filteredPieces,
						(piece) => {
							logger.info(
								'piece.resolvedStart',
				...
				if (nextPiece) {
					logger.info(
						(undo ? 'Disabling' : 'Enabling') +
							' next piece ' +
							nextPiece._id
					);
					Pieces.update(nextPiece._id, {
						$set: {
							disabled: !undo
						}
					});
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (!piece)
					throw new Meteor.Error(
						404,
						`Piece "${pieceId}" in rundown "${rundownId}" not found!`
					);

				const isPlaying: boolean = !!(
					piece.startedPlayback && !piece.stoppedPlayback
				);
				if (!isPlaying) {
					logger.info(
						`Playout reports piece "${pieceId}" has started playback on timestamp ${new Date(
							startedPlayback
						).toISOString()}`
					);
	...
	export function onPiecePlaybackStopped(
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (isPlaying) {
					logger.info(
						`Playout reports piece "${pieceId}" has stopped playback on timestamp ${new Date(
							stoppedPlayback
						).toISOString()}`
					);
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (playingPart) {
					...
					if (!isPlaying) {
						logger.info(
							`Playout reports part "${partId}" has started playback on timestamp ${new Date(
								startedPlayback
							).toISOString()}`
						);
						...
						if (rundown.currentPartId === partId) {
							...
							if (rundown.previousPartId) {
								const prevPart = Parts.findOne(
									rundown.previousPartId
								);

								if (!prevPart) {
									// We couldn't find the previous part: this is not a critical issue, but is clearly is a symptom of a larger issue
									logger.error(
										`Previous part "${rundown.previousPartId}" on rundown "${rundownId}" could not be found.`
									);
							...
							if (rundown.currentPartId) {
								...
								if (!currentPart) {
									// We couldn't find the previous part: this is not a critical issue, but is clearly is a symptom of a larger issue
									logger.error(
										`Previous part "${rundown.currentPartId}" on rundown "${rundownId}" could not be found.`
									);
							...
							let nextPart: Part | null =
								_.first(partsAfter) || null;
							...
							logger.error(
								`Part "${playingPart._id}" has started playback by the playout gateway, but has not been selected for playback!`
							);
						}
					...
					throw new Meteor.Error(
						404,
						`Part "${partId}" in rundown "${rundownId}" not found!`
					);
				}
			}
		);
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (part) {
					...
					const isPlaying =
						part.startedPlayback && !part.stoppedPlayback;
					if (isPlaying) {
						logger.info(
							`Playout reports part "${partId}" has stopped playback on timestamp ${new Date(
								stoppedPlayback
							).toISOString()}`
						);
	...
	export function stopAdLibPiece(
		rundownId: string,
		partId: string,
		pieceId: string
	) {
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (!rundown.active)
					throw new Meteor.Error(
						403,
						`Pieces can be only manipulated in an active rundown!`
					);
				if (!rundown.currentPartId)
					throw new Meteor.Error(
						400,
						`A part needs to be active to place a sticky item`
					);

				let showStyleBase = rundown.getShowStyleBase();

				const sourceLayer = showStyleBase.sourceLayers.find(
					(i) => i._id === sourceLayerId
				);
				if (!sourceLayer)
					throw new Meteor.Error(
						404,
						`Source layer "${sourceLayerId}" not found!`
					);
				...
				if (lastPieces.length > 0) {
					...
					if (!currentPart)
						throw new Meteor.Error(
							501,
							`Current Part "${rundown.currentPartId}" could not be found.`
						);

					const lastPiece = convertPieceToAdLibPiece(lastPieces[0]);
					const newAdLibPiece = convertAdLibToPiece(
						lastPiece,
						currentPart,
						false
					);
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (rundown.currentPartId !== part._id)
					throw new Meteor.Error(
						403,
						`Pieces can be only manipulated in a current part!`
					);
				if (!part.getLastStartedPlayback())
					throw new Meteor.Error(
						405,
						`Part "${partId}" has yet to start playback!`
					);
					...
					if (piece.sourceLayerId === sourceLayerId) {
						if (!piece.userDuration) {
							...
							if (
								...
								newExpectedDuration =
									now - piece.startedPlayback;
							}

							if (newExpectedDuration !== undefined) {
								console.log(
									`Cropping piece "${piece._id}" at ${newExpectedDuration}`
								);
	...
	export function rundownTogglePartArgument(
		...
		return rundownSyncFunction(
			...
			() => {
				...
				if (
					rundown.holdState === RundownHoldState.ACTIVE ||
					rundown.holdState === RundownHoldState.PENDING
				) {
					throw new Meteor.Error(
						403,
						`Part Arguments can not be toggled when hold is used!`
					);
				}
	...
	export function shouldUpdateStudioBaseline(studioId: string) {
		...
		if (activeRundowns.length === 0) {
			...
			const versionsContent =
				(markerObject.metadata || {}).versions || {};

			if (versionsContent.core !== PackageInfo.version)
				return 'coreVersion';

			if (versionsContent.studio !== (studio._rundownVersionHash || 0))
				return 'studio';

			if (versionsContent.blueprintId !== studio.blueprintId)
				return 'blueprintId';
			if (studio.blueprintId) {
				const blueprint = Blueprints.findOne(studio.blueprintId);
				if (!blueprint) return 'blueprintUnknown';
				if (
					versionsContent.blueprintVersion !==
	...
	if (currentPart) {
		...
		currentPieces.forEach((piece) => {
			if (
				...
				piece.playoutDuration === undefined &&
				piece.userDuration === undefined
			) {
...
function afterTake(
	...
	Meteor.setTimeout(() => {
		if (takePart.shouldNotifyCurrentPlayingPart) {
			IngestActions.notifyCurrentPlayingPart(
				rundownData.rundown,
				takePart
			);
		}
...
export function triggerUpdateTimelineAfterIngestData(
	rundownId: string,
	changedSegmentIds: Array<string>
) {
	// Lock behind a timeout, so it doesnt get executed loads when importing a rundown or there are large changes
	let data: UpdateTimelineFromIngestDataTimeout =
		updateTimelineFromIngestDataTimeouts[rundownId];
	if (data) {
