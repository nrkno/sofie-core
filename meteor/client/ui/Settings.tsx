...
const SettingsMenu = translateWithTracker<
	ISettingsMenuProps,
	ISettingsMenuState,
	ISettingsMenuTrackedProps
>(() => {
	...
	> {
		constructor(
			props: Translated<ISettingsMenuProps & ISettingsMenuTrackedProps>
		) {
			super(props);
			this.state = {};
		}
		...
		studioHasError(studio: Studio) {
			...
			const peripherals = this.props.peripheralDevices.filter(
				(device) => device.studioId === studio._id
			);
		...
		onDeleteStudio(studio: Studio) {
			...
			doModalDialog({
				...
				onAccept: () => {
					callMethod(
						'ModalDialog',
						StudiosAPI.methods.removeStudio,
						studio._id
					);
				}
			});
		...
		onDeleteShowStyleBase(item: ShowStyleBase) {
			...
			doModalDialog({
				...
				onAccept: () => {
					callMethod(
						'ModalDialog',
						ShowStylesAPI.methods.removeShowStyleBase,
						item._id
					);
				}
			});
			...
			doModalDialog({
				...
				message: (
					<React.Fragment>
						<p>
							{t(
								'Are you sure you want to delete the blueprint "{{blueprintId}}"?',
			...
			doModalDialog({
				...
				message: (
					<React.Fragment>
						<p>
							{t(
								'Are you sure you want to remove the device "{{deviceName}}" and all of it\'s sub-devices?',
			...
			return (
				<div className="tight-xs htight-xs text-s">
					...
					<h2 className="mhs">
						<button
							className="action-btn right"
							onClick={(e) => this.onAddShowStyleBase()}>
							<FontAwesomeIcon icon={faPlus} />
						</button>
						{t('Show Styles')}
					</h2>
						...
						<div className="col c12 rm-c9 settings-dialog">
							<ErrorBoundary>
								<Switch>
									...
									/>
									<Route
										path="/settings/showStyleBase/:showStyleBaseId"
										component={ShowStyleSettings}
									/>
									<Route
										path="/settings/peripheralDevice/:deviceId"
									...
									/>
									<Route
										path="/settings/tools/snapshots"
										component={SnapshotsView}
									/>
									<Route
										path="/settings/tools/migration"
										component={MigrationView}
									/>
									<Route
										path="/settings/tools/messages"
