...
class RundownViewNotifier extends WithManagedTracker {
	...
	private reactiveRundownStatus(rundownId: string | undefined) {
		...
		this.autorun(() => {
			...
			if (rundown) {
				...
				if (
					newNotification &&
					!Notification.isEqual(this._rundownStatus[unsyncedId], newNotification)
				) {
			...
			pieces.forEach((piece) => {
				...
				if (sourceLayer && part) {
					...
					this._mediaStatusComps[piece._id] = Tracker.autorun(() => {
						...
						if (
							newNotification &&
							!Notification.isEqual(this._mediaStatus[piece._id], newNotification)
						) {
	...
	private reactiveVersionStatus(rRundownId: string) {
		...
		this._rundownImportVersionInterval = rRundownId
			? Meteor.setInterval(() => this.updateVersionStatus(rRundownId), updatePeriod)
			: undefined;
	...
	private updateVersionStatus(rundownId: string) {
		...
		Meteor.call(
			...
			(err: Error, versionMismatch: string) => {
				...
				if (
					newNotification &&
					!Notification.isEqual(this._rundownImportVersionStatus, newNotification)
				) {
