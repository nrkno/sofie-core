...
async function customReformatFiles() {
	...
	for (const filename of files) {
		...
		// if (!filename.includes("database.ts")) continue;
		...
		const maybeReplaceCb = async (match) => {
			...
			try {
				if (
					(await await fsExists(orgTarget)) &&
					(await isDirectory(orgTarget)) &&
					!(await await fsExists(orgTarget + ".ts")) // in case there is a lib.ts and a lib directory
				) {
					// console.log("IS DIR", orgTarget);
					// is a directory, now check if there is an index file in it
					if (await fsExists(path.join(orgTarget), "index.ts")) {
						// console.log("orgTarget", filePath, match[1], orgTarget);
