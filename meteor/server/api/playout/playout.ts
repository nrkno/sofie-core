...
export namespace ServerPlayoutAPI {
	...
	export function resetRundown(rundownId: string) {
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (rundown.active && !rundown.rehearsal)
				throw new Meteor.Error(401, `rundownResetBroadcast can only be run in rehearsal!`);
	...
	export function resetAndActivateRundown(rundownId: string, rehearsal?: boolean) {
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (!rundown) throw new Meteor.Error(404, `Rundown "${rundownId}" not found!`);
			if (rundown.active && !rundown.rehearsal)
				throw new Meteor.Error(402, `rundownResetAndActivate cannot be run when active!`);
		...
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (blueprint.onPreTake) {
				try {
					waitForPromise(
						Promise.resolve(
							blueprint.onPreTake(new PartEventContext(rundown, undefined, takePart))
						).catch(logger.error)
					);
				} catch (e) {
					logger.error(e);
			...
			let m: Partial<Rundown> = {
				...
				holdState:
					!rundown.holdState || rundown.holdState === RundownHoldState.COMPLETE
			...
			Meteor.defer(() => {
				...
				if (blueprint.onPostTake) {
					waitForPromise(
						Promise.resolve(
							blueprint.onPostTake(new PartEventContext(rundown, undefined, takePart))
						).catch(logger.error)
					);
				}
			});
	...
	export function setNextPartInner(
		...
		if (rundown.holdState && rundown.holdState !== RundownHoldState.COMPLETE)
			throw new Meteor.Error(501, `Rundown "${rundown._id}" cannot change next during hold!`);
	...
	export function moveNextPart(
		...
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			return moveNextPartInner(rundownId, horisontalDelta, verticalDelta, setManually);
		});
	}
	function moveNextPartInner(
		...
		if (rundown.holdState && rundown.holdState !== RundownHoldState.COMPLETE)
			throw new Meteor.Error(501, `Rundown "${rundownId}" cannot change next during hold!`);
		...
		if (!currentNextSegment)
			throw new Meteor.Error(404, `Segment "${currentNextPart.segmentId}" not found!`);
		...
		if (verticalDelta !== 0) {
			...
			if (partIndex === -1)
				throw new Meteor.Error(404, `Part (from segment) not found in list of parts!`);
		}
		...
		if (!part)
			throw new Meteor.Error(501, `Part index ${partIndex} not found in list of parts!`);
	...
	export function activateHold(rundownId: string) {
		...
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (!rundown.currentPartId)
				throw new Meteor.Error(400, `Rundown "${rundownId}" no current part!`);
			if (!rundown.nextPartId)
	...
	export function deactivateHold(rundownId: string) {
		...
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (rundown.holdState !== RundownHoldState.PENDING)
				throw new Meteor.Error(400, `Rundown "${rundownId}" is not pending a hold!`);
	...
	export function disableNextPiece(rundownId: string, undo?: boolean) {
		...
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (!currentSement)
				throw new Meteor.Error(404, `Segment "${currentPart.segmentId}" not found!`);
			...
			let getNextPiece = (part: Part, undo?: boolean) => {
				...
				let nowInPart = 0;
				if (part.startedPlayback && part.timings && part.timings.startedPlayback) {
	...
	export function onPiecePlaybackStarted(
		...
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			const isPlaying: boolean = !!(piece.startedPlayback && !piece.stoppedPlayback);
			if (!isPlaying) {
				logger.info(
	...
	export function onPartPlaybackStarted(
		...
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (playingPart) {
				...
				const isPlaying = playingPart.startedPlayback && !playingPart.stoppedPlayback;
				if (!isPlaying) {
					logger.info(
	...
	export function sourceLayerStickyPieceStart(rundownId: string, sourceLayerId: string) {
		...
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (!sourceLayer)
				throw new Meteor.Error(404, `Source layer "${sourceLayerId}" not found!`);
	...
	export function sourceLayerOnPartStop(
		...
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (!part.getLastStartedPlayback())
				throw new Meteor.Error(405, `Part "${partId}" has yet to start playback!`);
	...
	export function shouldUpdateStudioBaseline(studioId: string) {
		...
		if (activeRundowns.length === 0) {
			...
			if (studio.blueprintId) {
				...
				if (versionsContent.blueprintVersion !== (blueprint.blueprintVersion || 0))
...
function beforeTake(rundownData: RundownData, currentPart: Part | null, nextPart: Part) {
	if (currentPart) {
		const adjacentPart = _.find(rundownData.parts, (part) => {
			return part.segmentId === currentPart.segmentId && part._rank > currentPart._rank;
		});
...
function afterTake(rundownData: RundownData, takePart: Part, timeOffset: number | null = null) {
