...
import {
	...
	PieceUi
} from './SegmentTimeline/SegmentTimelineContainer';
...
class KeyboardFocusMarker extends React.Component<
	IKeyboardFocusMarkerProps,
	IKeyboardFocusMarkerState
> {
	...
	render() {
		if (this.state.inFocus) {
			...
			return <div className="rundown-view__focus-lost-frame"></div>;
		}
	}
}
...
const WarningDisplay = translate()(
	timer(5000)(
		class extends React.Component<
			Translated<ITimingWarningProps>,
			ITimingWarningState
		> {
			...
			componentDidUpdate(prevProps: ITimingWarningProps) {
				...
				if (
					...
					getCurrentTime() + this.REHEARSAL_MARGIN >
						this.props.rundown.expectedStart &&
...
const TimingDisplay = translate()(
	withTiming<ITimingDisplayProps, {}>()(
		class extends React.Component<
			Translated<WithTiming<ITimingDisplayProps>>
		> {
			render() {
				const { t } = this.props;
						...
						!this.props.rundown.rehearsal ? (
							<span className="timing-clock plan-start left">
								...
								<Moment
									interval={0}
									format="HH:mm:ss"
									date={this.props.rundown.startedPlayback}
								/>
							</span>
						) : (
							<span className="timing-clock plan-start left">
								...
								<Moment
									interval={0}
									format="HH:mm:ss"
									date={this.props.rundown.expectedStart}
								/>
							</span>
						)}
						{this.props.rundown.startedPlayback &&
							...
							: (this.props.rundown.expectedStart && (
									<span
										className={ClassNames(
											'timing-clock countdown plan-start left',
											{
												heavy:
													getCurrentTime() >
													this.props.rundown
						...
						{this.props.rundown.currentPartId && (
							<span className="timing-clock current-remaining">
								...
								this.props.rundown.holdState !==
									...
									<div className="rundown__header-status rundown__header-status--hold">
										{t('Hold')}
									</div>
								) : null}
							</span>
						...
						{this.props.rundown.expectedDuration ? (
							<React.Fragment>
								...
								{this.props.rundown.expectedDuration && (
									<span
										className={ClassNames(
											'timing-clock heavy-light right',
											{
												heavy:
...
const RundownHeader = translate()(
	class extends React.Component<
		Translated<IRundownHeaderProps>,
		IRundownHeaderState
	> {
		bindKeys: Array<{
			key: string;
		...
		constructor(props: Translated<IRundownHeaderProps>) {
			...
			if (this.props.studioMode) {
				this.bindKeys = [
					...
					{
						key:
							RundownViewKbdShortcuts.RUNDOWN_DISABLE_NEXT_ELEMENT,
						up: this.keyDisableNextPiece,
						label: t('Disable the next element')
					},
					{
						key:
							RundownViewKbdShortcuts.RUNDOWN_UNDO_DISABLE_NEXT_ELEMENT,
		...
		componentDidMount() {
			...
			_.each(this.bindKeys, (k) => {
				const method = k.global
					? mousetrapHelper.bindGlobal
		...
		disableNextPiece = (e: any) => {
			...
			if (this.props.studioMode) {
				doUserAction(t, e, UserActionAPI.methods.disableNextPiece, [
		...
		take = (e: any) => {
			const { t } = this.props;
			if (this.props.studioMode) {
				doUserAction(t, e, UserActionAPI.methods.take, [
					this.props.rundown._id
				]);
			}
		};
			...
			if (this.props.studioMode) {
				if (this.props.rundown.active) {
					doUserAction(
						t,
						e,
						UserActionAPI.methods.moveNext,
						[this.props.rundown._id, horizonalDelta, verticalDelta],
						(err, response) => {
							if (!err && response) {
								const partId = response.result;
								if (partId)
									scrollToPart(partId).catch(
										() => console.error
									);
							}
						}
		...
		hold = (e: any) => {
			...
			if (this.props.studioMode && this.props.rundown.active) {
				doUserAction(t, e, UserActionAPI.methods.activateHold, [
					this.props.rundown._id,
					false
				]);
			}
		...
		holdUndo = (e: any) => {
			...
			if (
				this.props.studioMode &&
				this.props.rundown.active &&
				this.props.rundown.holdState === RundownHoldState.PENDING
			) {
				doUserAction(t, e, UserActionAPI.methods.activateHold, [
					this.props.rundown._id,
					true
		...
		rundownShouldHaveEnded() {
			return (
				getCurrentTime() >
				(this.props.rundown.expectedStart || 0) +
					(this.props.rundown.expectedDuration || 0)
			);
		}
			...
			function handleResult(err, response) {
				if (!err) {
					...
					doModalDialog({
						...
						message: t(
							'Something went wrong, please contact the system administrator if the problem persists.'
						),
						acceptOnly: true,
						warning: true,
						yes: t('OK'),
				...
				actions: [
					{
						...
						on: (e) => {
							doUserAction(
								t,
								e,
								UserActionAPI.methods.forceResetAndActivate,
								[rundownId, false],
								handleResult
							);
						}
					...
					doUserAction(
						...
						(err, response) => {
							if (!err) {
								...
								if (err.error === 409) {
									this.handleAnotherRundownActive(
										this.props.rundown._id,
										false,
										err,
										() => {
					...
					doModalDialog({
						...
						onAccept: (le: any) => {
							...
							doUserAction(
								t,
								e,
								UserActionAPI.methods.resetAndActivate,
								[this.props.rundown._id],
								(err, response) => {
									if (!err) {
										onSuccess();
				...
				if (!this.rundownShouldHaveStarted()) {
					...
					if (!this.props.rundown.active) {
						...
						doUserAction(
							t,
							e,
							UserActionAPI.methods.prepareForBroadcast,
							[this.props.rundown._id],
							(err, response) => {
								if (!err) {
									onSuccess();
						...
						doModalDialog({
							title: this.props.rundown.name,
							message: t(
								'Are you sure you want to activate Rehearsal Mode?'
							),
							onAccept: (e) => {
						...
						doModalDialog({
							...
							onAccept: () => {
								doUserAction(
									t,
									e,
									UserActionAPI.methods.deactivate,
									[this.props.rundown._id]
								);
							}
						});
		...
		resetRundown = (e: any) => {
			...
			let doReset = () => {
				...
				doUserAction(
					t,
					e,
					UserActionAPI.methods.resetRundown,
					[this.props.rundown._id],
					() => {
						this.deferFlushAndRewindSegments();
					}
			...
			if (this.props.rundown.active && !this.props.rundown.rehearsal) {
				...
				doModalDialog({
					title: this.props.rundown.name,
					message: t(
						'The rundown can not be reset while it is active'
					),
					onAccept: () => {
						// nothing
					},
		...
		reloadRundown = (e: any, changeRehearsal?: boolean) => {
			...
			if (this.props.studioMode) {
				doUserAction(
					t,
					e,
					UserActionAPI.methods.reloadData,
					[this.props.rundown._id, changeRehearsal],
					(err, response) => {
						if (!err && response) {
							if (
								!handleRundownReloadResponse(
									t,
									this.props.rundown,
									response.result
								)
		...
		takeRundownSnapshot = (e) => {
			...
			if (this.props.studioMode) {
				doUserAction(
					t,
					e,
					UserActionAPI.methods.storeRundownSnapshot,
					[this.props.rundown._id, 'Taken by user'],
					undefined,
					t(
						'A snapshot of the current Running\xa0Order has been created for troubleshooting.'
					)
				);
			}
		};
				...
				<React.Fragment>
						...
						<ContextMenu id="rundown-context-menu">
							...
							{this.props.studioMode ? (
								<React.Fragment>
									{!(
										...
										!this.props.rundown.active ? (
											<MenuItem
												...
												{t(
													'Prepare Studio and Activate (Rehearsal)'
												)}
											</MenuItem>
										) : (
											<MenuItem
									...
									<MenuItem
										onClick={(e) => this.reloadRundown(e)}>
										{t('Reload ENPS Data')}
									</MenuItem>
									<MenuItem
										onClick={(e) =>
					...
					<div
						...
						<ContextMenuTrigger
							...
							attributes={{
								className:
									'flex-col col-timing horizontal-align-center'
							}}>
							<WarningDisplay
							...
							<div className="row first-row super-dark">
								<div className="flex-col left horizontal-align-left">
									<div className="badge mod">
										<Tooltip
											overlay={t(
												'Add ?studio=1 to the URL to enter studio mode'
											)}
											visible={
												getHelpMode() &&
												!getAllowStudio()
...
export const RundownView = translateWithTracker<IProps, IState, ITrackedProps>(
	(props: IProps, state) => {
		let rundownId;
		...
		return {
			...
			studio: studio,
			showStyleBase:
				rundown && ShowStyleBases.findOne(rundown.showStyleBaseId),
			rundownLayouts:
				rundown &&
				RundownLayouts.find({
	...
	class RundownView extends MeteorReactComponent<
	...
	> {
		...
		static getDerivedStateFromProps(
			props: Translated<IProps & ITrackedProps>,
			state: IState
		) {
			...
			if (props.rundownLayouts) {
				...
				if (props.rundownLayoutId && !selectedLayout) {
					selectedLayout = props.rundownLayouts.find(
						(i) => i.name.indexOf(props.rundownLayoutId!) >= 0
					);
				}
				...
				if (!selectedLayout) {
					selectedLayout = props.rundownLayouts.find(
						(i) => i.type === RundownLayoutType.RUNDOWN_LAYOUT
					);
				}
		...
		componentDidMount() {
			...
			window.addEventListener(
				RundownViewEvents.goToLiveSegment,
		...
		componentDidUpdate(
			prevProps: IProps & ITrackedProps,
			prevState: IState
		) {
			if (
				this.props.rundown &&
				prevProps.rundown &&
				prevProps.rundown.currentPartId !==
					this.props.rundown.currentPartId &&
			...
			if (
				typeof this.props.rundown !== typeof this.props.rundown ||
				(this.props.rundown || { _id: '' })._id !==
					(prevProps.rundown || { _id: '' })._id ||
				(this.props.rundown || { active: false }).active !==
					(prevProps.rundown || { active: false }).active ||
			...
			if (
				typeof this.props.showStyleBase !==
					typeof prevProps.showStyleBase ||
				(this.props.showStyleBase &&
					this.props.showStyleBase.runtimeArguments)
			) {
				this.refreshHotkeys();
			}
		}
					...
					const handler = (e: KeyboardEvent) => {
						if (
							...
							doUserAction(
								t,
								e,
								UserActionAPI.methods.togglePartArgument,
								[
									this.props.rundown._id,
									this.props.rundown.nextPartId,
									i.property,
		...
		onSelectPiece = (
			...
			if (
				piece &&
				piece.content &&
				(piece.content as VTContent).editable &&
				(((piece.content as VTContent).editable as VTEditableParameters)
					.editorialDuration !== undefined ||
					((piece.content as VTContent)
						.editable as VTEditableParameters).editorialStart !==
						undefined)
			) {
				this.setState({
		...
		onBeforeUnload = (e: any) => {
			...
			e.preventDefault();
			e.returnValue = t(
				'This rundown is now active. Are you sure you want to exit this screen?'
			);
		...
		onWheel = (e: React.WheelEvent<HTMLDivElement>) => {
			if (
				...
				this.onTimeScaleChange(
					Math.min(
						500,
						this.state.timeScale * (1 + 0.001 * (e.deltaY * -1))
					)
				);
				e.preventDefault();
			}
		...
		onGoToLiveSegment = () => {
			if (
				...
				scrollToPart(this.props.rundown.nextPartId, true)
					.then(() => {
				...
				setTimeout(() => {
					...
					window.dispatchEvent(
						new Event(RundownViewEvents.rewindsegments)
					);
				}, 2000);
			} else if (
				this.props.rundown &&
				...
				scrollToPart(this.props.rundown.currentPartId, true)
					.then(() => {
						// allow for the scroll to finish
					})
					.catch((e) => {
		...
		onSetNext = (part: Part, e: any, offset?: number, take?: boolean) => {
			...
			if (
				...
				doUserAction(
					...
					[this.props.rundown._id, part._id, offset],
					(err, res) => {
						this.setState({
		...
		onPieceDoubleClick = (
			...
			if (
				...
				doUserAction(t, e, UserActionAPI.methods.pieceTakeNow, [
					this.props.rundown._id,
					this.props.rundown.currentPartId,
					item._id
				]);
			...
			if (this.props.segments) {
				return this.props.segments.map((segment, index, array) => {
					if (
						...
						return (
							<ErrorBoundary key={segment._id}>
								<VirtualElement
									id={
										SEGMENT_TIMELINE_ELEMENT_ID +
										segment._id
									}
									margin={'100% 0px 100% 0px'}
									initialShow={
										index < window.innerHeight / 260
									}
									...
									<SegmentTimelineContainer
										...
										onPieceDoubleClick={
											this.onPieceDoubleClick
										}
										onHeaderNoteClick={(level) =>
											this.onHeaderNoteClick(
												segment._id,
												level
											)
										}
		...
		onToggleHotkeys = () => {
			if (!this.state.isInspectorShelfExpanded) {
				...
				if (this._inspectorShelf) {
					this._inspectorShelf
						.getWrappedInstance()
						.switchTab(ShelfTabs.SYSTEM_HOTKEYS);
				}
			} else {
				this.setState({
					isInspectorShelfExpanded: false
						...
						.then(() => {
							NotificationCenter.push(
								new Notification(
									...
									t(
										'Playout\xa0Gateway "{{playoutDeviceName}}" is now restarting.',
						...
						.catch(() => {
							NotificationCenter.push(
								new Notification(
									...
									NoticeLevel.CRITICAL,
									t(
										'Could not restart Playout\xa0Gateway "{{playoutDeviceName}}".',
										{ playoutDeviceName: item.name }
									),
									'RundownView'
								)
		...
		onRestartCasparCG = (device: PeripheralDevice) => {
			...
			doModalDialog({
				...
				message: t(
					'Do you want to restart CasparCG Server "{{device}}"?',
					{ device: device.name }
				),
				onAccept: (event: any) => {
					callPeripheralDeviceFunction(
						...
						<RundownTimingProvider
							...
							<div
								className={ClassNames('rundown-view', {
									'notification-center-open': this.state
										.isNotificationsCenterOpen,
									'rundown-view--studio-mode': this.state
										.studioMode
								})}
								style={this.getStyle()}
										...
										{this.state.isSupportPanelOpen && (
											<SupportPopUp>
												...
												{this.state.studioMode &&
													this.props
														.casparCGPlayoutDevices &&
													this.props.casparCGPlayoutDevices.map(
														(i) => (
															<button
																className="btn btn-primary"
																onClick={() =>
						...
						<div className="rundown-view rundown-view--unpublished">
							<div className="rundown-view__label">
								<p>
									{!this.props.rundown
										? t(
												'This rundown has been unpublished from Sofie.'
										  )
										: !this.props.studio
										? t(
												'Error: The studio of this Rundown was not found.'
										  )
										: !this.props.showStyleBase
										? t(
												'Error: The ShowStyle of this Rundown was not found.'
										  )
										: t('Unknown error')}
								</p>
								<p>
...
export function handleRundownReloadResponse(
	...
	rundown: Rundown,
	result: UserActionAPI.ReloadRundownResponse
): boolean {
	...
	if (result === UserActionAPI.ReloadRundownResponse.MISSING) {
		...
		const notification = NotificationCenter.push(
			new Notification(
				...
				NoticeLevel.CRITICAL,
				t(
					'Rundown {{rundownName}} is missing, what do you want to do?',
					...
					// actions:
					{
						...
						action: () => {
							doUserAction(
								...
								UserActionAPI.methods.unsyncRundown,
								[rundown._id],
								(err) => {
									if (!err) {
										notification.stop();
					...
					},
					{
						...
						action: () => {
							doModalDialog({
								title: rundown.name,
								message: t(
									'Do you really want to remove the rundown "{{rundownName}}"? This cannot be undone!',
									{ rundownName: rundown.name }
								),
								onAccept: () => {
									// nothing
									doUserAction(
										t,
										'Missing rundown action',
										UserActionAPI.methods.removeRundown,
										[rundown._id],
										(err) => {
											if (!err) {
												notification.stop();
