...
export class ConnectionStatusNotifier extends WithManagedTracker {
		...
		this.autorun(() => {
			...
			if (newNotification.persistent) {
				this._notificationList.set(
					_.compact([newNotification, systemNotification])
				);
			} else {
	...
	private getStatusText(
		...
		switch (status) {
			case 'connecting':
				return (
					<span>
						{t('Connecting to the')} {t('Sofie Automation Server')}.
					</span>
				);
			case 'failed':
				return (
			...
			case 'offline':
				return (
					<span>
						{t('Your machine is offline and cannot connect to the')}{' '}
			...
			case 'connected':
				return (
					<span>
						{t('Connected to the')} {t('Sofie Automation Server')}.
					</span>
				);
		}
		return null;
	...
	private createNewStatusNotification(
	...
	private updateServiceMessages(serviceMessages: {
		...
		systemMessageIds
			...
			.forEach((id) => {
				...
				const notification = this.createNotificationFromServiceMessage(
					newMessage
				);
	...
	private createNotificationFromServiceMessage(
...
export const ConnectionStatusNotification = translate()(
	class extends React.Component<Translated<IProps>, IState> {
