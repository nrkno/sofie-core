...
function createRundownSnapshot(rundownId: string): RundownSnapshot {
	...
	logger.info(
		`Generating Rundown snapshot "${snapshotId}" for rundown "${rundownId}"`
	);

	const rundown = Rundowns.findOne(rundownId);
	if (!rundown) throw new Meteor.Error(404, `Rundown ${rundownId} not found`);
	const ingestData = IngestDataCache.find(
		{ rundownId: rundownId },
		{ sort: { modified: -1 } }
	).fetch(); // @todo: check sorting order
	const userActions = UserActionsLog.find({
		args: { $regex: `.*"${rundownId}".*` }
	}).fetch();
	...
	const expectedMediaItems = ExpectedMediaItems.find({
		partId: { $in: parts.map((i) => i._id) }
	}).fetch();
	const baselineObjs = RundownBaselineObjs.find({
		rundownId: rundownId
	}).fetch();
	...
	return {
		...
		snapshot: {
			...
			name: `Rundown_${rundown.name}_${rundown._id}_${formatDateTime(
				getCurrentTime()
			)}`,
			version: CURRENT_SYSTEM_VERSION
		},
...
function createSystemSnapshot(studioId: string | null): SystemSnapshot {
	let snapshotId = Random.id();
	logger.info(
		`Generating System snapshot "${snapshotId}"` +
			(studioId ? `for studio "${studioId}"` : '')
	);
	...
	return {
		...
		snapshot: {
			...
			name:
				`System` +
				(studioId ? `_${studioId}` : '') +
				`_${formatDateTime(getCurrentTime())}`,
			version: CURRENT_SYSTEM_VERSION
		},
		studios,
		showStyleBases,
...
function createDebugSnapshot(studioId: string): DebugSnapshot {
	let snapshotId = Random.id();
	logger.info(
		`Generating Debug snapshot "${snapshotId}" for studio "${studioId}"`
	);
	...
	_.each(systemSnapshot.devices, (device) => {
		if (
			...
			let deviceSnapshot = ServerPeripheralDeviceAPI.executeFunction(
				device._id,
				'getSnapshot'
			);
...
function handleResponse(
	...
	snapshotFcn: () => { snapshot: SnapshotBase }
) {
	try {
...
function storeSnaphot(
	...
	if (!system.storePath)
		throw new Meteor.Error(500, `CoreSystem.storePath not set!`);
...
function restoreFromSnapshot(snapshot: AnySnapshot) {
	...
	if (!snapshot.snapshot)
		throw new Meteor.Error(500, `Restore input data is not a snapshot`);

	if (snapshot.snapshot.type === SnapshotType.RUNDOWN) {
		// A snapshot of a rundown
		...
		throw new Meteor.Error(
			402,
			`Unknown snapshot type "${snapshot.snapshot.type}"`
		);
	}
}
...
function restoreFromRundownSnapshot(snapshot: RundownSnapshot) {
	...
	if (rundownId !== snapshot.rundown._id)
		throw new Meteor.Error(
			500,
			`Restore snapshot: rundownIds don\'t match, "${rundownId}", "${snapshot.rundown._id}!"`
		);
	...
	snapshot.rundown.notifiedCurrentPlayingPartExternalId = dbRundown
		? dbRundown.notifiedCurrentPlayingPartExternalId
		: undefined;
	...
	saveIntoDb(
		ExpectedMediaItems,
		{ partId: { $in: snapshot.parts.map((i) => i._id) } },
		snapshot.expectedMediaItems
	);
...
function restoreFromSystemSnapshot(snapshot: SystemSnapshot) {
	...
	let changes = sumChanges(
		...
		snapshot.blueprints
			? saveIntoDb(Blueprints, {}, snapshot.blueprints)
			: null,
		snapshot.rundownLayouts
			? saveIntoDb(RundownLayouts, {}, snapshot.rundownLayouts)
			: null,
		saveIntoDb(
			PeripheralDevices,
...
Picker.route(
	'/snapshot/system/:studioId',
	(params, req: IncomingMessage, response: ServerResponse, next) => {
...
// Setup methods:
let methods: Methods = {};
methods[SnapshotFunctionsAPI.STORE_SYSTEM_SNAPSHOT] = (
	studioId: string | null,
	reason: string
) => {
	return storeSystemSnapshot(studioId, reason);
};
methods[SnapshotFunctionsAPI.STORE_RUNDOWN_SNAPSHOT] = (
	rundownId: string,
	reason: string
) => {
	return storeRundownSnapshot(rundownId, reason);
};
methods[SnapshotFunctionsAPI.STORE_DEBUG_SNAPSHOT] = (
	studioId: string,
	reason: string
) => {
