...
export function activateRundown(rundown: Rundown, rehearsal: boolean) {
	logger.info(
		'Activating rundown ' + rundown._id + (rehearsal ? ' (Rehearsal)' : '')
	);
	...
	if (!newRundown)
		throw new Meteor.Error(404, `Rundown "${rundown._id}" not found!`);
	rundown = newRundown;
	...
	Meteor.defer(() => {
		...
		if (blueprint.onRundownActivate) {
			Promise.resolve(
				blueprint.onRundownActivate(new RundownContext(rundown, studio))
			).catch(logger.error);
		}
	});
}
export function deactivateRundown(rundown: Rundown) {
	...
	Meteor.defer(() => {
		...
		if (blueprint.onRundownDeActivate) {
			Promise.resolve(
				blueprint.onRundownDeActivate(new RundownContext(rundown))
			).catch(logger.error);
		}
	});
}
