...
export const HttpSendDeviceSettingsComponent = translate()(
	class HttpSendDeviceSettingsComponent extends React.Component<
		Translated<IHttpSendDeviceSettingsComponentProps>,
		IHttpSendDeviceSettingsComponentState
	> {
		...
		handleConfirmRemoveAccept = (e) => {
			this.state.deleteConfirmMakeReadyId &&
				this.removeMakeReady(this.state.deleteConfirmMakeReadyId);
			this.setState({
				showDeleteConfirmMakeReady: false,
				deleteConfirmMakeReadyId: undefined
			});
		...
		removeMakeReady = (rowId: string) => {
			// TODO this
			let unsetObject = {};
			unsetObject['settings.devices.' + this.props.deviceId + '.options.makeReadyCommands'] = {
				id: rowId
			};
				...
				return (
					<React.Fragment key={i}>
						<tr
							...
							<th className="settings-studio-device-httpsend__url c5">{cmd.url}</th>
							<td className="settings-studio-device-httpsend__type c4">{cmd.type}</td>
			...
			return (
				<React.Fragment>
					...
					<table className="expando settings-studio-device-httpsend-table">
						<tbody>{this.renderHttpSendCommands()}</tbody>
					</table>

					<ModalDialog
						title={t('Remove this command?')}
						acceptText={t('Remove')}
