import {
	postHandler,
	BodyParsingIncomingMessage
} from '../../../api/serviceMessages/postHandler';
import {
	Criticality,
	ServiceMessage
...
describe('ServiceMessages API POST endpoint', () => {
	...
	describe('data storage', () => {
		it('should call API writeMessage with the given id', () => {
			...
			expect(mockedWriteMessage.mock.calls[0][0]).toHaveProperty(
				'id',
				expected
			);
		});
	...
	describe('http response', () => {
		it('should reply 201 Created for new messages', () => {
			const spy = jest
				.spyOn(serviceMessagesApi, 'writeMessage')
				.mockImplementation(() => ({
					isUpdate: false
				}));
			mockRequest.body = JSON.parse(JSON.stringify(validInput));
