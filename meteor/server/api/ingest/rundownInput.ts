...
import { selectShowStyleVariant, afterRemoveSegments, afterRemoveParts, ServerRundownAPI, removeSegments, updatePartRanks, produceRundownPlaylistInfo, unsyncRundownInner } from '../rundown'
...
import { getRundownId, getSegmentId, getPartId, getStudioFromDevice, getRundown, canBeUpdated, getRundownPlaylist, getSegment, checkAccessAndGetPeripheralDevice } from './lib'
	...
	rundownPlaylistSyncFunction(rundownPlaylistId, RundownSyncFunctionPriority.INGEST, () => {
		...
		if (canBeUpdated(rundown)) {
			...
			if (okToRemove) {
				...
				unsyncRundownInner(cache, rundown._id)
			}
		 } else {
