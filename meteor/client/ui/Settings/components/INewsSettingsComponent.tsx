...
import { PeripheralDevice, PeripheralDevices, INewsDevice } from '../../../../lib/collections/PeripheralDevices'
...
export const INewsSettingsComponent = translate()(
	class INewsSettingsComponent extends React.Component<
		Translated<IINewsSettingsComponentProps>,
		IINewsSettingsComponentState
	> {
		...
		renderHosts() {
			...
			return (settings.hosts || []).map((item, index) => {
				return (
					<React.Fragment key={item._id}>
						<tr
							...
							<td className="settings-studio-custom-config-table__value c3">{item.host}</td>
		...
		renderQueues() {
			const { t } = this.props
			let settings = (this.props.device.settings || {}) as INewsDeviceSettings
			return (settings.queues || []).map((item, index) => {
				return (
					<React.Fragment key={item._id}>
						<tr
							className={ClassNames({
							...
							<td className="settings-studio-custom-config-table__value c3">{item.queue}</td>
							<td className="settings-studio-custom-config-table__actions table-item-actions c3">
								<button className="action-btn" onClick={(e) => this.editItem(item)}>
									<FontAwesomeIcon icon={faPencilAlt} />
								</button>
