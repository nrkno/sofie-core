...
> {
	...
	confirmReSyncRO(rundown: Rundown) {
		...
		doModalDialog({
			...
			message:
				t('Are you sure you want to re-sync the "{{name}}" rundown with MOS script?', {
					name: rundown.name
				}) +
				'\n' +
				t('Please note: This action is irreversible!')
		...
		return (
			<React.Fragment>
				<tr className="rundown-list-item">
					...
					<td className="rundown-list-item__status">{this.props.rundown.status}</td>
					<td className="rundown-list-item__air-status">{this.props.rundown.airStatus}</td>
		...
		componentDidMount() {
			...
			this.autorun(() => {
				...
				const showStyleVariantIds = _.uniq(
					_.map(Rundowns.find().fetch(), (rundown) => rundown.showStyleVariantId)
				);
						...
						{this.state.subsReady ? (
							<div className="mod mvl">
								<table className="table system-status-table expando expando-tight">
									...
									{unsynced.length > 0 && <tbody>{this.renderRundowns(unsynced)}</tbody>}
								</table>
							</div>
						) : (
							<Spinner />
