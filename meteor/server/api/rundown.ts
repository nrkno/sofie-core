...
export function selectShowStyleVariant(
	...
	if (!studio.supportedShowStyleBase.length) {
		logger.debug(
			`Studio "${studio._id}" does not have any supportedShowStyleBase`
		);
		return null;
	}
	...
	if (!studioBlueprint)
		throw new Meteor.Error(
			500,
			`Studio "${studio._id}" does not have a blueprint`
		);

	if (!studioBlueprint.blueprint.getShowStyleId)
		throw new Meteor.Error(
			500,
			`Studio "${studio._id}" blueprint missing property getShowStyleId`
		);

	const showStyleId = studioBlueprint.blueprint.getShowStyleId(
		context,
		showStyleBases,
		ingestRundown
	);
	if (showStyleId === null) {
		logger.debug(
			`StudioBlueprint for studio "${studio._id}" returned showStyleId = null`
		);
		return null;
	}
	...
	if (!showStyleVariants.length)
		throw new Meteor.Error(
			500,
			`ShowStyleBase "${showStyleBase._id}" has no variants`
		);
	...
	if (!showStyleBlueprint)
		throw new Meteor.Error(
			500,
			`ShowStyleBase "${showStyleBase._id}" does not have a valid blueprint`
		);

	const variantId = showStyleBlueprint.blueprint.getShowStyleVariantId(
		context,
		showStyleVariants,
		ingestRundown
	);
	if (variantId === null) {
		logger.debug(
			`StudioBlueprint for studio "${studio._id}" returned variantId = null in .getShowStyleVariantId`
		);
		return null;
	} else {
		const showStyleVariant = _.find(
			showStyleVariants,
			(s) => s._id === variantId
		);
		if (!showStyleVariant)
			throw new Meteor.Error(
				404,
				`Blueprint returned variantId "${variantId}", which was not found!`
			);
...
export function updatePartRanks(rundownId: string): Array<Part> {
	const allSegments = Segments.find(
		{ rundownId: rundownId },
		{ sort: { _rank: 1 } }
	).fetch();
	...
	logger.debug(
		`updatePartRanks (${allParts.length} parts, ${allSegments.length} segments)`
	);
	...
	_.each(allParts, (part) => {
		if (part.afterPart) {
			if (!partsToPutAfter[part.afterPart])
				partsToPutAfter[part.afterPart] = [];
				...
				if (insertI !== -1) {
					_.each(dynamicParts, (dynamicPart, i) => {
						...
						if (dynamicPart._rank !== newRank) {
							...
							ps.push(
								asyncCollectionUpdate(Parts, dynamicPart._id, {
									$set: { _rank: dynamicPart._rank }
								})
							);
						}
...
export namespace ServerRundownAPI {
	export function removeRundown(rundownId: string) {
		...
		if (rundown.active)
			throw new Meteor.Error(
				400,
				`Not allowed to remove an active Rundown "${rundownId}".`
			);

		rundown.remove();
	}
	export function resyncRundown(
		rundownId: string
...
export namespace ClientRundownAPI {
	export function rundownNeedsUpdating(rundownId: string) {
		...
		if (rundown.importVersions.core !== PackageInfo.version)
			return 'coreVersion';
		...
		if (
			rundown.importVersions.showStyleVariant !==
			(showStyleVariant._rundownVersionHash || 0)
		)
		...
		if (
			rundown.importVersions.showStyleBase !==
			(showStyleBase._rundownVersionHash || 0)
		)
		...
		if (rundown.importVersions.studio !== (studio._rundownVersionHash || 0))
			return 'studio';
