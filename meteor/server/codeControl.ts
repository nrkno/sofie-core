	...
	return Meteor.wrapAsync((...args0: any[]) => {
		...
		if (!_.isFunction(cb)) {
			...
			throw new Meteor.Error(
				500,
				'Callback is not a function, it is a ' + typeof cb
			);
		}
...
function evaluateFunctions() {
	...
	_.each(groups, (group, id) => {
		const runningFcn = _.find(
			group,
			(fcn) => fcn.status === syncFunctionFcnStatus.RUNNING
		);
		let startNext = false;
		if (runningFcn) {
			...
			if (Date.now() - startTime > runningFcn.timeout) {
				...
				logger.error(
					`syncFunction "${runningFcn.name}" took too long to evaluate`
				);
			...
			if (_.isObject(nextFcn)) {
				...
				Meteor.setTimeout(() => {
					if (nextFcn.status === syncFunctionFcnStatus.RUNNING) {
						logger.error(
							`syncFunction "${nextFcn.name}" took too long to evaluate`
						);
	...
	return (...args) => {
		...
		if (isFunctionQueued(id)) {
			...
			logger.debug(
				'Function ' +
					(name || 'Anonymous') +
					' is already queued to execute, ignoring call.'
			);
		} else {
			syncFcn(...args);
		}
