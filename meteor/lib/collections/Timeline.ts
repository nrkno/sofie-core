...
export function getTimelineId(
	objOrStudioId: TimelineObjGeneric | string,
	id?: string
): string {
	if (typeof objOrStudioId === 'string') {
		if (!objOrStudioId)
			throw new Meteor.Error(500, `Parameter studioId missing`);
		...
		if (!obj.studioId) {
			...
			throw new Meteor.Error(
				500,
				`TimelineObj missing studioId attribute, id: "${obj.id}")`
			);
		}
		return obj.studioId + '_' + obj.id;
	}
}
export function setTimelineId<T extends TimelineObjGeneric>(
	objs: Array<T>
): Array<T> {
