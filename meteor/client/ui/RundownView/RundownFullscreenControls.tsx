...
export class RundownFullscreenControls extends React.Component<IProps, IState> {
	...
	constructor(props) {
		...
		this.throttledRefreshFullScreenState = _.throttle(
		...
		return (
			<div className="status-bar">
				<VelocityReact.VelocityTransitionGroup
					...
					{!this.props.isFollowingOnAir && (
						<button
							...
							{this.state.onAirHover ? (
								<Lottie
									options={this.onAirOver}
									isStopped={false}
									isPaused={false}
								/>
							) : (
								<Lottie
									options={this.onAirOut}
									isStopped={false}
									isPaused={false}
								/>
							)}
					...
					<SupportPopUpToggle
						onClick={this.props.onToggleSupportPanel}
						isOpen={this.props.isSupportPanelOpen}
					/>
				</VelocityReact.VelocityTransitionGroup>
			</div>
		);
