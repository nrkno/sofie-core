...
describe('Test blueprint config', () => {
	...
	test('getStudioConfigRef', () => {
		expect(ConfigRef.getStudioConfigRef('st0', 'key0')).toEqual(
			'${studio.st0.key0}'
		);
	});
	test('getShowStyleConfigRef', () => {
		expect(ConfigRef.getShowStyleConfigRef('var0', 'key1')).toEqual(
			'${showStyle.var0.key1}'
		);
	});
	...
	describe('retrieveRefs', () => {
		...
		test('bad format', () => {
			expect(ConfigRef.retrieveRefs('$(studio.one.two)')).toEqual(
				'$(studio.one.two)'
			);
		...
		test('undefined - normal', () => {
			...
			expect(
				ConfigRef.retrieveRefs('${studio.one.two}_extra', modifier)
			).toEqual('undefined_extra');
			expect(
				ConfigRef.retrieveRefs('${showStyle.one.two}_extra', modifier)
			).toEqual('undefined_extra');
		...
		test('studio with data', () => {
			...
			expect(
				ConfigRef.retrieveRefs(
					`\${studio.${studio._id}.name} = \${studio.${studio._id}.obj}`
				)
		...
		test('showstyle with data', () => {
			...
			expect(
				ConfigRef.retrieveRefs(
					`\${showStyle.${variant._id}.name} = \${showStyle.${variant._id}.obj}`
				)
			).toEqual('undefined = [object Object]'); // Not pretty, but expected
		});
	});
