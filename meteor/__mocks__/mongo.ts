	...
	export class Collection<T extends CollectionObject>
		...
		find(query: any, options?: FindOptions) {
			...
			if (unimplementedUsedOptions.length > 0) {
				throw new Error(
					`find being performed using unimplemented options: ${unimplementedUsedOptions}`
				);
			}
						...
						stop() {
							...
							if (index === -1)
								throw new Meteor.Error(
									500,
									'Cannot stop observer that is not registered'
								);
		...
		private get documents(): MockCollection<T> {
			if (!mockCollections[this.localName])
				mockCollections[this.localName] = {};
