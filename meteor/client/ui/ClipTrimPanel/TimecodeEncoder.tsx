...
export class TimecodeEncoder extends React.Component<IProps, IState> {
	constructor(props: IProps) {
		...
		this.state = {
			currentValue: TimecodeEncoder.secondsToTimecode(
				props.value || 0,
				props.fps
			),
			validValue: TimecodeEncoder.secondsToTimecode(
				props.value || 0,
				props.fps
			),
			hasError: false,
	...
	static getDerivedStateFromProps(
		...
		const newValid = TimecodeEncoder.secondsToTimecode(
			props.value || 0,
			props.fps
		);
		return _.extend(
	...
	private static secondsToTimecode(time: number, fps: number): string {
		return Timecode.init({
			...
			timecode: time,
			drop_frame: !Number.isInteger(fps)
		}).toString();
	}
