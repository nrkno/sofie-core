...
export function getOrderedPiece(part: Part): Array<PieceResolved> {
	...
	_.each(tlResolved.objects, (obj0) => {
		...
		if (
			obj0.resolved.resolved &&
			obj0.resolved.instances &&
			obj0.resolved.instances.length > 0
		) {
	...
	if (unresolvedCount > 0) {
		logger.error(
			`Got ${unresolvedCount} unresolved timeline-objects for part #${
				part._id
			} (${unresolvedIds.join(', ')})`
		);
	}
	if (pieces.length !== resolvedPieces.length) {
		logger.error(
			`Got ${resolvedPieces.length} ordered pieces. Expected ${pieces.length} for part #${part._id}`
		);
	}
...
export function getResolvedPieces(part: Part): Piece[] {
	...
	if (pieces.length !== events.length) {
		logger.warn(
			`Got ${events.length} ordered pieces. Expected ${pieces.length}. for piece #${part._id}`
		);
	}
			...
			for (let i = index + 1; i < source.length; i++) {
				...
				if (piece.sourceLayerId === sourcePiece.sourceLayerId) {
					...
					piece.playoutDuration =
						(sourcePiece.enable.start as number) -
						(piece.enable.start as number);
					return;
				}
			}
		}
...
export function getResolvedPiecesFromFullTimeline(
...
): { pieces: Piece[]; time: number } {
	const objs = clone(
		allObjs.filter(
			(o) =>
				o.isGroup &&
				((o as any).isPartGroup || (o.metadata && o.metadata.pieceId))
		)
	);
	...
	const partIds = _.compact([
		rundownData.rundown.previousPartId,
		rundownData.rundown.currentPartId
	]);
	const pieces: Piece[] = rundownData.pieces.filter(
		(p) => partIds.indexOf(p.partId) !== -1
	);
	...
	if (rundownData.rundown.currentPartId && rundownData.rundown.nextPartId) {
		...
		if (part && part.autoNext) {
			pieces.push(
				...rundownData.pieces.filter(
					(p) => p.partId === rundownData.rundown.nextPartId
				)
			);
		}
	}
	...
	if (tlResolved.statistics.unresolvedCount > 0) {
		logger.warn(
			`Got ${
				tlResolved.statistics.unresolvedCount
			} unresolved pieces (${unresolvedIds.join(', ')})`
		);
	}
	if (pieces.length !== events.length) {
		logger.warn(
			`Got ${events.length} ordered pieces. Expected ${pieces.length}.`
		);
	}
			...
			for (let i = index + 1; i < source.length; i++) {
				...
				if (piece.sourceLayerId === sourcePiece.sourceLayerId) {
					...
					if (piece.playoutDuration) {
						piece.playoutDuration = Math.min(
							piece.playoutDuration,
							infDuration
						);
					} else {
	...
	const newAdLibPiece = literal<AdLibPiece>({
		...
		infiniteMode:
			piece.originalInfiniteMode !== undefined
				...
				: piece.infiniteMode,
		expectedDuration: _.isNumber(piece.enable.duration)
...
export function convertAdLibToPiece(
	adLibPiece: AdLibPiece | Piece,
	part: Part,
	queue: boolean
): Piece {
	...
	const newPiece = literal<Piece>({
		..._.omit(
			...
			'startedPlayback',
			'stoppedPlayback'
		), // TODO - this could be typed stronger
		_id: newId,
		enable: {
			start: queue ? 0 : 'now',
			duration: duration
...
export function resolveActivePieces(part: Part, now: number): Piece[] {
	...
	const partStartTime = part.startedPlayback
