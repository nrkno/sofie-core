...
export class KeyboardController extends ControllerAbstract {
			...
			if (scrollPosition !== undefined) {
				if (
					...
					this._targetPosition =
						this._prompterView.findAnchorPosition(
							newPosition,
							scrollPosition - 10,
							-1
						) || newPosition;
		...
		if (this._mode === Mode.NORMAL) {
			...
			if (scrollPosition !== undefined) {
				if (
					...
					if (setNewPosition) {
						let stopAcceleration =
							Math.sign(this._currentSpeed) * this._acceleration;
						const d = this._getDistanceToStop(
							this._currentSpeed,
							stopAcceleration
						);
	...
	private _getDistanceToStop(currentSpeed, stopAcceleration): number {
		...
		return (
			(stopAcceleration * Math.pow(timeToStop, 2)) / 2 +
			currentSpeed * timeToStop
		);
	}
	private _getAccelerationToStopInTime(
		currentSpeed,
		normalStopAcceleration,
		distanceLeft
	): number {
