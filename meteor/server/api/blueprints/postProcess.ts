...
export function postProcessPieces(
	...
	return _.map(_.compact(pieces), (itemOrig: IBlueprintPiece) => {
		...
		if (!piece._id)
			piece._id = innerContext.getHashId(
				`${blueprintId}_${partId}_piece_${i++}`
			);
...
export function postProcessAdLibPieces(
	...
	return _.map(_.compact(adLibPieces), (itemOrig: IBlueprintAdLibPiece) => {
		let piece: AdLibPiece = {
			...itemOrig,
			_id: innerContext.getHashId(
				`${blueprintId}_${partId}_adlib_piece_${i++}`
			),
			rundownId: innerContext.rundown._id,
			partId: partId,
			status: RundownAPI.PieceStatusCode.UNKNOWN,
...
function convertTimelineObject(
	rundownId: string,
	o: TimelineObjectCoreExt
): TimelineObjRundown {
	return {
...
export function postProcessRundownBaselineItems(
	...
	return _.map(
		...
		(o: TimelineObjGeneric, i): TimelineObjGeneric => {
			const obj: TimelineObjGeneric = convertTimelineObject(
				innerContext.rundown._id,
				o
			);
