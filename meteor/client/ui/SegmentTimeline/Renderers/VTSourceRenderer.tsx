...
export class VTSourceRendererBase extends CustomLayerItemRenderer<
...
> {
	...
	updateTime = () => {
		if (this.vPreview) {
			...
			const itemDuration =
				(piece.content
					? (piece.content.sourceDuration as number)
					: undefined) ||
		...
		if (this.props.piece) {
			...
			if (
				...
				return (
					this.props.mediaPreviewUrl +
					'media/preview/' +
					encodeURIComponent(metadata.mediaId)
				);
			}
		}
		return undefined;
	...
	getInspectorWarnings = (time: number): JSX.Element | undefined => {
		...
		const metadata = item.contentMetaData as MediaObject;
		const timebase =
			metadata.mediainfo && metadata.mediainfo.timebase
				? metadata.mediainfo.timebase
				: 20;
		if (this.state.blacks) {
			...
			if (tot > 0)
				msgBlacks = `${Math.round(tot / timebase)} black frame${
					tot > timebase ? 's' : ''
				} in clip`;
		}
		if (this.state.freezes) {
			...
			// @todo: hardcoded 25fps
			if (tot > 0)
				msgFreezes += `${Math.round(tot / timebase)} freeze\n frame${
					tot > timebase ? 's' : ''
				} in clip`;
		}
		if (show) {
