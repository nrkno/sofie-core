...
class SourceLayer extends React.Component<ISourceLayerProps> {
	...
	getPartContext = (props) => {
		const partElement = document.querySelector(
			'#' + SegmentTimelinePartElementId + this.props.part._id
		);
		...
		if (
			...
			typeof this.props.onContextMenu === 'function'
		) {
				...
				this.props.layer.pieces.filter((piece) => {
					...
					return piece.partId === this.props.part._id
						? // filter only pieces, that have not been hidden from the UI
						  piece.hidden !== true && piece.virtual !== true
						: false;
				})
			)
		...
		return (
			<div
				className={ClassNames('segment-timeline__output-group', {
					...
					collapsed:
						this.props.collapsedOutputs[this.props.layer._id] ===
						true
				})}>
				{DEBUG_MODE && (
...
export const SegmentTimelinePart = translate()(
	withTiming<IProps, IState>((props: IProps) => {
		...
		class SegmentTimelinePart0 extends React.Component<
			Translated<WithTiming<IProps>>,
			IState
		> {
			...
			constructor(props: Translated<WithTiming<IProps>>) {
				...
				const isLive =
					this.props.rundown.currentPartId === this.props.part._id;
				const isNext =
					this.props.rundown.nextPartId === this.props.part._id;
						...
						? Math.max(
								(startedPlayback &&
									...
									SegmentTimelinePart0.getCurrentLiveLinePosition(
										props.part,
										props.timingDurations.currentTime ||
											getCurrentTime()
									) +
										SegmentTimelinePart0.getLiveLineTimePadding(
											props.timeScale
										)) ||
									0,
								props.timingDurations.partDurations
									? props.part.displayDuration ||
											props.timingDurations.partDurations[
												props.part._id
											]
									: 0
			...
			static getDerivedStateFromProps(
				...
				const isLive =
					nextProps.rundown.currentPartId === nextProps.part._id;
				const isNext =
					nextProps.rundown.nextPartId === nextProps.part._id;

				const startedPlayback = nextProps.part.startedPlayback;

				const isDurationSettling =
					!!nextProps.rundown.active &&
					!isLive &&
					!!startedPlayback &&
					!nextProps.part.duration;

				const liveDuration =
					(isLive || isDurationSettling) &&
					!nextProps.autoNextPart &&
					!nextProps.part.autoNext
						? Math.max(
								(startedPlayback &&
									...
									0,
								nextProps.timingDurations.partDurations
									? nextProps.part.displayDuration ||
											nextProps.timingDurations
												.partDurations[
												nextProps.part._id
											]
			...
			static getCurrentLiveLinePosition(
				part: PartUi,
				currentTime: number
			) {
			...
			getLayerStyle() {
				...
				if (this.props.relative) {
					...
					return {
						minWidth:
							Math.floor(
								this.getPartDuration() * this.props.timeScale
							).toString() + 'px',
			...
			getPartDuration(): number {
				...
				return Math.max(
					...
					this.props.part.duration ||
						(this.props.timingDurations.partDisplayDurations &&
							this.props.timingDurations.partDisplayDurations[
								this.props.part._id
							]) ||
			...
			getPartStartsAt(): number {
				return Math.max(
					0,
					(this.props.firstPartInSegment &&
						this.props.timingDurations.partDisplayStartsAt &&
						this.props.timingDurations.partDisplayStartsAt[
							this.props.part._id
						] -
							this.props.timingDurations.partDisplayStartsAt[
								this.props.firstPartInSegment._id
							]) ||
						0
				);
			}
							...
							if (layer.used) {
								return (
									<OutputGroup
										...
										timeScale={this.props.timeScale}
										autoNextPart={this.props.autoNextPart}
										liveLinePadding={SegmentTimelinePart0.getLiveLineTimePadding(
			...
			render() {
				...
				const isEndOfShow =
					...
					(!this.state.isLive ||
						(this.state.isLive && !this.props.rundown.nextPartId));
				let invalidReasonColorVars:
					| React.CSSProperties
					| undefined = undefined;
				if (
					this.props.part.invalidReason &&
					this.props.part.invalidReason.color
				) {
					const invalidColor = this.convertHexToRgba(
						this.props.part.invalidReason.color
					);
					if (invalidColor) {
					...
					return (
						<div
							className={ClassNames('segment-timeline__part', {
								...
								'duration-settling': this.state
									.isDurationSettling
							})}
							data-obj-id={this.props.part._id}
							id={
								SegmentTimelinePartElementId +
								this.props.part._id
							}
							style={_.extend(
								this.getLayerStyle(),
							...
							{this.props.part.invalid ? (
								<div className="segment-timeline__part__invalid-cover"></div>
							) : null}
							{this.props.part.floated ? (
								<div className="segment-timeline__part__floated-cover"></div>
							) : null}
							...
							{this.props.rundown.nextTimeOffset &&
							this.state.isNext && ( // This is the off-set line
									<div
										className={ClassNames(
											'segment-timeline__part__nextline',
											{
												'auto-next': this.props.part
													.willProbablyAutoNext,
												invalid: this.props.part
													.invalid,
												floated: this.props.part.floated
											}
										)}
							...
							{this.state.isLive &&
								!this.props.relative &&
								!this.props.autoNextPart &&
								!this.props.part.autoNext && (
									<div
										className="segment-timeline__part__future-shade"
										style={this.getFutureShadeStyle()}></div>
								)}
