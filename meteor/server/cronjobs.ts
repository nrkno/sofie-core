	...
	function nightlyCronjob() {
		...
		if (
			...
			if (rundownCacheCount)
				logger.info(
					'Cronjob: Will remove cached data from ' + rundownCacheCount + ' rundowns'
				);
			...
			PeripheralDevices.find({
				...
				PeripheralDevices.find({
					...
					if (
						...
						logger.info(
							'Cronjob: Trying to restart CasparCG on device "' + subDevice._id + '"'
						);
