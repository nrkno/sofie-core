...
function checkDatabaseVersions() {
	...
	if (!databaseSystem) {
		...
		let dbVersion = databaseSystem.version
			? parseVersion(databaseSystem.version)
			: null;
	...
	if (expectVersion) {
		if (currentVersion) {
			...
			return {
				...
				messages: [
					`Current ${meName} version missing (when comparing with ${theyName})`
				]
			};
		}
	} else {
		return {
			statusCode: StatusCode.FATAL,
			messages: [
				`Expected ${theyName} version missing (when comparing with ${meName})`
			]
		};
	}
}
...
function checkBlueprintCompability(blueprint: Blueprint) {
	if (!PackageInfo.dependencies)
		throw new Meteor.Error(500, `Package.dependencies not set`);
	...
	let tsrStatus = checkDatabaseVersion(
		...
		parseRange(
			BlueprintIntegrationPackageInfo.dependencies[
	...
	if (coreStatus && coreStatus.statusCode >= StatusCode.WARNING_MAJOR) {
		...
		tsrStatus.messages[0] =
			'Core - TSR library version: ' + tsrStatus.messages[0];
		setSystemStatus(systemStatusId, tsrStatus);
	} else if (integrationStatus.statusCode >= StatusCode.WARNING_MAJOR) {
		integrationStatus.messages[0] =
			'Integration version: ' + integrationStatus.messages[0];
