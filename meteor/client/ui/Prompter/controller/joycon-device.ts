...
declare global {
	...
	type GamepadHapticsResult = 'invalid-parameter' | 'preempted' | 'not-supported' | 'complete'

	interface GamepadHapticActuator {
		canPlayEffectType(type: GamepadHapticEffectType)
		playEffect(type: GamepadHapticEffectType, params: object): Promise<GamepadHapticsResult>
		reset(): Promise<GamepadHapticsResult>
	}

	interface GamepadVibrationActuator extends GamepadHapticActuator { }

	interface Gamepad {
		vibrationActuator?: GamepadVibrationActuator
	}
}
...
export class JoyconController extends ControllerAbstract {
	...
	private _joycon: Gamepad | null = null
	private _haptic: GamepadHapticActuator | null = null
	...
	constructor(view: PrompterViewInner) {
		...
		window.addEventListener('gamepadconnected', (e: GamepadEvent) => {
			if (!this._joycon) {
				...
				if (this._joycon!.vibrationActuator) {
	...
	private _updateScrollPosition() {
		...
		if (this._currentPosition !== undefined && scrollPosition !== undefined) {
			if (this._currentPosition === scrollPosition) {
				...
				if (!this._hasReachedEnd && this._lastSpeed !== 0 && this._haptic) {
					this._hasReachedEnd = true
					this._haptic.playEffect('dual-rumble', { duration: 0.03, startDelay: 0, strongMagnitude: 0.08, weakMagnitude: 0 }).catch(console.error)
					setTimeout(() => {
						this._haptic && this._haptic.playEffect('dual-rumble', { duration: 0.02, startDelay: 0, strongMagnitude: 0.04, weakMagnitude: 0 }).catch(console.error)
					}, 70)
				}
