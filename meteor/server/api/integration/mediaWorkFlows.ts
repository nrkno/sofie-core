	...
	export function getMediaWorkFlowStepRevisions(id: string, token: string) {
		...
		if (peripheralDevice.studioId) {
			...
			throw new Meteor.Error(
				400,
				'Device "' + peripheralDevice._id + '" has no studio'
			);
		}
	}
...
let methods: Methods = {};
methods[PeripheralDeviceAPI.methods.getMediaWorkFlowRevisions] = (
	deviceId: string,
	deviceToken: string
) => {
	return MediaManagerIntegration.getMediaWorkFlowRevisions(
		deviceId,
		deviceToken
	);
};
methods[PeripheralDeviceAPI.methods.getMediaWorkFlowStepRevisions] = (
	deviceId: string,
	deviceToken: string
) => {
	return MediaManagerIntegration.getMediaWorkFlowStepRevisions(
		deviceId,
		deviceToken
	);
};
methods[PeripheralDeviceAPI.methods.updateMediaWorkFlow] = (
	deviceId: string,
	deviceToken: string,
	docId: string,
	obj: MediaWorkFlow | null
) => {
	return MediaManagerIntegration.updateMediaWorkFlow(
		deviceId,
		deviceToken,
		docId,
		obj
...
methods[PeripheralDeviceAPI.methods.updateMediaWorkFlowStep] = (
	deviceId: string,
	deviceToken: string,
	docId: string,
	obj: MediaWorkFlowStep | null
) => {
	return MediaManagerIntegration.updateMediaWorkFlowStep(
		deviceId,
		deviceToken,
		docId,
		obj
