...
export namespace UIStateStorage {
	...
	export function getItemBooleanMap(
		scope: string,
		tag: string,
		defaultValue: BooleanMap
	): BooleanMap {
	...
	export function getItemBoolean(
		scope: string,
		tag: string,
		defaultValue: boolean
	): boolean {
		return typeof (_collapsedState[scope] || {})[tag] === 'boolean'
			? !!_collapsedState[scope][tag]
			: defaultValue;
	}
	...
	function _cleanUp() {
		_.each(_collapsedState, (object, key) => {
			...
			if (
				modified === undefined ||
				Date.now() - Number(modified) > EXPIRATION_DATE
			) {
