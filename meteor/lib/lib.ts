 ...
 * @return {Promise<any>}
 */
export function MeteorPromiseCall(
	callName: string,
	...args: any[]
): Promise<any> {
...
>(
	...
	_.each(newData, (o) => {
		if (newObjIds[o[identifier]]) {
			throw new Meteor.Error(
				500,
				`saveIntoDb into collection "${collection.rawCollection.name}": Duplicate identifier ${identifier}: "${o[identifier]}"`
			);
		}
	...
	_.each(newData, function(o) {
		...
		if (oldObj) {
			...
			if (!eql) {
				...
				let oUpdate = options.beforeUpdate
					? options.beforeUpdate(o, oldObj)
					: o;
				if (options.update) {
	...
	_.each(removeObjs, function(obj: DocClass, key) {
		if (obj) {
			let p: Promise<any> | undefined;
			let oRemove: DBInterface = options.beforeRemove
				? options.beforeRemove(obj)
				: obj;
			if (options.remove) {
...
export const getCollectionIndexes: (
	collection: Mongo.Collection<any>
) => Array<any> = Meteor.wrapAsync(function getCollectionIndexes(
	collection: Mongo.Collection<any>,
	cb
) {
...
export const getCollectionStats: (
	collection: Mongo.Collection<any>
) => Array<any> = Meteor.wrapAsync(function getCollectionStats(
	collection: Mongo.Collection<any>,
	cb
) {
	let raw = collection.rawCollection();
...
export function fetchAfter<T>(
	collection: Mongo.Collection<T> | Array<T>,
	selector: MongoSelector<T>,
	rank: number | null
): T | undefined {
...
function cleanUpRateLimit() {
	...
	for (const name in rateLimitCache) {
		if (
			rateLimitCache[name] &&
			Math.abs(now - rateLimitCache[name]) > maxTime
		) {
...
export function rateLimitAndDoItLater(
	name: string,
	f1: Function,
	limit: number
) {
	...
	if (Math.random() < 0.05)
		Meteor.setTimeout(cleanUprateLimitAndDoItLater, 10000);
...
function cleanUprateLimitAndDoItLater() {
	...
	for (const name in rateLimitAndDoItLaterCache) {
		if (
			rateLimitAndDoItLaterCache[name] &&
			rateLimitAndDoItLaterCache[name] < now - 100
		) {
...
export function rateLimitIgnore(name: string, f1: Function, limit: number) {
	...
	if (Math.random() < 0.05)
		Meteor.setTimeout(cleanUprateLimitIgnore, 10 * 1000);
...
export function asyncCollectionUpsert<DocClass, DBInterface>(
	...
	return new Promise((resolve, reject) => {
		collection.upsert(
			...
			(
				err: any,
				returnValue: { numberAffected: number; insertedId: string }
			) => {
				if (err) reject(err);
 ...
 */
export const caught: <T>(v: Promise<T>) => Promise<T> = ((f) => (p) => (
	p.catch(f), p
))(() => {
	// nothing
});
 ...
 */
export const waitForPromiseAll: <T>(
	ps: Array<Promise<T>>
) => Array<T> = Meteor.wrapAsync(function waitForPromises<T>(
	ps: Array<Promise<T>>,
...
export const waitForPromise: <T>(p: Promise<T>) => T = Meteor.wrapAsync(
...
export function mutatePath<T>(
	...
	const generateWildcardAttrInfo = () => {
		const keys = _.filter(
			_.keys(substitutions),
			(k) => k.indexOf(currentPath) === 0
		);
		if (keys.length === 0) {
		...
		if (attr === '$') {
			...
			for (const obj of o) {
				...
				if (_.isMatch(obj, info.query)) {
					mutatePath(
						obj,
						path.substr(currentPath.length + 2),
						info.trimmedSubstitutions,
						mutator
					);
				}
			}
...
export function pullFromPath<T>(obj: Object, path: string, matchValue: T) {
	let mutator = (o: Object, lastAttr: string) => {
		if (_.has(o, lastAttr)) {
			...
			return (o[lastAttr] = _.filter(
				o[lastAttr],
				(entry: T) => !_.isMatch(entry, matchValue)
			));
		}
	};
	mutatePath(obj, path, {}, mutator);
...
export function setOntoPath<T>(
	...
	mutatePath(
		...
		path,
		substitutions,
		(parentObj: Object, key: string) => (parentObj[key] = valueToSet)
	);
}
/**
...
export function unsetPath(obj: Object, path: string, substitutions: Object) {
	mutatePath(
		...
		path,
		substitutions,
		(parentObj: Object, key: string) => delete parentObj[key]
	);
}
/**
 ...
 */
type Difference<A, B extends A> = Pick<
	B,
	Exclude<keyof B, keyof RequiredProperties<A>>
>;
/**
 ...
 */
export function extendMandadory<A, B extends A>(
	original: A,
	extendObj: Difference<A, B> & Partial<A>
): B {
...
export function firstIfArray<T>(
...
export type WrapAsyncCallback<T> = ((error: Error) => void) &
