...
const UserLogPlayerPage = translateWithTracker<
	IRecordingListProps,
	IRecordingListState,
		...
		updateSubscription() {
			if (this.props.match && this.props.match.params) {
				let h =
					...
					this.state.dateTo +
					'_' +
					this.props.match.params.rundownId;
				if (h !== this._currentsub) {
					...
					this._sub = meteorSubscribe(PubSub.userActionsLog, {
						args: {
							$regex: `.*"${this.props.match.params.rundownId}".*`
						},
						timestamp: {
		...
		executeAndProgress(action: NextUserLogAction) {
			...
			if (currentIndex === -1 || !nextItem) {
				...
				const nextAction = {
					message: nextItem,
					timeout: setTimeout(
						() => this.executeAndProgress(nextAction),
						targetTime - getCurrentTime()
					),
			...
			return (
				<p>
					<button
						...
						onClick={() => this.startExecution(msg)}>
						Play from here
					</button>
					<button
						className="action-btn mod mhm"
						onClick={(e) => this.executeSingle(e, msg)}>
						Exectute Single
					</button>
				</p>
			);
		...
		render() {
			...
			return (
				<div className="mtl gutter">
					...
					<div className="paging">{this.renderDatePicker()}</div>
...
const UserLogRundownSelect = translateWithTracker<
	IRundownSelectProps,
	IRundownSelectState,
	IRundownSelectTrackedProps
>((props: IRundownSelectProps) => {
	...
	class RundownSelection extends MeteorReactComponent<
