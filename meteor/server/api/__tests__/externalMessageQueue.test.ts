...
describe('Test external message queue static methods', () => {
	...
	testInFiber('toggleHold unknown id', () => {
		try {
			...
			expect(e.message).toBe(
				'[404] ExternalMessageQueue "cake" not found on toggleHold'
			);
		}
	});
	...
	testInFiber('retry unknown id', () => {
		try {
			...
			expect(e.message).toBe(
				'[404] ExternalMessageQueue "is_a_lie" not found on retry'
			);
		}
	});
	...
	testInFiber('setRunMessageQueue', () => {
		Meteor.call(
			ExternalMessageQueueAPI.methods.setRunMessageQueue,
			false,
			(err: Error) => {
