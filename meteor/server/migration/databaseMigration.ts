			...
			if (blueprint.blueprintType === BlueprintManifestType.SHOWSTYLE) {
				...
				ShowStyleBases.find({
					...
					let chunk: MigrationChunk = {
						...
						sourceName:
							'Blueprint ' + blueprint.name + ' for showStyle ' + showStyleBase.name,
						blueprintId: blueprint._id,
	...
	_.each(allMigrationSteps, (step: MigrationStepInternal) => {
		...
		if (
			...
			try {
				...
				throw new Meteor.Error(
					500,
					`Error in migration step "${step.id}": ${error.reason || error.toString()}`
				);
			}
...
function completeMigration(chunks: Array<MigrationChunk>) {
	_.each(chunks, (chunk) => {
		if (chunk.sourceType === MigrationStepType.CORE) {
			...
			if (!blueprint)
				throw new Meteor.Error(404, `Blueprint "${chunk.blueprintId}" not found!`);
