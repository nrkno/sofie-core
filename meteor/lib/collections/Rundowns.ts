...
export class Rundown implements DBRundown {
	...
	getShowStyleCompound(): ShowStyleCompound {
		if (!this.showStyleVariantId)
			throw new Meteor.Error(500, 'Rundown has no show style attached!');
		...
		if (ss) {
			...
			throw new Meteor.Error(
				404,
				`ShowStyle "${this.showStyleVariantId}" not found!`
			);
	}
	...
	getStudio(): Studio {
		if (!this.studioId)
			throw new Meteor.Error(500, 'Rundown is not in a studio!');
	...
	remove() {
		if (!Meteor.isServer)
			throw new Meteor.Error(
				'The "remove" method is available server-side only (sorry)'
			);
		...
		let ps: [
			...
			makePromise(() => {
				let parts = _.map(this.getParts(), (part) => {
					...
					part.getAllPieces = () => {
						return _.map(
							_.filter(pieces, (piece) => {
								return piece.partId === part._id;
							}),
							(part) => {
								return _.clone(part);
							}
...
export const Rundowns: TransformedCollection<
	Rundown,
	DBRundown
> = createMongoCollection<Rundown>('rundowns', {
