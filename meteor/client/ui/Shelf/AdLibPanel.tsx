...
export function matchFilter(
	...
	if (filter) {
		...
		// Source layer types
		const sourceLayerType = showStyleBase.sourceLayers.find(
			(i) => i._id === item.sourceLayerId
		);
		if (
			sourceLayerType &&
...
const AdLibListView = translate()(
	class extends React.Component<
		Translated<IListViewPropsHeader>,
		IListViewStateHeader
	> {
		...
		static getDerivedStateFromProps(props: IListViewPropsHeader, state) {
			...
			if (
				props.showStyleBase &&
				props.showStyleBase.outputLayers &&
		...
		componentDidUpdate(prevProps: IListViewPropsHeader) {
			if (
				...
				const segment: HTMLElement | null = document.querySelector(
					segmentSelector
				);
				if (segment) {
		...
		renderSegments() {
			return this.props.uiSegments
				.filter((a) =>
					this.props.filter
						? this.props.filter.currentSegment
							? a.isLive
							: true
						: true
				)
...
export const AdLibPanelToolbar = translate()(
	class AdLibPanelToolbar extends React.Component<
		Translated<IToolbarPropsHeader>,
		IToolbarStateHader
	> {
		...
		render() {
			...
			return (
				<div
					...
					<div
						className="adlib-panel__list-view__toolbar__buttons"
						style={{ display: 'none' }}>
...
export function fetchAndFilter(
	...
	const sharedHotkeyList = _.groupBy(
		props.showStyleBase.sourceLayers,
		(item) => item.activateKeyboardHotkeys
	);
	let segments: Array<Segment> = props.rundown.getSegments();

	const uiSegments = props.rundown
		? (segments as Array<SegmentUi>).map((segSource) => {
	...
	if (
		...
		(props.filter.rundownBaseline === true ||
			props.filter.rundownBaseline === 'only')
	) {
		const t = props.t;
				...
				props.showStyleBase.sourceLayers
					...
					.map((layer) =>
						literal<AdLibPieceUi>({
							...
							hotkey: layer.clearKeyboardHotkey
								? layer.clearKeyboardHotkey.split(',')[0]
								: '',
							name: t('Clear {{layerName}}', {
								layerName: layer.abbreviation || layer.name
							}),
							status: RundownAPI.PieceStatusCode.UNKNOWN,
							isSticky: false,
...
export const AdLibPanel = translateWithTracker<
	IAdLibPanelProps,
	IState,
	IAdLibPanelTrackedProps
>(
	...
	class AdLibPanel extends MeteorReactComponent<
		Translated<IAdLibPanelProps & IAdLibPanelTrackedProps>,
		IState
	> {
		...
		componentDidUpdate(prevProps: IAdLibPanelProps & IAdLibPanelTrackedProps) {
			...
			if (
				...
				this.props.liveSegment !== prevProps.liveSegment &&
				this.state.followLive
			) {
				this.setState({
					selectedSegment: this.props.liveSegment
				});
			...
			if (this.props.liveSegment && this.props.liveSegment.pieces) {
				this.props.liveSegment.pieces.forEach((item) => {
					if (item.hotkey) {
						...
						const sourceLayer = this.props.sourceLayerLookup[
							item.sourceLayerId
						];
						if (sourceLayer && sourceLayer.isQueueable) {
							const queueHotkey = [
								RundownViewKbdShortcuts.ADLIB_QUEUE_MODIFIER,
								item.hotkey
							].join('+');
							mousetrapHelper.bind(
								queueHotkey,
		...
		onToggleAdLib = (piece: AdLibPieceUi, queue: boolean, e: any) => {
			...
			if (
				...
				this.props.sourceLayerLookup[piece.sourceLayerId] &&
				!this.props.sourceLayerLookup[piece.sourceLayerId].isQueueable
			) {
				console.log(
					`Item "${piece._id}" is on sourceLayer "${piece.sourceLayerId}" that is not queueable.`
				);
				return;
			}
		...
		onSelectSegment = (segment: SegmentUi) => {
			...
			this.setState({
				...
				followLive: this.props.liveSegment
					? segment._id === this.props.liveSegment._id
					: true
			});
		};
