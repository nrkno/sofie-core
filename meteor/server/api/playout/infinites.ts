...
export const updateSourceLayerInfinitesAfterPart: (
	rundown: Rundown,
	previousPart?: Part,
	runUntilEnd?: boolean
) => void = syncFunctionIgnore(updateSourceLayerInfinitesAfterPartInner);
export function updateSourceLayerInfinitesAfterPartInner(
	rundown: Rundown,
	previousPart?: Part,
	runUntilEnd?: boolean
): void {
	...
	for (let part of partsToProcess) {
		...
		for (let k in activeInfiniteItemsSegmentId) {
			...
			if (
				!i.infiniteMode ||
				(i.infiniteMode === PieceLifespan.OutOnNextSegment &&
					s !== part.segmentId)
			) {
		...
		if (!currentItems)
			throw new Meteor.Error(
				500,
				`currentItemsCache didn't contain "${part._id}", which it should have`
			);

		let currentInfinites = currentItems.filter(
			(i) => i.infiniteId && i.infiniteId !== i._id
		);
		...
		const midInfinites = currentInfinites.filter(
			(i) => !i.enable.end && !i.enable.duration && i.infiniteMode
		);
		if (
			!runUntilEnd &&
			Object.keys(activeInfiniteItemsSegmentId).length === 0 &&
			midInfinites.length === 0
		) {
		...
		currentItems = currentItems.filter(
			(i) => removedInfinites.indexOf(i._id) < 0
		);
		...
		for (let k in activeInfinitePieces) {
			...
			const existingItems = currentItems.filter(
				(i) => i.sourceLayerId === newPiece.sourceLayerId
			);
			...
			if (
				existingPiece &&
				pieceToInsert &&
				_.isEqual(existingPiece, pieceToInsert)
			) {
				...
				existingPiece &&
				pieceToInsert &&
				existingPiece._id === pieceToInsert._id
			) {
		...
		currentItems = currentItems.filter(
			(i) => oldInfiniteContinuation.indexOf(i._id) < 0
		);
...
export const cropInfinitesOnLayer = syncFunction(function cropInfinitesOnLayer(
	rundown: Rundown,
	part: Part,
	newPiece: Piece
) {
	...
	const newItemExclusivityGroup =
		sourceLayerLookup[newPiece.sourceLayerId].exclusiveGroup;
			...
			(i) =>
				(i.sourceLayerId === newPiece.sourceLayerId ||
					(newItemExclusivityGroup &&
						...
						sourceLayerLookup[i.sourceLayerId].exclusiveGroup ===
							newItemExclusivityGroup)) &&
				i._id !== newPiece._id &&
				i.infiniteMode
		);
			...
			asyncCollectionUpdate(Pieces, piece._id, {
				$set: {
					userDuration: {
							...
							newPiece
						)}.start + ${newPiece.adlibPreroll || 0}`
					},
					infiniteMode: PieceLifespan.Normal,
					originalInfiniteMode:
						piece.originalInfiniteMode !== undefined
...
export const stopInfinitesRunningOnLayer = syncFunction(
	function stopInfinitesRunningOnLayer(
		rundown: Rundown,
		part: Part,
		sourceLayer: string
	) {
