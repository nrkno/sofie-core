...
export class Part implements DBPart {
		...
		if (runtimeNotes) {
			...
			_.each(pieces, (piece) => {
				...
				if (
					partLookup &&
					piece.sourceLayerId &&
					partLookup[piece.sourceLayerId]
				) {
					const part = partLookup[piece.sourceLayerId];
					const st = checkPieceContentStatus(
						piece,
						part,
	...
	getLastStartedPlayback() {
		...
		if (
			!this.timings.startedPlayback ||
			this.timings.startedPlayback.length === 0
		)
			return undefined;

		return this.timings.startedPlayback[
			this.timings.startedPlayback.length - 1
		];
	}
	getLastPlayOffset() {
		if (!this.timings) return undefined;

		if (!this.timings.playOffset || this.timings.playOffset.length === 0)
			return undefined;
...
export const Parts: TransformedCollection<Part, DBPart> = createMongoCollection<
	Part
>('parts', { transform: (doc) => applyClassToDocument(Part, doc) });
