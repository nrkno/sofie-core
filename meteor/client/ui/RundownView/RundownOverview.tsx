		...
		<ErrorBoundary>
			<div
				className={ClassNames('rundown__overview__segment__part', {
					...
					'has-played':
						props.part.startedPlayback &&
						(props.part.getLastStartedPlayback() || 0) > 0 &&
						(props.part.duration || 0) > 0
				})}
				style={{
					width:
						(Math.max(
							(props.segmentLiveDurations && props.segmentLiveDurations[props.part._id]) || 0,
				...
				{props.isNext && <div className="rundown__overview__segment__part__next-line"></div>}
				{props.isLive && (
					<div
...
const SegmentOverview: React.SFC<ISegmentPropsHeader> = (props: ISegmentPropsHeader) => {
	const segmentDuration = props.segmentLiveDurations
		? props.segment.items
				.map((i) => props.segmentLiveDurations[i._id])
		...
		(props.segment.items && (
			<div
				className={ClassNames('rundown__overview__segment', {
					...
					live: props.segment.items.find((i) => i._id === props.rundown.currentPartId)
						? true
						: false
				})}
				style={{
...
export const RundownOverview = withTiming<RundownOverviewProps, RundownOverviewState>()(
	withTracker<WithTiming<RundownOverviewProps>, RundownOverviewState, RundownOverviewTrackedProps>(
									...
									if (this.props.rundown) {
										return (
											<SegmentOverview
												...
												segmentStartsAt={
													(this.props.timingDurations && this.props.timingDurations.partStartsAt) ||
													{}
												}
											/>
										);
