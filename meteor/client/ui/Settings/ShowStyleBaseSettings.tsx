...
export default translateWithTracker<IProps, IState, ITrackedProps>(
	(props: IProps) => {
		let showStyleBase = ShowStyleBases.findOne(
			props.match.params.showStyleBaseId
		);
	...
	class ShowStyleBaseSettings extends MeteorReactComponent<
...
const StudioRuntimeArgumentsSettings = translate()(
	...
	> {
			...
			doModalDialog({
				...
				message: (
					<React.Fragment>
						<p>
							{t(
								'Are you sure you want to delete this runtime argument "{{property}}: {{value}}"?',
								{
		...
		renderItems() {
			const { t } = this.props;
			return (this.props.showStyleBase.runtimeArguments || []).map(
				(item, index) => {
					return (
...
const OutputSettings = translate()(
	class OutputSettings extends React.Component<
		Translated<IOutputSettingsProps>,
		IOutputSettingsState
	> {
			...
			doModalDialog({
				...
				message: (
					<React.Fragment>
						<p>
							{t(
								'Are you sure you want to delete source layer "{{outputId}}"?',
...
const HotkeyLegendSettings = translate()(
	...
	> {
		...
		renderItems() {
			const { t } = this.props;
			return (this.props.showStyleBase.hotkeyLegend || []).map(
				(item, index) => {
					return (
							...
							<tr
								...
								<td className="settings-studio-custom-config-table__actions table-item-actions c3">
									...
									<button
										className="action-btn"
										onClick={(e) =>
											this.onDeleteHotkeyLegend &&
											this.onDeleteHotkeyLegend(item)
										}>
										<FontAwesomeIcon icon={faTrash} />
									</button>
								</td>
			...
			return (
				<div>
					...
					<div className="mod mhs">
						<button
							className="btn btn-primary"
							onClick={this.onAddHotkeyLegend}>
							<FontAwesomeIcon icon={faPlus} />
						</button>
					</div>
				</div>
...
const ShowStyleVariantsSettings = translate()(
	...
	> {
		...
		onAddShowStyleVariant = () => {
			callMethod(
				'Menu',
				ShowStylesAPI.methods.insertShowStyleVariant,
				this.props.showStyleBase._id
			);
		};
		confirmRemove = (showStyleVariant: ShowStyleVariant) => {
			...
			doModalDialog({
				...
				onAccept: () => {
					callMethod(
						'ModalDialog',
						ShowStylesAPI.methods.removeShowStyleVariant,
				...
				message: (
					<React.Fragment>
						<p>
							{t(
								'Are you sure you want to remove the variant "{{showStyleVariantId}}"?',
			...
			return (
				<div>
					...
					<div className="mod mhs">
						<button
							className="btn btn-primary"
							onClick={this.onAddShowStyleVariant}>
							<FontAwesomeIcon icon={faPlus} />
						</button>
					</div>
				</div>
