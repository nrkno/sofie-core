...
>(
	(props: Translated<IAdLibPanelProps>) => {
		const unfinishedPieces = _.groupBy(
			props.rundown.currentPartId
				? Pieces.find({
	...
	class DashboardPanel extends MeteorReactComponent<
		Translated<
			IAdLibPanelProps &
	...
	> {
		...
		isAdLibOnAir(adLib: AdLibPieceUi) {
			if (
				this.props.unfinishedPieces[adLib._id] &&
				this.props.unfinishedPieces[adLib._id].length > 0
			) {
				return true;
			}
			return false;
							...
							style={{
								...
								left:
									filter.x >= 0
										...
										: filter.width < 0
										? `calc(${-1 * filter.width -
												1} * var(--dashboard-button-grid-width))`
										: undefined,
								top:
									filter.y >= 0
										? `calc(${filter.y} * var(--dashboard-button-grid-height))`
										: filter.height < 0
										? `calc(${-1 * filter.height -
												1} * var(--dashboard-button-grid-height))`
										: undefined,
								right:
									filter.x < 0
										? `calc(${-1 * filter.x -
