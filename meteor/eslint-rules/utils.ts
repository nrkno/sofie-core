/** https://github.com/jest-community/eslint-plugin-jest/blob/7cba106d0ade884a231b61098fa0bf33af2a1ad7/src/rules/utils.ts */
...
export type MaybeTypeCast<Expression extends TSESTree.Expression> = TSTypeCastExpression<Expression> | Expression
...
): Expression => (isTypeCastExpression(expression) ? followTypeAssertionChain(expression.expression) : expression)
 ...
 * @template V
 */
const isStringLiteral = <V extends string>(node: TSESTree.Node, value?: V): node is StringLiteral<V> =>
 ...
 * @template V
 */
const isTemplateLiteral = <V extends string>(node: TSESTree.Node, value?: V): node is TemplateLiteral<V> =>
 ...
 * @template V
 */
export const isStringNode = <V extends string>(node: TSESTree.Node, specifics?: V): node is StringNode<V> =>
 ...
 */
export interface CallExpressionWithSingleArgument<Argument extends TSESTree.Expression = TSESTree.Expression>
 ...
 */
export const hasOnlyOneArgument = (call: TSESTree.CallExpression): call is CallExpressionWithSingleArgument =>
 ...
 * @template V
 */
export const isIdentifier = <V extends string>(node: TSESTree.Node, name?: V): node is KnownIdentifier<V> =>
 ...
 */
interface ExpectMember<PropertyName extends ExpectPropertyName = ExpectPropertyName>
	extends KnownMemberExpression<PropertyName> {
	object: ExpectCall | ExpectMember
	parent: TSESTree.Node
}

export const isExpectMember = <Name extends ExpectPropertyName = ExpectPropertyName>(
	node: TSESTree.Node,
	name?: Name
): node is ExpectMember<Name> =>
	node.type === AST_NODE_TYPES.MemberExpression && isSupportedAccessor(node.property, name)
...
export const isParsedEqualityMatcherCall = <MatcherName extends EqualityMatcher = EqualityMatcher>(
	matcher: ParsedExpectMatcher,
	name?: MatcherName
): matcher is ParsedEqualityMatcherCall<TSESTree.Expression, MatcherName> =>
	(name ? matcher.name === name : EqualityMatcher.hasOwnProperty(matcher.name)) &&
...
type BaseParsedModifier<Modifier extends ModifierName = ModifierName> = ParsedExpectMember<Modifier>
 ...
 */
interface NegatableParsedModifier<Modifier extends NegatableModifierName = NegatableModifierName>
 ...
 */
export interface NotNegatableParsedModifier<Modifier extends NotNegatableModifierName = NotNegatableModifierName>
	extends BaseParsedModifier<Modifier> {
	negation?: never
}

export type ParsedExpectModifier = NotNegatableParsedModifier | NegatableParsedModifier
...
const parseExpectMember = <S extends ExpectPropertyName>(expectMember: ExpectMember<S>): ParsedExpectMember<S> => ({
	name: getAccessorValue<S>(expectMember.property),
	node: expectMember,
})

const reparseAsMatcher = (parsedMember: ParsedExpectMember): ParsedExpectMatcher => ({
	...parsedMember,
	/**
	 ...
	 */
	arguments:
		parsedMember.node.parent.type === AST_NODE_TYPES.CallExpression ? parsedMember.node.parent.arguments : null,
})
 ...
 */
const reparseMemberAsModifier = (parsedMember: ParsedExpectMember<ModifierName>): ParsedExpectModifier => {
 ...
 */
const shouldBeParsedExpectModifier = (member: ParsedExpectMember): member is ParsedExpectMember<ModifierName> =>
	ModifierName.hasOwnProperty(member.name)

export const parseExpectCall = <ExpectNode extends ExpectCall>(expect: ExpectNode): Expectation<ExpectNode> => {
...
export interface JestFunctionCallExpressionWithIdentifierCallee<FunctionName extends JestFunctionName>
	extends TSESTree.CallExpression {
	callee: JestFunctionIdentifier<FunctionName>
}

interface JestEachMemberExpression<TName extends Exclude<JestFunctionName, HookName>>
	extends KnownMemberExpression<'each'> {
	object: KnownIdentifier<TName> | (KnownMemberExpression & { object: KnownIdentifier<TName> })
}

export interface JestCalledEachCallExpression<TName extends Exclude<JestFunctionName, HookName>>
	extends TSESTree.CallExpression {
...
export interface JestTaggedEachCallExpression<TName extends Exclude<JestFunctionName, HookName>>
	extends TSESTree.CallExpression {
...
type JestEachCallExpression<TName extends Exclude<JestFunctionName, HookName>> =
	| JestCalledEachCallExpression<TName>
	| JestTaggedEachCallExpression<TName>

export type JestFunctionCallExpression<
	FunctionName extends Exclude<JestFunctionName, HookName> = Exclude<JestFunctionName, HookName>
> =
...
const joinNames = (a: string | null, b: string | null): string | null => (a && b ? `${a}.${b}` : null)
...
export const isHook = (
	...
	node.callee.type === AST_NODE_TYPES.Identifier && HookName.hasOwnProperty(node.callee.name)
...
export const getTestCallExpressionsFromDeclaredVariables = (
	...
	return declaredVariables.reduce<Array<JestFunctionCallExpression<TestCaseName>>>(
		(acc, { references }) =>
			acc.concat(
				references
					...
					.filter(
						(node): node is JestFunctionCallExpression<TestCaseName> =>
							!!node && node.type === AST_NODE_TYPES.CallExpression && isTestCaseCall(node)
					)
			),
		[]
	)
...
const isTestCaseName = (node: TSESTree.LeftHandSideExpression) =>
	node.type === AST_NODE_TYPES.Identifier && TestCaseName.hasOwnProperty(node.name)

const isTestCaseProperty = (
	node: TSESTree.Expression | TSESTree.PrivateIdentifier
): node is AccessorNode<TestCaseProperty> =>
	isSupportedAccessor(node) && TestCaseProperty.hasOwnProperty(getAccessorValue(node))
 ...
 */
export const isTestCaseCall = (node: TSESTree.CallExpression): node is JestFunctionCallExpression<TestCaseName> => {
	...
	if (callee.type === AST_NODE_TYPES.MemberExpression && isTestCaseProperty(callee.property)) {
...
const isDescribeAlias = (node: TSESTree.LeftHandSideExpression) =>
	node.type === AST_NODE_TYPES.Identifier && DescribeAlias.hasOwnProperty(node.name)

const isDescribeProperty = (
	node: TSESTree.Expression | TSESTree.PrivateIdentifier
): node is AccessorNode<DescribeProperty> =>
	isSupportedAccessor(node) && DescribeProperty.hasOwnProperty(getAccessorValue(node))
 ...
 * @return {node is JestFunctionCallExpression<TestCaseName>}
 */
export const isDescribeCall = (node: TSESTree.CallExpression): node is JestFunctionCallExpression<DescribeAlias> => {
	...
	if (callee.type === AST_NODE_TYPES.MemberExpression && isDescribeProperty(callee.property)) {
		// if we're an `each()`, ensure we're the outer CallExpression (i.e `.each()()`)
		if (
			getAccessorValue(callee.property) === 'each' &&
			node.callee.type !== AST_NODE_TYPES.TaggedTemplateExpression &&
...
export const scopeHasLocalReference = (scope: TSESLint.Scope.Scope, referenceName: string) => {
