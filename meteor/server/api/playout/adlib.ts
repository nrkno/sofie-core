...
export namespace ServerPlayoutAdLibAPI {
	...
	function innerStartAdLibPiece(
		...
		let nextPartInstance =
			rundownPlaylist.nextPartInstanceId && rundownPlaylist.nextPartInstanceId !== partInstanceId
		...
		if (!queue && nextPartInstance && partInstance && adLibPiece.sourceLayerId.match(/dve_box/)) {
			...
			if (
				partInstanceIds.indexOf(nextPartInstance._id) !== -1 &&
				partInstanceIds.indexOf(partInstance._id) === -1
			) {
