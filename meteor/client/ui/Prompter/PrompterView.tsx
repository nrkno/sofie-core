...
export class PrompterViewInner extends MeteorReactComponent<
...
> {
	...
	constructor(props) {
		...
		this.configOptions = {
			...
			followTake:
				queryParams['followtake'] === undefined
					? true
					: queryParams['followtake'] === '1',
			fontSize:
				parseInt(firstIfArray(queryParams['fontsize']) as string, 10) ||
				undefined,
			margin:
				parseInt(firstIfArray(queryParams['margin']) as string, 10) ||
				undefined,

			marker: (firstIfArray(queryParams['marker']) as any) || undefined,
			showMarker:
				queryParams['showmarker'] === undefined
					? true
					: queryParams['showmarker'] === '1',
			showScroll:
				queryParams['showscroll'] === undefined
					? true
					: queryParams['showscroll'] === '1'
		};
	...
	calculateMarginPosition() {
		let pixelMargin =
			((this.configOptions.margin || 0) * window.innerHeight) / 100;
		return pixelMargin;
	}
	scrollToLive() {
		const scrollMargin = this.calculateScrollPosition();
		const current =
			document.querySelector('.prompter .live') ||
			document.querySelector('.prompter .next');

		if (current) {
			Velocity(document.body, 'finish');
			Velocity(current, 'scroll', {
				offset: -1 * scrollMargin,
				duration: 400,
				easing: 'ease-out'
			});
		}
	...
	checkCurrentTakeMarkers = () => {
		...
		if (rundown !== undefined) {
			...
			const anchors: Array<Element> = Array.from(
			...
			for (let index = 0; index < anchors.length; index++) {
				...
				if (
					...
					current.classList.contains(`part-${rundown.currentPartId}`)
				) {
				...
				if (
					rundown.nextPartId &&
					current.classList.contains(`part-${rundown.nextPartId}`)
				) {
			...
			if (takeIndicator) {
				if (currentPositionEnd && currentPositionEnd < positionTop) {
					...
					currentPositionStart > positionBottom
				) {
...
export const PrompterView = translateWithTracker<IProps, {}, ITrackedProps>(
	(props: IProps) => {
...
export const Prompter = translateWithTracker<
	IPrompterProps,
	{},
	IPrompterTrackedProps
>((props: IPrompterProps) => {
	...
	class Prompter extends MeteorReactComponent<
		Translated<IPrompterProps & IPrompterTrackedProps>,
		IPrompterState
	> {
		...
		UNSAFE_componentWillUpdate() {
			...
			let readPosition =
				((this.props.config.margin || 0) * window.innerHeight) / 100;
		...
		renderPrompterData(prompterData: PrompterData) {
			...
			_.each(prompterData.lines, (line, i: number) => {
				...
				currentNextLine =
					this.props.currentPartId === line.partId
						? 'live'
						: this.props.nextPartId === line.partId
						? 'next'
						: null;
					...
					<div
						...
						<div className="overlay-fix">
							<div
								className={
									'read-marker ' +
									(!this.props.config.showMarker
										? 'hide'
										: this.props.config.marker || 'hide')
								}></div>
						...
						<div
							...
							style={{
								paddingLeft: this.props.config.margin
									? this.props.config.margin + 'vw'
									: undefined,
								paddingRight: this.props.config.margin
									? this.props.config.margin + 'vw'
									: undefined,
								paddingTop:
