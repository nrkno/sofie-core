...
export function getLookeaheadObjects(
	...
	_.each(studio.mappings || {}, (mapping: MappingExt, layerId: string) => {
		const lookaheadDepth =
			mapping.lookahead === LookaheadMode.PRELOAD ? mapping.lookaheadDepth || 1 : 1; // TODO - test other modes
		...
		_.each(lookaheadObjs.timed, (entry, i) => {
			...
			const finiteDuration =
				...
				(currentPart && currentPart.autoNext && entry.partId === activeRundown.nextPartId);
...
export function findLookaheadForlayer(
	...
	const { timeOrderedParts, currentPartIndex, currentSegmentId } = getPartsOrderedByTime(
		rundownData
	);
	...
	findObjectsForPart(
		...
		(startingPartIsFuture ? res.future : res.timed).push({ obj: o, partId: partId })
	);
