	...
	export function getMediaObjectRevisions(
		...
		if (studioId) {
			...
			throw new Meteor.Error(
				400,
				'getMediaObjectRevisions: Device "' +
	...
	export function updateMediaObject(
		...
		if (_.isNull(doc)) {
			...
			if (doc.mediaId !== doc.mediaId.toUpperCase())
				throw new Meteor.Error(
					400,
					'mediaId must only use uppercase characters'
				);
...
let methods: Methods = {};
methods[PeripheralDeviceAPI.methods.getMediaObjectRevisions] = (
	deviceId: string,
	deviceToken: string,
	collectionId: string
) => {
	return MediaScannerIntegration.getMediaObjectRevisions(
		deviceId,
		deviceToken,
		collectionId
	);
};
methods[PeripheralDeviceAPI.methods.updateMediaObject] = (
	deviceId: string,
	deviceToken: string,
	collectionId: string,
	id: string,
	...
	return MediaScannerIntegration.updateMediaObject(
		deviceId,
		deviceToken,
		collectionId,
		id,
