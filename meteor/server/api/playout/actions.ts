...
export function activateRundown(rundown: Rundown, rehearsal: boolean) {
	...
	if (anyOtherActiveRundowns.length) {
		...
		throw new Meteor.Error(
			...
			_.map(anyOtherActiveRundowns, (rundown) => rundown._id).join(',')
		);
	}
	...
	Meteor.defer(() => {
		...
		if (blueprint.onRundownActivate) {
			Promise.resolve(blueprint.onRundownActivate(new RundownContext(rundown, studio))).catch(
				logger.error
			);
		}
	});
...
export function deactivateRundown(rundown: Rundown) {
	...
	Meteor.defer(() => {
		...
		if (blueprint.onRundownDeActivate) {
			Promise.resolve(blueprint.onRundownDeActivate(new RundownContext(rundown))).catch(
				logger.error
			);
		}
	});
...
export function deactivateRundownInner(rundown: Rundown) {
	...
	const previousPart = rundown.currentPartId ? Parts.findOne(rundown.currentPartId) : null;
...
export function prepareStudioForBroadcast(studio: Studio) {
	...
	if (ssrcBgs.length > 1)
		logger.info(ssrcBgs[0].value + ' and ' + ssrcBgs[1].value + ' will be loaded to atems');
