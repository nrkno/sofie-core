...
export function getLookeaheadObjects(
	...
	const currentPart = activeRundown.currentPartId
		? rundownData.partsMap[activeRundown.currentPartId]
		: undefined;
	...
	const calculateStartAfterPreviousObj = (
		prevObj: TimelineObjRundown
	): TimelineTypes.TimelineEnable => {
		const prevHasDelayFlag =
			(prevObj.classes || []).indexOf('_lookahead_start_delay') !== -1;
	...
	_.each(studio.mappings || {}, (mapping: MappingExt, layerId: string) => {
		const lookaheadDepth =
			mapping.lookahead === LookaheadMode.PRELOAD
				? mapping.lookaheadDepth || 1
				: 1; // TODO - test other modes
		...
		_.each(lookaheadObjs.timed, (entry, i) => {
			...
			const finiteDuration =
				...
				(currentPart &&
					...
					entry.partId === activeRundown.nextPartId);
		...
		_.each(lookaheadObjs.future, (entry, i) => {
			...
			const enable =
				singleFutureObj && lastTimedObj
					? calculateStartAfterPreviousObj(lastTimedObj.obj)
					: { while: '1' };
...
export function findLookaheadForlayer(
	...
	if (mode === LookaheadMode.RETAIN && piecesUsingLayer.length === 1) {
		...
		if (piece.content && piece.content.timelineObjects) {
			const obj = piece.content.timelineObjects.find(
				(o) => o !== null && o.layer === layer
			);
			if (obj) {
				fixTimelineId(obj);
				return {
	...
	const {
		...
		currentPartIndex,
		currentSegmentId
	} = getPartsOrderedByTime(rundownData);
	if (timeOrderedParts.length === 0) {
		return { timed: [], future: [] };
	}

	const timeOrderedPartsWithPieces: PartInfoWithPieces[] = timeOrderedParts.map(
		(part) => ({
			...part,
	...
	const startingPartIsFuture =
		startingPartOnLayer.partId !== activeRundown.currentPartId;
	findObjectsForPart(
		rundownData,
		layer,
		timeOrderedPartsWithPieces,
		startingPartOnLayerIndex,
		startingPartOnLayer
	).forEach((o) =>
		(startingPartIsFuture ? res.future : res.timed).push({
			obj: o,
			partId: partId
		})
	);
	...
	while (nextPartOnLayerIndex !== -1 && res.future.length < lookaheadDepth) {
		nextPartOnLayerIndex = _.findIndex(
			timeOrderedPartsWithPieces,
			(v, i) => i > nextPartOnLayerIndex && v.pieces.length > 0
		);
...
function getPartsOrderedByTime(rundownData: RundownData) {
	...
	const currentIndex = parts.findIndex(
		(l) => l.partId === activeRundown.currentPartId
	);
...
function findObjectsForPart(
	...
	if (allObjs.length > 1) {
		if (startingPartOnLayer.part) {
			...
			const transObj2 = transObj
				? startingPartOnLayer.pieces.find((l) => l._id === transObj._id)
				: undefined;
			const hasTransition =
				allowTransition &&
				transObj2 &&
				...
				transObj2.content.timelineObjects.find(
					(o) => o != null && o.layer === layer
				);
			...
			orderedItems.forEach((i) => {
				...
				if (obj) {
					...
					let transitionKF:
						| TimelineTypes.TimelineKeyframe
						| undefined = undefined;
					if (allowTransition) {
						transitionKF = _.find(
							obj.keyframes || [],
							(kf) => kf.enable.while === '.is_transition'
						);
						...
						if (
							...
							classesFromPreviousPart.length > 0
						) {
					...
					const newContent = Object.assign(
						{},
						obj.content,
						transitionKF ? transitionKF.content : {}
					);
