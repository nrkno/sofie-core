...
export const onRONotificationClick = new ReactiveVar<
	((e: RONotificationEvent) => void) | undefined
>(undefined);
export const reloadRundownClick = new ReactiveVar<
...
class RundownViewNotifier extends WithManagedTracker {
	...
	stop() {
		...
		if (this._rundownImportVersionInterval)
			Meteor.clearInterval(this._rundownImportVersionInterval);
			...
			if (rundown) {
				...
				if (rundown.unsynced) {
					newNotification = new Notification(
						...
						t(
							'The Rundown has been UNSYNCED from ENPS! No data updates will currently come through.'
						),
						'Rundown',
						getCurrentTime(),
						true,
				...
				if (rundown.notes) {
					rundown.notes.forEach((note) => {
						...
						if (
							!Notification.isEqual(
								this._rundownStatus[rundownNoteId],
								newNotification
							)
						) {
							this._rundownStatus[
	...
	private reactivePeripheralDeviceStatus(studioId: string | undefined) {
		...
		this.autorun(() => {
			...
			const devices = reactivePeripheralDevices
				? reactivePeripheralDevices.get()
				: [];
		...
		this.autorun(() => {
			...
			_.flatten(
				...
				const id =
					...
					(item.origin.pieceId ||
						item.origin.partId ||
						item.origin.segmentId ||
						item.origin.rundownId) +
					'-' +
					item.origin.name +
					'-' +
					item.type;
				...
				if (
					!this._notes[id] ||
					!Notification.isEqual(newNotification, this._notes[id])
				) {
					...
					this._mediaStatusComps[piece._id] = Tracker.autorun(() => {
						...
						if (
							...
							newNotification = new Notification(
								...
								segment ? segment._id : 'line_' + piece.partId,
								getCurrentTime(),
								true,
								[
									{
	...
	private reactiveVersionStatus(rRundownId: string) {
		...
		if (this._rundownImportVersionInterval)
			Meteor.clearInterval(this._rundownImportVersionInterval);
		this._rundownImportVersionInterval = rRundownId
			? Meteor.setInterval(
					() => this.updateVersionStatus(rRundownId),
					updatePeriod
			  )
			: undefined;
	...
	private reactiveQueueStatus(studioId: string, rundownId: string) {
		...
		meteorSubscribe(PubSub.externalMessageQueue, {
			studioId: studioId,
			rundownId: rundownId
		});
		reactiveUnsentMessageCount = reactiveData.getUnsentExternalMessageCount(
			studioId,
			rundownId
		);
		this.autorun(() => {
			if (
				reactiveUnsentMessageCount.get() > 0 &&
				this._unsentExternalMessagesStatus === undefined
			) {
	...
	private updateVersionStatus(rundownId: string) {
		...
		Meteor.call(
			RundownAPI.methods.rundownNeedsUpdating,
			rundownId,
			(err: Error, versionMismatch: string) => {
				let newNotification: Notification | undefined = undefined;
				if (err) {
					newNotification = new Notification(
	...
	private makeDeviceMessage(device: PeripheralDevice): string {
		...
		if (!device.connected) {
			return t('Device {{deviceName}} is disconnected', {
...
export const RundownNotifier = class extends React.Component<IProps> {
	...
	shouldComponentUpdate(nextProps: IProps): boolean {
		if (
			this.props.rundownId === nextProps.rundownId &&
