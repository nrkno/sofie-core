...
describe('test peripheralDevice general API methods', () => {
	...
	testInFiber('setStatus', () => {
		expect(PeripheralDevices.findOne(device._id)).toBeTruthy();
		expect(
			(PeripheralDevices.findOne(device._id) as PeripheralDevice).status
		).toMatchObject({
	...
	testInFiber('pingWithCommand and functionReply', () => {
		...
		let command = PeripheralDeviceCommands.find({
			deviceId: device._id
		}).fetch()[0];
	...
	*/
	...
	testInFiber('killProcess with a rundown present', () => {
		...
		try {
			Meteor.call(
				PeripheralDeviceAPI.methods.killProcess,
				device._id,
				device.token,
				true
			);
	...
	*/
	...
	testInFiber('uninitialize', () => {
		setLoggerLevel('debug');
		Meteor.call(
			PeripheralDeviceAPI.methods.unInitialize,
			device._id,
			device.token
		);
