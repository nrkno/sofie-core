...
export function loadSystemBlueprints(
	system: ICoreSystem
): WrappedSystemBlueprint | undefined {
	...
	if (!blueprint)
		throw new Meteor.Error(
			404,
			`Blueprint "${system.blueprintId}" not found! (referenced by CoreSystem)`
		);

	if (blueprint.blueprintType !== BlueprintManifestType.SYSTEM) {
		throw new Meteor.Error(
			500,
			`Blueprint "${system.blueprintId}" is not valid for a CoreSystem!`
		);
	}
...
export function loadStudioBlueprints(
	studio: Studio
): WrappedStudioBlueprint | undefined {
	...
	if (blueprint.blueprintType !== BlueprintManifestType.STUDIO) {
		throw new Meteor.Error(
			500,
			`Blueprint "${studio.blueprintId}" is not valid for a Studio "${studio._id}"!`
		);
	}
...
export function getBlueprintOfRundownAsync(
	rundown: Rundown
...
export function getBlueprintOfRundown(
	...
	const fcn = () => {
		if (!rundown.showStyleBaseId)
			throw new Meteor.Error(
				400,
				`Rundown "${rundown._id}" is missing showStyleBaseId!`
			);
...
export function loadShowStyleBlueprints(
	showStyleBase: ShowStyleBase
): WrappedShowStyleBlueprint {
	if (!showStyleBase.blueprintId) {
		throw new Meteor.Error(
			500,
			`ShowStyleBase "${showStyleBase._id}" has no defined blueprint!`
		);
	}
...
export function evalBlueprints(
	blueprint: Blueprint,
	noCache?: boolean
): SomeBlueprintManifest {
	...
	if (!noCache) {
		...
		cached = blueprintCache[blueprint._id]
			? blueprintCache[blueprint._id]
			: null;
		if (
			cached &&
			(!cached.modified || cached.modified !== blueprint.modified)
		) {
	...
	if (cached) {
		...
		const entry = new SaferEval(context, {
			filename: (blueprint.name || blueprint._id) + '.js'
		}).runInContext(blueprint.code);
