...
export const SourceLayerItemContainer = class extends MeteorReactComponent<
	IPropsHeader
> {
	...
	updateDataTracker() {
		...
		this.statusComp = this.autorun(() => {
			...
			if (props.isLiveLine) {
				...
				let timelineObj = Timeline.findOne({
					id: getPieceGroupId(props.piece)
				});
				...
				if (timelineObj) {
					...
					if (_.isNumber(timelineObj.enable.start)) {
						// this is a normal absolute trigger value
						pieceCopy.renderedInPoint = timelineObj.enable.start;
					} else if (timelineObj.enable.start === 'now') {
						// this is a special absolute trigger value
						if (
							props.part &&
					...
					if (
						typeof timelineObj.enable.duration === 'number' &&
						!pieceCopy.cropped
					) {
			...
			if (props.piece.sourceLayer) {
				const { metadata, status } = checkPieceContentStatus(
					props.piece,
				...
				console.error(
					`Piece "${props.piece._id}" has no sourceLayer:`,
					props.piece
				);
			}
