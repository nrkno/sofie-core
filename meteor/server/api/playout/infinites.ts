...
export function updateSourceLayerInfinitesAfterPartInner(
	...
	for (let part of partsToProcess) {
		...
		for (let k in activeInfiniteItemsSegmentId) {
			...
			if (
				!i.infiniteMode ||
				(i.infiniteMode === PieceLifespan.OutOnNextSegment && s !== part.segmentId)
			) {
	...
	for (const piece of pieces) {
		ps.push(
			asyncCollectionUpdate(Pieces, piece._id, {
				$set: {
					userDuration: {
						end: `#${getPieceGroupId(newPiece)}.start + ${newPiece.adlibPreroll || 0}`
					},
...
export const stopInfinitesRunningOnLayer = syncFunction(function stopInfinitesRunningOnLayer(
	rundown: Rundown,
	part: Part,
	sourceLayer: string
) {
