...
describe('Test ingest actions for rundowns and segments', () => {
	...
	testInFiber('System migrations, initial setup', () => {
		...
		const migrationStatus0: GetMigrationStatusResult = Meteor.call(
			MigrationMethods.getMigrationStatus
		);
		...
		expect(
			migrationStatus0.migration.automaticStepCount
		).toBeGreaterThanOrEqual(1);

		const migrationResult0: RunMigrationResult = Meteor.call(
			MigrationMethods.runMigration,
		...
		const migrationStatus1: GetMigrationStatusResult = Meteor.call(
			MigrationMethods.getMigrationStatus
		);
		expect(migrationStatus1.migrationNeeded).toEqual(true);
		expect(
			migrationStatus1.migration.automaticStepCount
		).toBeGreaterThanOrEqual(1);

		const migrationResult1: RunMigrationResult = Meteor.call(
			MigrationMethods.runMigration,
			...
			userInput(migrationStatus1, {
				...
				'Studios.settings.supportedMediaFormats':
					'1920x1080i5000, 1280x720, i5000, i5000tff'
			})
		);
	...
	testInFiber('Ensure migrations run in correct order', () => {
		...
		expect(
			_.find(migration.steps, (s) => s.id.match(/myCoreMockStep1/))
		).toBeTruthy();
		expect(
			_.find(migration.steps, (s) => s.id.match(/myCoreMockStep2/))
		).toBeTruthy();
		expect(
			_.find(migration.steps, (s) => s.id.match(/myCoreMockStep3/))
		).toBeTruthy();
			...
			studioMigrations: [
				{
					...
					validate: (context: MigrationContextStudio) => {
						if (!context.getConfig('mocktest2'))
							return `mocktest2 config not set`;
						return false;
					},
					canBeRunAutomatically: true,
				...
				},
				{
					...
					validate: (context: MigrationContextStudio) => {
						if (!context.getConfig('mocktest3'))
							return `mocktest3 config not set`;
						return false;
					},
					canBeRunAutomatically: true,
					migrate: (context: MigrationContextStudio) => {
				...
				},
				{
					...
					validate: (context: MigrationContextStudio) => {
						if (!context.getConfig('mocktest1'))
							return `mocktest1 config not set`;
						return false;
					},
					canBeRunAutomatically: true,
					migrate: (context: MigrationContextStudio) => {
		...
		const myCoreMockStep1 = _.find(steps, (s) =>
			s.id.match(/myCoreMockStep1/)
		) as MigrationStep;
		const myCoreMockStep2 = _.find(steps, (s) =>
			s.id.match(/myCoreMockStep2/)
		) as MigrationStep;
		const myCoreMockStep3 = _.find(steps, (s) =>
			s.id.match(/myCoreMockStep3/)
		) as MigrationStep;
		const myStudioMockStep1 = _.find(steps, (s) =>
			s.id.match(/myStudioMockStep1/)
		) as MigrationStep;
		const myStudioMockStep2 = _.find(steps, (s) =>
			s.id.match(/myStudioMockStep2/)
		) as MigrationStep;
		const myStudioMockStep3 = _.find(steps, (s) =>
			s.id.match(/myStudioMockStep3/)
		) as MigrationStep;
		const myShowStyleMockStep1 = _.find(steps, (s) =>
			s.id.match(/myShowStyleMockStep1/)
		) as MigrationStep;
		const myShowStyleMockStep2 = _.find(steps, (s) =>
			s.id.match(/myShowStyleMockStep2/)
		) as MigrationStep;
		const myShowStyleMockStep3 = _.find(steps, (s) =>
			s.id.match(/myShowStyleMockStep3/)
		) as MigrationStep;
