...
export class RundownListItem extends React.Component<
	Translated<IRundownListItemProps>,
	IRundownListItemStats
> {
	constructor(props) {
		super(props);
	}
	...
	getshowStyleBaseLink(showStyleBaseId) {
		...
		return (
			...
			encodeURIComponent(encodeURIComponent(showStyleBaseId))
		);
	}
	...
	confirmReSyncRO(rundown: Rundown) {
		...
		doModalDialog({
			...
			message:
				t(
					'Are you sure you want to re-sync the "{{name}}" rundown with MOS script?',
					{ name: rundown.name }
				) +
				'\n' +
				t('Please note: This action is irreversible!')
			...
			<React.Fragment>
				<tr className="rundown-list-item">
					<th className="rundown-list-item__name">
						...
						<Link to={this.getRundownLink(this.props.rundown._id)}>
						...
						{getAllowConfigure() ? (
							<Link
								to={this.getshowStyleBaseLink(
									this.props.rundown.showStyleBaseId
								)}>{`${this.props.rundown.showStyleBaseName}-${this.props.rundown.showStyleVariantName}`}</Link>
						) : (
				...
				{this.props.rundown.startedPlayback !== undefined &&
					this.props.rundown.expectedDuration !== undefined &&
					this.props.rundown.active && (
		...
		rundowns: _.map(
			...
			(rundown) => {
				...
				const showStyleVariant = _.find(
					showStyleVariants,
					(s) => s._id === rundown.showStyleVariantId
				);
	...
	> {
				...
				<React.Fragment>
					{step === ToolTipStep.TOOLTIP_START_HERE ? (
						<ReactNotification
							...
							{t(
								'Enable configuration mode by adding ?configure=1 to the address bar.'
							)}
						</ReactNotification>
					) : (
						undefined
					...
					{step === ToolTipStep.TOOLTIP_START_HERE ||
					step === ToolTipStep.TOOLTIP_RUN_MIGRATIONS ? (
						<ReactNotification
							actions={[
								literal<NotificationAction>({
							...
							{t(
								'You need to run migrations to set the system up for operation.'
							)}
						</ReactNotification>
					) : (
						undefined
		...
		render() {
			...
			return (
				<React.Fragment>
					{this.props.coreSystem
						? this.registerHelp(this.props.coreSystem)
						: null}
					{this.props.coreSystem &&
					this.props.coreSystem.version === GENESIS_SYSTEM_VERSION &&
					synced.length === 0 &&
						...
						<div className="mtl gutter has-statusbar">
							...
							<div>
								<ul>
									<li>
										{t(
											'Start with giving this browser configuration permissions by adding this to the URL: '
										)}
										&nbsp;
										<Tooltip
						...
						{this.state.subsReady ? (
							<div className="mod mvl">
								<table className="table system-status-table expando expando-tight">
									<thead>
										<tr className="hl">
											<th className="c3">
												<Tooltip
													overlay={t(
														'Click on a rundown to control your studio'
													)}
													visible={getHelpMode()}
													placement="top">
													<span>{t('Rundown')}</span>
									...
									<tbody>{this.renderRundowns(synced)}</tbody>
									{unsynced.length > 0 && (
										<tbody>
											<tr className="hl">
												<th
...
const ActiveProgressBar = timer(1000)(
	class extends React.Component<IActiveProgressBarProps> {
		render() {
			return this.props.rundown.startedPlayback &&
				this.props.rundown.expectedDuration ? (
