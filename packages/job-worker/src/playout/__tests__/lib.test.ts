...
describe('selectNextPart', () => {
	...
	test('from nothing', () => {
		{
			// default
			const nextPart = selectNextPart(context, defaultPlaylist, null, getSegmentsAndParts())
	...
	test('from nothing - allow unplayable', () => {
		{
			// default
			const nextPart = selectNextPart(context, defaultPlaylist, null, getSegmentsAndParts(), false)
			expect(nextPart).toEqual({ index: 0, part: defaultParts[0] })
		}
	...
	test('from partInstance', () => {
		...
		{
			// default
			const nextPart = selectNextPart(context, defaultPlaylist, previousPartInstance, getSegmentsAndParts())
