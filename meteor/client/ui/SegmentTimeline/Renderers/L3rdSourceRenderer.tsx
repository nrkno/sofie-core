...
interface IState {}
export const L3rdSourceRenderer = translate()(
	class extends CustomLayerItemRenderer<IProps & InjectedTranslateProps, IState> {
		...
		componentDidUpdate(
			prevProps: Readonly<IProps & InjectedTranslateProps>,
			...
			return (
				<React.Fragment>
					<span
						...
						<span className="segment-timeline__piece__label">{this.props.piece.name}</span>
					</span>
					<span
					...
					<FloatingInspector
						key={this.props.piece._id + '-inspector'}
										...
										<tr>
											...
											<td className="mini-inspector__row--timing">
												...
												{this.props.piece.infiniteMode ? (
													(this.props.piece.infiniteMode === PieceLifespan.OutOnNextPart && (
														<span className="mini-inspector__duration">{t('Until next take')}</span>
													)) ||
													(this.props.piece.infiniteMode === PieceLifespan.OutOnNextSegment && (
														...
														{RundownUtils.formatTimeToShortTime(
															this.props.piece.renderedDuration ||
																(_.isNumber(this.props.piece.enable.duration)
																	? parseFloat((this.props.piece.enable.duration as any) as string)
																	: 0)
														)}
													</span>
												)}
