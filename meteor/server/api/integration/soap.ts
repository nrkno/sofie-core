...
type ExternalMessageQueueObjSOAP0 = ExternalMessageQueueObjSOAP &
	ExternalMessageQueueObj;
export async function sendSOAPMessage(
	msg: ExternalMessageQueueObjSOAP0 & ExternalMessageQueueObj
) {
	...
	if (!msg.message.fcn)
		throwFatalError(
			msg,
			new Meteor.Error(401, 'attribute .message.fcn missing!')
		);
	if (!msg.message.clip_key)
		throwFatalError(
			msg,
			new Meteor.Error(401, 'attribute .message.clip_key missing!')
		);
	if (!msg.message.clip)
		throwFatalError(
			msg,
			new Meteor.Error(401, 'attribute .message.clip missing!')
		);
	...
	let soapClient: soap.Client = await new Promise(
		(resolve: (soapClient: soap.Client) => any, reject) => {
	...
	msg.message.clip_key = await iterateDeeplyAsync(
		msg.message.clip_key,
		iteratee
	);
	...
	await new Promise((resolve, reject) => {
		...
		if (fcn) {
			...
			reject(
				new Meteor.Error(
					401,
					'SOAP method "' + msg.message.fcn + '" missing on endpoint!'
				)
			);
		}
	});
}
async function resolveSOAPFcnData(
	soapClient: soap.Client,
	valFcn: ExternalMessageQueueObjSOAPMessageAttrFcn
) {
