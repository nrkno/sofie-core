...
export namespace ServerPlayoutAdLibAPI {
	export function pieceTakeNow(rundownId: string, partId: string, pieceId: string) {
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (sourceL && sourceL.type !== SourceLayerType.GRAPHICS)
				throw new Meteor.Error(403, `Piece "${pieceId}" is not a GRAPHICS item!`);
	...
	export function segmentAdLibPieceStart(
		...
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (!adLibPiece)
				throw new Meteor.Error(404, `Part Ad Lib Item "${adLibPieceId}" not found!`);
	...
	export function stopAdLibPiece(rundownId: string, partId: string, pieceId: string) {
		...
		return rundownSyncFunction(rundownId, RundownSyncFunctionPriority.Playout, () => {
			...
			if (!piece)
				throw new Meteor.Error(404, `Part AdLib-copy-piece "${pieceId}" not found!`);
			if (!piece.dynamicallyInserted)
				throw new Meteor.Error(501, `"${pieceId}" does not appear to be a dynamic Piece!`);
