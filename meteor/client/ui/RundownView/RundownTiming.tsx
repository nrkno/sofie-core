...
>((props) => {
	...
	class RundownTimingProvider
		...
		>
		implements
			React.ChildContextProvider<IRundownTimingProviderChildContext> {
		...
		constructor(
			props: IRundownTimingProviderProps &
				IRundownTimingProviderTrackedProps
		) {
			super(props);

			this.refreshTimerInterval =
				props.refreshInterval || TIMING_DEFAULT_REFRESH_INTERVAL;
		...
		onRefreshTimer = () => {
			...
			const isLowResolution =
				this.refreshDecimator % LOW_RESOLUTION_TIMING_DECIMATOR === 0;
		...
		componentDidMount() {
			this.refreshTimer = Meteor.setInterval(
				this.onRefreshTimer,
				this.refreshTimerInterval
			);
			this.onRefreshTimer();
		...
		componentDidUpdate(
			prevProps: IRundownTimingProviderProps &
				IRundownTimingProviderTrackedProps
		) {
			// change refresh interval if needed
			if (
				this.refreshTimerInterval !== this.props.refreshInterval &&
				this.refreshTimer
			) {
				this.refreshTimerInterval =
					this.props.refreshInterval ||
					TIMING_DEFAULT_REFRESH_INTERVAL;
		...
		dispatchHREvent(now: number) {
			const event = new CustomEvent<TimeEventArgs>(
		...
		updateDurations(now: number, isLowResolution: boolean) {
			...
			_.keys(this.displayDurationGroups).forEach(
				(key) => delete this.displayDurationGroups[key]
			);
			...
			if (rundown && parts) {
				parts.forEach((part, itIndex) => {
					...
					const aIndex =
						this.linearParts.push([part._id, waitAccumulator]) - 1;
					...
					const playOffset =
						(part.timings &&
							part.timings.playOffset &&
							_.last(part.timings.playOffset)) ||
						0;
					...
					if (
						...
						asPlayedRundownDuration +=
							part.duration || part.expectedDuration || 0;
					}
					...
					if (
						...
						partDisplayDuration = Math.max(
							0,
							(part.duration && part.duration + playOffset) ||
								displayDurationFromGroup ||
								part.expectedDuration ||
								this.props.defaultDuration ||
								DEFAULT_DURATION
						);
						this.partPlayed[part._id] =
							(part.duration || 0) - playOffset;
					}

					// the part is the current part but has not yet started playback
					if (
						this.props.rundown &&
						this.props.rundown.currentPartId === part._id &&
						!part.startedPlayback
					) {
					...
					this.partExpectedDurations[part._id] =
						part.expectedDuration || part.duration || 0;
					this.partStartsAt[part._id] = startsAtAccumulator;
					this.partDisplayStartsAt[
						part._id
					...
					displayStartsAtAccumulator += this.partDisplayDurations[
						part._id
			...
			if (currentAIndex >= 0) {
				...
				let onAirPartDuration =
					currentLivePart.duration ||
					currentLivePart.expectedDuration ||
					0;
				if (
					currentLivePart.displayDurationGroup &&
					!currentLivePart.displayDuration
				) {
					onAirPartDuration =
						this.partDisplayDurations[currentLivePart._id] ||
						onAirPartDuration;
				}
...
export type TimingFilterFunction = (
	durations: RundownTiming.RundownTimingContext
) => any;
...
export type WithTiming<T> = T &
	RundownTiming.InjectedROTimingProps & { children?: React.ReactNode };
type IWrappedComponent<IProps, IState> = new (
...
export function withTiming<IProps, IState>(
	...
	return (WrappedComponent) => {
		return class WithTimingHOCComponent extends React.Component<
			IProps,
			IState
		> {
			static contextTypes = {
			...
			constructor(props, context) {
				...
				if (typeof expandedOptions.filter === 'function') {
					...
					this.filterGetter = _.property(
						expandedOptions.filter as string
					);
				}
			}
			...
			render() {
				const durations: RundownTiming.RundownTimingContext = this
					.context.durations;
...
export const PartCountdown = withTiming<IPartCountdownProps, {}>()(
	class PartCountdown extends React.Component<
		WithTiming<IPartCountdownProps>
	> {
		render() {
			return (
				<span>
...
export const CurrentPartRemaining = withTiming<IPartRemainingProps, {}>({
	...
	class CurrentPartRemaining extends React.Component<
		WithTiming<IPartRemainingProps>
	> {
		render() {
			const displayTimecode = this.props.timingDurations
				.remainingTimeOnCurrentPart;
			return (
				<span
					className={ClassNames(
						this.props.className,
						!!(Math.floor((displayTimecode || 0) / 1000) > 0)
		...
		speak() {
			...
			let displayTime =
				this.props.timingDurations.remainingTimeOnCurrentPart || 0;
...
export const SegmentDuration = withTiming<ISegmentDurationProps, {}>()(
	class SegmentDuration extends React.Component<
		WithTiming<ISegmentDurationProps>
	> {
		render() {
			if (
				this.props.partIds &&
				...
				let partExpectedDurations = this.props.timingDurations
					.partExpectedDurations;
