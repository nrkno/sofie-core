...
export const MediaManagerSettingsComponent = translate()(
	...
	> {
		...
		handleConfirmRemoveStorageAccept = (e) => {
			this.state.deleteConfirmStorageId &&
				this.removeStorage(this.state.deleteConfirmStorageId);
			this.setState({
				showDeleteStorageConfirm: false,
				deleteConfirmStorageId: undefined
			});
		...
		handleConfirmRemoveFlowAccept = (e) => {
			this.state.deleteConfirmFlowId &&
				this.removeFlow(this.state.deleteConfirmFlowId);
			this.setState({
				showDeleteFlowConfirm: false,
				deleteConfirmFlowId: undefined
			});
		...
		addNewStorage = () => {
			...
			let iter = 0;
			while (
				(settings.storages || []).findIndex(
					(i) => i.id === newDeviceId + iter.toString()
				) >= 0
			) {
				iter++;
			}
		...
		addNewFlow = () => {
			...
			let iter = 0;
			while (
				(settings.mediaFlows || []).findIndex(
					(i) => i.id === newFlowId + iter.toString()
				) >= 0
			) {
				iter++;
			}
					...
					<React.Fragment key={flow.id}>
						<tr
							...
							<td className="settings-studio-device__actions table-item-actions c3">
								<button
									className="action-btn"
									onClick={(e) => this.editFlowItem(flow.id)}>
									<FontAwesomeIcon icon={faPencilAlt} />
								</button>
								<button
									className="action-btn"
		...
		addNewMonitor = () => {
			...
			do {
				...
				_.find(
					_.keys(settings.monitors || {}),
					(monitorId) => monitorId === newMonitorId
				)
			);
			...
			doModalDialog({
				...
				message: (
					<React.Fragment>
						<p>
							{t(
								'Are you sure you want to delete the monitor "{{monitorId}}"?',
								{ monitorId: monitorId }
							)}
						</p>
					</React.Fragment>
		...
		onEditMonitorId = (
			...
			if (oldMonitorId && newMonitorId) {
				// @ts-ignore
				const settings: MediaManagerDeviceSettings =
				...
				if (!existingMonitor) {
					...
					doModalDialog({
						...
						message: (
							<React.Fragment>
								<p>
									{t('The ID {{monitorId}} already exists!', {
		...
		renderMonitors() {
			...
			return _.map(
				settings.monitors || {},
				(monitor: MonitorSettings, monitorId: string) => {
					return (
