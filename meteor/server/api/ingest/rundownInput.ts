...
export function rundownSyncFunction<T extends Function>(
	rundownId: string,
...
export namespace RundownInput {
	...
	export function dataRundownList(
		...
		const peripheralDevice = PeripheralDeviceSecurity.getPeripheralDevice(
			deviceId,
			deviceToken,
			self
		);
...
export function handleUpdatedRundownForStudio(
	studio: Studio,
...
export function updateRundownAndSaveCache(
	...
	logger.info(
		(existingDbRundown ? 'Updating' : 'Adding') + ' rundown ' + rundownId
	);
...
function updateRundownFromIngestData(
	...
	const rundownRes = showStyleBlueprint.getRundown(
		blueprintContext,
		ingestRundown
	);

	// Ensure the ids in the notes are clean
	const rundownNotes = _.map(blueprintContext.getNotes(), (note) =>
		literal<PartNote>({
			...note,
			origin: {
	...
	const showStyleBlueprintDb =
		(Blueprints.findOne(showStyle.base.blueprintId) as Blueprint) || {};
	...
	if (!dbRundown)
		throw new Meteor.Error(500, 'Rundown not found (it should have been)');
	...
	logger.info(
		`... got ${rundownRes.globalAdLibPieces.length} adLib objects from baseline.`
	);
	...
	const existingSegments = Segments.find({
		rundownId: dbRundown._id
	}).fetch();
	...
	_.each(ingestRundown.segments, (ingestSegment: IngestSegment) => {
		...
		const existingSegment = _.find(
			existingSegments,
			(s) => s._id === segmentId
		);
		const existingParts = existingRundownParts.filter(
...
function handleUpdatedSegment(
	...
	return rundownSyncFunction(
		...
		() => {
			...
			const updatedSegmentId = updateSegmentFromIngestData(
				studio,
				rundown,
				ingestSegment
			);
...
export function updateSegmentsFromIngestData(
	...
	for (let ingestSegment of ingestSegments) {
		const segmentId = updateSegmentFromIngestData(
			studio,
			rundown,
			ingestSegment
		);
...
export function handleRemovedPart(
	...
	return rundownSyncFunction(
		...
		() => {
			...
			ingestSegment.parts = ingestSegment.parts.filter(
				(p) => p.externalId !== partExternalId
			);
...
export function handleUpdatedPartInner(
	...
	ingestSegment.parts = ingestSegment.parts.filter(
		(p) => p.externalId !== ingestPart.externalId
	);
...
function generateSegmentContents(
	...
	const allNotes = _.map(context.getNotes(), (note) =>
		literal<PartNote>({
			...note,
			origin: {
				name: note.origin.name,
	...
	blueprintRes.parts.forEach((blueprintPart, i) => {
		...
		const notes = _.filter(
			allNotes,
			(note) => note.origin.partId === blueprintPart.part.externalId
		);
		...
		const adlibs = postProcessAdLibPieces(
			context,
