...
export default translateWithTracker<IProps, IState, ITrackedProps>((props: IProps) => {
	...
	> {
		...
		getOptionBlueprints() {
			return _.map(
				Blueprints.find({ blueprintType: BlueprintManifestType.SHOWSTYLE }).fetch(),
				(blueprint) => {
					return {
					...
					<React.Fragment>
						<p>
							{t(
								...
								{ property: item && item.property, value: item && item.value }
							)}
						</p>
											...
											<div className="mod mvs mhs">
												<label className="field">
													<EditAttribute
														modifiedClassName="bghl"
														attribute={
															'sourceLayers.' + item.index + '.assignHotkeysToGlobalAdlibs'
														}
														obj={this.props.showStyleBase}
														type="checkbox"
														collection={ShowStyleBases}
											...
											<div className="mod mvs mhs">
												<label className="field">
													...
													<EditAttribute
														modifiedClassName="bghl"
														attribute={
															'sourceLayers.' + item.index + '.activateStickyKeyboardHotkey'
														}
														obj={this.props.showStyleBase}
														type="text"
														collection={ShowStyleBases}
		...
		confirmDelete = (output: IOutputLayer) => {
			...
			doModalDialog({
				...
				message: (
					<React.Fragment>
						<p>
							{t('Are you sure you want to delete source layer "{{outputId}}"?', {
						...
						<tr
							...
							<td className="settings-studio-custom-config-table__actions table-item-actions c3">
								...
								<button
									...
									onClick={(e) => this.onDeleteHotkeyLegend && this.onDeleteHotkeyLegend(item)}>
									<FontAwesomeIcon icon={faTrash} />
								</button>
							</td>
						</tr>
							...
							<tr className="expando-details hl">
								<td colSpan={5}>
									...
									<div className="mod alright">
										<button
											className="btn btn-primary"
											onClick={(e) => this.finishEditItem(showStyleVariant._id)}>
											<FontAwesomeIcon icon={faCheck} />
										</button>
									</div>
								</td>
