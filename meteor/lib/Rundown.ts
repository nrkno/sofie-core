	...
	if (parts.length > 0) {
		if (checkFollowingSegment) {
			...
			if (followingParts.length > 0) {
				...
				followingPart = extendMandadory<Part, PartExtended>(
					firstFollowingPart,
					{
		...
		partsE = _.map(parts, (part, itIndex) => {
			...
			partE.renderedDuration =
				partE.expectedDuration || DEFAULT_DISPLAY_DURATION; // furthestDuration
		...
		const resolveDuration = (item: PieceExtended): number => {
			const expectedDurationNumber =
				typeof item.enable.duration === 'number'
...
export function calculatePieceTimelineEnable(
	...
	if (
		(end !== undefined || piece.enable.end !== undefined) &&
		...
		return {
					...
					? end
					: offsetTimelineEnableExpression(piece.enable.end, offset),
			duration: duration
		};
