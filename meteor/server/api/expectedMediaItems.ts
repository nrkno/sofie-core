...
function generateExpectedMediaItems(
	...
	if (
		piece.content &&
		piece.content.fileName &&
		piece.content.path &&
		piece.content.mediaFlowIds &&
		piece.partId
	) {
		(piece.content.mediaFlowIds as string[]).forEach(
			function(flow) {
				const id = getHash(
					pieceType + '_' + piece._id + '_' + flow + '_' + rundownId + '_' + piece.partId
				);
				result.push({
					_id: id,
					label: piece.name,
...
export const updateExpectedMediaItemsOnRundown: (rundownId: string) => void = syncFunctionIgnore(
	function updateExpectedMediaItemsOnRundown(rundownId: string) {
		...
		if (!rundown) {
			...
			logger.info(
				`Removed ${removedItems} expected media items for deleted rundown "${rundownId}"`
			);
			return;
		}
